...
namespace HomeMRU_Modulo
{
    public partial class MRUmi_Modulo : UserControl
    {
		...
		public string Armar_Link_MRU(string menuaccess,int menumsnro, String action , int menuraiz,int menunro, String menuname, int mrucant, string menudir)
		{
			...
			if (c_datos.Menu_Habilitado(menuaccess , menumsnro))	 
			{
				...
				salida =" <div onclick=\""+Corregir(action,menumsnro,menuraiz,menunro,menudir)+"\" class='MRUGeneral_Link'  style='background-color:"+BGColor+"'  title='"+ Traducir((String)Eval("menuname")) +"' > ";
 				salida+= Common.Utils.Armar_Icono("img/Modulos/SVG/LINK.svg", "IconoModuloMRUModulo",""," border='0' ", "");
				//salida+= "    <span style='font-size:7pt'>("+mrucant+") </span>   "+Traducir(menuname) ;
		  ...
		  if (string.IsNullOrEmpty(Utils.SessionUserName)){
            ...
            else
            {
               try{
						...
						sql +="  AND menuraiz.menudesc ='"+Utils.Session_ModuloActivo+"' ";
   ...
   public string Corregir(String action,int menumsnro, int MenuRaiz,int menunro,string menudir)
	{		
		...
		salida = Utils.ArmarAction(action,menudir,Convert.ToString(menumsnro), Convert.ToString(MenuRaiz) , Convert.ToString(menunro) ,"");
		  //salida = Utils.ArmarAction(action,Convert.ToString(Utils.Session_ModuloActivo), Convert.ToString(menumsnro), Convert.ToString(MenuRaiz) , Convert.ToString(menunro) );
