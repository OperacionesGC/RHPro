...
Case 4: ' Oracle 9
        ...
        '   Flog.writeline Espacios(Tabulador * 2) & "CreateTempTable " & NombreTabla
        'End If
        Select Case UCase(NombreTabla)
        Case "WF_TPA", "#WF_TPA":
        ...
        Case "WF_TT_SITREV", "#WF_TT_SITREV":
            cadena = TTempWF_tt_sitrev & "(codigo integer, diaini integer)"
        Case "WF_ESCALAUTM", "#WF_ESCALAUTM":
            cadena = TTempWF_EscalaUTM & "(desde FLOAT(63), hasta FLOAT(63), factor FLOAT(63), rebaja FLOAT(63))"
        Case "WF_RETROACTIVO", "#WF_RETROACTIVO":
        ...
        'If CBool(USA_DEBUG) Then
        '   Flog.writeline Espacios(Tabulador * 2) & "CreateTempTable SQL" & StrSql
        'End If
...
CE:
    ...
    Else
        'If CBool(USA_DEBUG) Then
        '   Flog.writeline Espacios(Tabulador * 2) & "CreateTempTable ERROR" & StrSql
        'End If
...
Public Sub Actualizar_WF_Tpa(ByVal I As Long, ByVal tpanro As Long, ByVal Orden As Long, ByVal Nom As String, ByVal Valor As Double, ByVal Fecha As Date)
Dim nombre As String

    nombre = "par" & Format(tpanro, "00000")
            
    Arr_WF_TPA(I).tipoparam = tpanro
    Arr_WF_TPA(I).ftorden = Orden
    Arr_WF_TPA(I).nombre = nombre
    Arr_WF_TPA(I).Valor = Valor
    Arr_WF_TPA(I).Fecha = Fecha
    
    'FGZ - 13/07/2011 ---- le agregué este campo para controlar si el parametro ya fué calculado
     Arr_WF_TPA(I).Calculado = True
End Sub
