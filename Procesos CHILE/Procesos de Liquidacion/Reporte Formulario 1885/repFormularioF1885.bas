Attribute VB_Name = "repFormularioF1885"
Global Const Version = "1.00" ' Dimatz Rafael
...
Global R_A As Integer
Global D_A As Integer
Global D_E As Integer
Global D_NC As Integer
Global I_I As Integer
Global C_Afect As Integer
Global C_E As Integer
Global C_Adicional As Integer
...
Dim empNom As String
    ...
    Nombre_Arch = PathFLog & "ReporteFormularioF1885" & "-" & NroProceso & ".log"
    ...
    If Not objRs.EOF Then
       ...
       Flog.writeline "Generando el reporte"
        ...
        'busco el rut de la empresa
         ...
         StrSql = "insert into rep_formularioF1885(bpronro,declarante_rut,declarante_razsoc,declarante_domicilio,declarante_comuna,declarante_email,declarante_fax,declarante_tel,prodesc)"
         ...
         StrSql = StrSql & ")"
         objConn.Execute StrSql, , adExecuteNoRecords
         Flog.writeline " Se Grabo el Formulario F1885"
        ...
        StrSql = StrSql & " WHERE repnro = 417"
        ...
        If Not HuboErrores Then
            ...
            Select Case objRs2!confnrocol
                ...
                Case 6
                    'acu rentas pagadas
                    C_Afect = objRs2!confval
                Case 7
                    'acu renta accesorias
                    C_E = objRs2!confval
                Case 8
                    'acu rem imponible
                    C_Adicional = objRs2!confval
            End Select
            objRs2.MoveNext
            Loop
           ...
           While Not rsEmpl.EOF
                ...
                Call ReporteFormularioF1885(listapronro, ternro, tituloReporte, orden)
...
'--------------------------------------------------------------------
Sub ReporteFormularioF1885(ListaPro As String, ternro As Long, descripcion As String, orden As Long)
...
Dim Receptor_Acciones
Dim Dividendos_afectos
Dim dividendos_exentos
Dim dividendos_noconstitutivos
Dim incrementoImpuesto
Dim Creditos_afectos
Dim Creditos_exentos
Dim Creditos_adicional

Dim l_Total_Dividendos_afectos
Dim l_Total_Dividendos_exentos
Dim l_Total_dividendos_noconstitutivos
Dim l_Total_incrementoImpuesto
Dim l_Total_Creditos_afectos
Dim l_Total_Creditos_exentos
Dim l_Total_Creditos_adicional
Dim l_Total_Receptor_Acciones
...
StrSql = "select * from rep_formulariof1885 inner join batch_proceso on rep_formulariof1885.bpronro = batch_proceso.bpronro where rep_formulariof1885.bpronro=" & NroProceso
OpenRecordset StrSql, rsaux

If Not rsaux.EOF Then
    'l_Total_Receptor_Acciones = rsaux!Receptor_Acciones
    If IsNull(rsaux!Dividendos_afectos) Then
          l_Total_Dividendos_afectos = 0
    Else
          l_Total_Dividendos_afectos = rsaux!Dividendos_afectos
    End If
    If IsNull(rsaux!dividendos_exentos) Then
        l_Total_Dividendos_exentos = 0
    Else
        l_Total_Dividendos_exentos = rsaux!dividendos_exentos
    End If
    If IsNull(rsaux!dividendos_noconstitutivos) Then
        l_Total_dividendos_noconstitutivos = 0
    Else
        l_Total_dividendos_noconstitutivos = rsaux!dividendos_noconstitutivos
    End If
    If IsNull(rsaux!incrementoImpuesto) Then
        l_Total_incrementoImpuesto = 0
    Else
        l_Total_incrementoImpuesto = rsaux!incrementoImpuesto
    End If
    If IsNull(rsaux!Creditos_afectos) Then
        l_Total_Creditos_afectos = 0
    Else
        l_Total_Creditos_afectos = rsaux!Creditos_afectos
    End If
    If IsNull(rsaux!Credito_exentos) Then
        l_Total_Creditos_exentos = 0
    Else
        l_Total_Creditos_exentos = rsaux!Credito_exentos
    End If
    If IsNull(rsaux!Creditos_adicional) Then
        l_Total_Creditos_adicional = 0
    Else
        l_Total_Creditos_adicional = rsaux!Creditos_adicional
    End If
End If
rsaux.Close
   ...
   proNro = ListaProAux(I)
        ...
        If Not rsConsult.EOF Then
          ...
          rsConsult.Close
            ...
            'busco la relacion contractual del empleado
            ...
            'busco valor de Receptor del Dividendo Cantidad de Acciones al 31/12
            sql = " SELECT almonto "
            sql = sql & " FROM acu_liq"
            sql = sql & " WHERE acunro = " & R_A 'acuLiqPago
            ...
            If Not rsConsult.EOF Then
                If IsNull(rsConsult!almonto) Or (rsConsult!almonto = "") Then
                    Receptor_Acciones = 0
                Else
                    Receptor_Acciones = Replace(CInt(rsConsult!almonto), ",", ".")
                    l_Total_Receptor_Acciones = Receptor_Acciones + l_Total_Receptor_Acciones
                End If
            Else
                Flog.writeline "No hay valor para Receptor Dividendo Cantidad de Acciones"
                
            End If
            rsConsult.Close
            
            'busco valor Receptor Dividendo Afectos
            sql = " SELECT almonto "
            sql = sql & " FROM acu_liq"
            sql = sql & " WHERE acunro = " & D_A 'acuImpUnicoR
            sql = sql & " AND cliqnro =  " & cliqnro
            OpenRecordset sql, rsConsult
            If Not rsConsult.EOF Then
                If IsNull(rsConsult!almonto) Or (rsConsult!almonto = "") Then
                    Dividendos_afectos = 0
                Else
                    Dividendos_afectos = Replace(CInt(rsConsult!almonto), ",", ".")
                    l_Total_Dividendos_afectos = Dividendos_afectos + l_Total_Dividendos_afectos
                End If
            Else
                Flog.writeline "No hay valor para Receptor Dividendo Afectos"
                
            End If
            rsConsult.Close
            
            'busco el valor Receptor Exentos
                sql = " SELECT almonto "
                sql = sql & " FROM acu_liq"
                sql = sql & " WHERE acunro = " & D_E 'acuCotPrev
                sql = sql & " AND cliqnro =  " & cliqnro
                OpenRecordset sql, rsConsult
                If Not rsConsult.EOF Then
                    If IsNull(rsConsult!almonto) Or (rsConsult!almonto = "") Then
                        dividendos_exentos = 0
                    Else
                        dividendos_exentos = Replace(CInt(rsConsult!almonto), ",", ".")
                        l_Total_Dividendos_exentos = dividendos_exentos + l_Total_Dividendos_exentos
                    End If
                Else
                    Flog.writeline "No hay valor para Receptor Exentos"
                End If
            rsConsult.Close
            
            'busco monto Dividendos No Constituivos
            sql = " SELECT almonto "
            sql = sql & " FROM acu_liq"
            sql = sql & " WHERE acunro = " & D_NC 'acuIsapre
            sql = sql & " AND cliqnro =  " & cliqnro
            OpenRecordset sql, rsConsult
            If Not rsConsult.EOF Then
                If IsNull(rsConsult!almonto) Or (rsConsult!almonto = "") Then
                    dividendos_noconstitutivos = 0
                Else
                    dividendos_noconstitutivos = Replace(CInt(rsConsult!almonto), ",", ".")
                    l_Total_dividendos_noconstitutivos = dividendos_noconstitutivos + l_Total_dividendos_noconstitutivos
                End If
            Else
                Flog.writeline "no hay valor para Dividendos No Constituivos"
            End If
            rsConsult.Close
                       
            'busco valor  del Incremento Impuesto
            sql = " SELECT almonto "
            sql = sql & " FROM acu_liq"
            sql = sql & " WHERE acunro = " & I_I 'acuRentaTotal
            sql = sql & " AND cliqnro =  " & cliqnro
            OpenRecordset sql, rsConsult
            If Not rsConsult.EOF Then
                If IsNull(rsConsult!almonto) Or (rsConsult!almonto = "") Then
                    incrementoImpuesto = 0
                Else
                    incrementoImpuesto = Replace(CInt(rsConsult!almonto), ",", ".")
                    l_Total_incrementoImpuesto = incrementoImpuesto + l_Total_incrementoImpuesto
                End If
            Else
                Flog.writeline "no hay valor para Incremento Impuesto"
            End If
            rsConsult.Close
            
            
            'busco valor Creditos Afectos
            sql = " SELECT almonto "
            sql = sql & " FROM acu_liq"
            sql = sql & " WHERE acunro = " & C_Afect 'acuRentasPagadas
            sql = sql & " AND cliqnro =  " & cliqnro
            OpenRecordset sql, rsConsult
            If Not rsConsult.EOF Then
                If IsNull(rsConsult!almonto) Or (rsConsult!almonto = "") Then
                    Creditos_afectos = 0
                Else
                    Creditos_afectos = Replace(CInt(rsConsult!almonto), ",", ".")
                    l_Total_Creditos_afectos = l_Total_Creditos_afectos + Creditos_afectos
                End If
            Else
                Flog.writeline "no hay valor para Creditos Afecto"
                
            End If
            rsConsult.Close
            
            
            'busco valor Creditos Exentos
            sql = " SELECT almonto "
            sql = sql & " FROM acu_liq"
            sql = sql & " WHERE acunro = " & C_E 'acuRentasAccesorias
            sql = sql & " AND cliqnro =  " & cliqnro
            OpenRecordset sql, rsConsult
            If Not rsConsult.EOF Then
                If IsNull(rsConsult!almonto) Or (rsConsult!almonto = "") Then
                    Creditos_exentos = 0
                Else
                    Creditos_exentos = Replace(CInt(rsConsult!almonto), ",", ".")
                    l_Total_Creditos_exentos = l_Total_Creditos_exentos + Creditos_exentos
                End If
            Else
                Flog.writeline "no hay valor para Creditos Exentos"
                
            End If
            rsConsult.Close

            'busco valor Creditos Adicional
            sql = " SELECT almonto "
            sql = sql & " FROM acu_liq"
            sql = sql & " WHERE acunro = " & C_Adicional 'acuRemImponible
            sql = sql & " AND cliqnro =  " & cliqnro
            OpenRecordset sql, rsConsult
            If Not rsConsult.EOF Then
                If IsNull(rsConsult!almonto) Or (rsConsult!almonto = "") Then
                    Creditos_adicional = 0
                Else
                    Creditos_adicional = Replace(CInt(rsConsult!almonto), ",", ".")
                    l_Total_Creditos_adicional = Creditos_adicional + l_Total_Creditos_adicional
                End If
            Else
                Flog.writeline "no hay valor para Creditos Adicional"
            ...
            StrSql = " INSERT INTO rep_formulariof1885_det "
            StrSql = StrSql & " (bpronro, anio, mes, Pagador_rut, Receptor_Acciones, Dividendos_afectos, Dividendos_exentos, Dividendos_noconstitutivos, incrementoImpuesto, "
            StrSql = StrSql & " Creditos_afectos, Creditos_exentos, Creditos_adicional, nroCertificado,ternro) "
            StrSql = StrSql & " VALUES "
            StrSql = StrSql & "(" & NroProceso
            StrSql = StrSql & ",'" & pliqAnio & "'"
            StrSql = StrSql & ",'" & pliqMes & "'"
            StrSql = StrSql & ",'" & det_rut & "'"
            StrSql = StrSql & "," & Receptor_Acciones
            StrSql = StrSql & "," & Dividendos_afectos
            StrSql = StrSql & "," & dividendos_exentos
            StrSql = StrSql & "," & dividendos_noconstitutivos
            StrSql = StrSql & "," & incrementoImpuesto
            StrSql = StrSql & "," & Creditos_afectos
            StrSql = StrSql & "," & Creditos_exentos
            StrSql = StrSql & "," & Creditos_adicional
            StrSql = StrSql & ",'" & Null & "'"
...
StrSql = " UPDATE rep_formulariof1885 set Receptor_Acciones= " & l_Total_Receptor_Acciones & ",Dividendos_afectos= " & l_Total_Dividendos_afectos & ","
StrSql = StrSql & " dividendos_exentos=" & l_Total_Dividendos_exentos & ",dividendos_noconstitutivos=" & l_Total_dividendos_noconstitutivos & ","
StrSql = StrSql & " incrementoImpuesto=" & l_Total_incrementoImpuesto & ",Creditos_afectos=" & l_Total_Creditos_afectos & ","
StrSql = StrSql & " Credito_exentos=" & l_Total_Creditos_exentos & ",Creditos_adicional =" & l_Total_Creditos_adicional & ""
StrSql = StrSql & " Where bpronro = " & NroProceso
objConn.Execute StrSql, , adExecuteNoRecords
Flog.writeline " Se actualizo el Formulario F1885"
  

Exit Sub

MError:
    Flog.writeline "Error en Formulario F1885: " & NroProceso & " Error: " & Err.Description
