Attribute VB_Name = "MdlIsapres"
Option Explicit

'Const Version = 1.09
'Const FechaVersion = "23/01/2007"
'Autor = Martin Ferraro
'Reporte ISAPRES para Chile - Se tomo en cuenta el SIJP

Global Const Version = "1.10"
Global Const FechaModificacion = "02/02/2016"
Global Const UltimaModificacion = "Dimatz Rafael CAS-32780 - RH Pro (Producto) - Chile - Resp. legal y Rep. relacionados"
...
Public Sub Main()
' ---------------------------------------------------------------------------------------------
' Descripcion: Procedimiento inicial del Generador de Reporte ISAPRES.
' Autor      : Martin Ferraro
' Fecha      : 23/01/2007
' Ultima Mod.:
' Descripcion:
...
Dim ArrParametros
    ...
    Nombre_Arch = PathFLog & "Generacion_Reporte_ISAPRES" & "-" & NroProcesoBatch & ".log"
    ...
    StrSql = "SELECT * FROM batch_proceso WHERE btprcnro = 145 AND bpronro =" & NroProcesoBatch
    ...
    If Not rs_batch_proceso.EOF Then
        ...
        Call Isapre(NroProcesoBatch, bprcparam)
    Else
...
Public Sub Isapre(ByVal bpronro As Long, ByVal Parametros As String)
' --------------------------------------------------------------------------------------------
' Descripcion: Procedimiento de generacion del reporte del isapres
' Autor      : Martin Ferraro
' Fecha      : 23/01/2007
' --------------------------------------------------------------------------------------------
...
If Not IsNull(Parametros) Then
    ...
    If UBound(arrParam) = 13 Then
        ...
        Flog.writeline Espacios(Tabulador * 1) & "Isapre = " & EstrnroOS
...
StrSql = StrSql & " empleado.ternom, empleado.ternom2, proceso.prodesc, isapre.estrnro osnro, isapreestr.estrdabr osdesc "
...
'Filtro Obra Social (ISAPRES)
StrSql = StrSql & " INNER JOIN his_estructura isapre ON empleado.ternro = isapre.ternro "
StrSql = StrSql & " AND isapre.tenro = 17 "
StrSql = StrSql & " AND isapre.htetdesde <= " & ConvFecha(FecDesdePer) & " AND (isapre.htethasta IS NULL OR isapre.htethasta >= " & ConvFecha(FecDesdePer) & ") "
If EstrnroOS <> 0 Then
    StrSql = StrSql & " AND isapre.estrnro = " & EstrnroOS
End If
StrSql = StrSql & " INNER JOIN estructura isapreestr ON isapreestr.estrnro = isapre.estrnro "
...
StrSql = StrSql & " ORDER BY osdesc , " & OrdenSql & " , proceso.pronro"
...
Do While Not rs_Empleados.EOF
    ...
    If EsElUltimoEmpleado(rs_Empleados, UltimoEmpleado) Then
        ...
        If UltimaOS <> rs_Empleados!osnro Then
            
            'Inserto la cabecera
            Flog.writeline Espacios(Tabulador * 1) & "Insertando ISAPRE " & rs_Empleados!osnro & " - " & rs_Empleados!osdesc
            StrSql = " INSERT INTO rep_isapre "
            StrSql = StrSql & " ("
            StrSql = StrSql & " bpronro,"
            StrSql = StrSql & " rut_rl,"
            StrSql = StrSql & " isanro,"
            StrSql = StrSql & " isades,"
            ...
            StrSql = StrSql & " , '" & Mid(EmpreRUT, 1, 12) & "'"
            StrSql = StrSql & " , " & rs_Empleados!osnro
            StrSql = StrSql & " , '" & Mid(rs_Empleados!osdesc, 1, 50) & "'"
        ...
        StrSql = " INSERT INTO rep_isapre_det "
        StrSql = StrSql & " ("
        StrSql = StrSql & " bpronro,"
    ...
    StrSql = "UPDATE batch_proceso SET bprcprogreso = " & Replace(Progreso, ",", ".") & _
