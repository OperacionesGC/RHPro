...
Flog.writeline "Inicio Proceso:" & Nombre & " " & NroProc & " " & Format(CDate(Now), FormatoInternoFecha)
...
Dim i As Integer
    ...
    Ejecutando(i).HoraInicioEj = Format(CDate(Now), FormatoInternoFecha)
...
If CantProc >= MaxConcurrentes Then
    Flog.writeline "Estan corriendo el maximo Posible de Procesos" & Format(CDate(Now), FormatoInternoFecha)
End If
...
'             " AND ((bprcfecha < " & ConvFecha(Date) & ") OR ((bprcfecha = " & ConvFecha(Date) & ") AND ((bprchora < '" & Format(Time, "hh:mm:ss") & "') OR (bprchora < '" & Format(Time, "h:mm:ss") & "'))))" & _
'             " ORDER BY  bprcurgente, bpronro"
    ...
    If objrsProcesosPendientes.EOF Then
        Flog.writeline "No Hay Pendientes " & Format(CDate(Now), FormatoInternoFecha)
    End If
    ...
    If Not objrsProcesosPendientes.EOF And PuedeEjecutarConcurrente() Then
        ...
        Do While (Actual <= Ultimo) And seguir
            If PuedeEjecutar(Pendientes(Actual).Proceso, Pendientes(Actual).TipoProceso) Then
                If Format(CDate(Pendientes(Actual).bprchora), FormatoInternoHora) <= Format(Time, FormatoInternoHora) Then
                    ...
                    End If
                Else
                    Flog.writeline "Aun no se puede ejecutar el proceso " & Pendientes(Actual).Proceso & " de tipo " & Pendientes(Actual).TipoProceso & ". Esta programado para las " & Format(CDate(Pendientes(Actual).bprchora), FormatoInternoHora) & " y son las " & Format(CDate(Time), FormatoInternoHora)
                End If
            Else
...
For i = 1 To UBound(Ejecutando)
    If Ejecutando(i).Proceso <> 0 Then
        ...
        If hProc = 0 Then ' ya no esta en memoria
            ...
            Ejecutando(i).Progreso = 0
            Ejecutando(i).HoraInicioEj = Format(CDate(Time), FormatoInternoFecha)
            Ejecutando(i).HoraFinEj = Ejecutando(i).HoraInicioEj
        End If
    End If
Next i
...
Dim Ok As Long
    ...
    StrSql = "SELECT * " & _
             ...
             " WHERE bprcfecha   >=" & ConvFecha(CDate(Date - 10)) & _
    ...
    Do While Not rsEj.EOF
        ...
        StrSql = "UPDATE batch_proceso SET bprchorafinej = '" & Format(CDate(Time), FormatoInternoHora) & "', bprcfecfinej = " & ConvFecha(Now) & ", bprcestado = 'Abortado por Usuario'" & _
...
Dim TerminarProceso As Boolean
    ...
    Do While Not rsEj.EOF
        ...
        Else
            If DateDiff("n", Format(CDate(Time), FormatoInternoHora), Format(CDate(rsEj!bprcHoraFinEj), FormatoInternoHora)) > TiempoDeEsperaNoResponde Then
                TerminarProceso = True
            Else
        ...
        If TerminarProceso Then
            ...
            ' actualizo los datos del proceso
            StrSql = "UPDATE batch_proceso SET bprchorafinej = '" & Format(CDate(Time), FormatoInternoHora) & "', bprcfecfinej = " & ConvFecha(Now) & ", bprcestado = 'Abortado'" & _
            " WHERE bpronro = " & rsEj!bpronro
            objConn.Execute StrSql, , adExecuteNoRecords
        End If
        rsEj.MoveNext
...
'             "ORDER BY bpronro desc "
    ...
    Do While Not rsEj.EOF
        ...
        If Ejecutando(MiIndice).Progreso = rsEj!bprcprogreso Then
           ...
           If DateDiff("n", Format(CDate(Ejecutando(MiIndice).HoraFinEj), FormatoInternoHora), Format(CDate(Time), FormatoInternoHora)) > TiempoDeEsperaSinProgreso Then
                ...
                StrSql = "UPDATE batch_proceso SET bprchorafinej = '" & Format(Time, "hh:mm:ss") & "', bprcfecfinej = " & ConvFecha(Now) & ", bprcestado = 'No Responde'" & _
        ...
        Else
            ...
            If IsNull(rsEj!bprcprogreso) Then
                ...
                End If
            Else
                Flog.writeline "Proceso " & rsEj!bpronro & " indice : " & MiIndice & ", HoraFinEj: " & Format(Time, "dd/mm/yyyy hh:mm:ss") & " - " & Now
                Ejecutando(MiIndice).Progreso = rsEj!bprcprogreso
                Ejecutando(MiIndice).HoraFinEj = Format(CDate(Time), FormatoInternoHora)
            End If
        End If
...
Public Sub LimpioProceso(ByVal Indice As Long)
    ...
    Ejecutando(Indice).HoraInicioEj = Format(CDate(Time), FormatoInternoHora)
    Ejecutando(Indice).HoraFinEj = Format(CDate(Time), FormatoInternoHora)
...
Do While i <= UBound(Pendientes)
    ...
    Ejecutando(i).Proceso = 0
    Ejecutando(i).Progreso = 0
    Ejecutando(i).HoraFinEj = Format(CDate(Now), FormatoInternoFecha)
    Ejecutando(i).HoraInicioEj = Format(CDate(Now), FormatoInternoFecha)
    i = i + 1
Loop
End Sub
