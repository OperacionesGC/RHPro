Attribute VB_Name = "MdlFirmawareSerial"
Option Explicit
...
'Vendor specific feature register defines for SMART "sub commands"
        ...
        For drvNumber = PRIMARY_MASTER To QUARTIARY_SLAVE
           ...
           With di
              Select Case .bDriveType
                 ...
                 Case 1
                    'Modelo (.Model)
                    'Nro Serie: (.SerialNumber)
                    Configurado = HDD_Configurado(Trim$(.SerialNumber), m_Drive) Or Configurado
                 Case 2
                    'ATAPI drive - informacion no disponible
                 Case Else
           ...
           With di
              Select Case .bDriveType
                 ...
                 Case 1
                    'Modelo (.Model)
                    'Nro Serie: (.SerialNumber)
                    Call Instalar_HDD(Trim$(.SerialNumber), m_Drive)
                 Case 2
                    'ATAPI drive - informacion no disponible
                 Case Else
                    'Tipo de dispositivo desconocido
