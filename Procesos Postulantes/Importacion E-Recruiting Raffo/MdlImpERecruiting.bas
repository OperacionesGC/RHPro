...
'Const Version = "1.17"
'Const FechaVersion = "27/02/2014" 'JPB - CAS-21159 - Raffo - Adecuaciones E-recruting
                                  '   Se adecuó el proceso Importar2 para que incluya nuevos campos de la tabla pos_postulante
                                  
'Const Version = "1.18"
'Const FechaVersion = "14/04/2014" 'JPB - CAS-21159 - Raffo - bugs E recruiting
'                                  '   Se modificó el proceso Importar2 para que guarde correctamente el campo de disp. horaria
               
'Const Version = "1.19"
'Const FechaVersion = "22/04/2014" 'JPB - CAS-21159 - Raffo - bugs E recruiting
'                                  'Ahora si encuentra mas de un tercero con el mismo mail reporta error y no continua.
                                               
'Const Version = "1.20"
'Const FechaVersion = "08/05/2014" 'JPB - Levanto el string de conexion de LE segun lo congfigurado en el confrep en el reporte 222
'                                        Se modifica la forma de comparar el string de conexion de LE y la actual.
                                  
                                               
Const Version = "1.21"
Const FechaVersion = "11/11/2014" 'JPB - CAS-21159 - Raffo - Adecuación Múltiple seleccion y lugar de nacimiento ER
'                                       Se modifica Importa2 para que guarde las Areas de Interes al postulante
...
If Usa_LE Then
    ...
    'JPB - Levanto el string de conexion segun lo congfigurado en el confrep en el reporte 222
    StrSql = " SELECT cnnro,cnstring FROM conexion  "
    StrSql = StrSql & " WHERE cnnro = (select confval from confrep where repnro = 222 and confnrocol = 1 and conftipo = 'BD') "
    ...
    'JPB - 08/05/2014 - Se le agrega Lcase en la comparación porque en los casos que la unica diferencia se el tamaño de la letra daba que eran diferentes (incorrectamente)
    If Usa_LE Then
        Misma_BD = True
        indice = 15 'DataSource Name
        If LCase(objConn.Properties(indice)) <> LCase(ConnLE.Properties(indice)) Then
            Misma_BD = False
        End If
...
' Autor      : Martin Ferraro
' Fecha      : 18/04/2009
' Ultima Mod.: 24/04/2014 - JPB - Se controla que no exista mas de un tercero con el mismo Email. En el caso que exista se informa y corta
' Descripcion:
' ---------------------------------------------------------------------------------------------
...
ListaTerceros = ""
    ...
    Else
     ...
     Registros = rs_tercero.GetRows()
     rs_tercero.MoveFirst
     
     CantRegistros = UBound(Registros, 2) + 1

     If CantRegistros > 1 Then
       For indice = 0 To (CantRegistros - 1)
           If ListaTerceros = "" Then
                 ListaTerceros = Registros(0, indice)
           Else
              ListaTerceros = ListaTerceros & "," & Registros(0, indice)
           End If
         
        Next indice

        Flog.writeline Espacios(Tabulador * 1) & "ERROR: El Email " & Mail & " se repite para los siguientes terceros: " & ListaTerceros
 
        BuscarTerceroXMail = -1
...
Public Function MapeoBooleano(ByVal Valor As Integer) As Integer
   Valor = Replace(Valor, "1", "-1")
   Valor = Replace(Valor, "2", "0")
   MapeoBooleano = Valor
End Function
...
Public Sub Asociar_Estructura_LE(ByVal TernroRHPro As Integer, ByVal TernroER As Integer)
' ---------------------------------------------------------------------------------------------
' Descripcion: Procedimiento encargado de pasar las areas de interes al postulante
' Autor      : JPB
' Fecha      : 11/11/2014
' Ultima Mod.:
' Descripcion:
' ---------------------------------------------------------------------------------------------
        ...
        Dim rs_Consult As New ADODB.Recordset
        StrSql = "DELETE interes_ter WHERE ternro=" & TernroRHPro
        ConnLE.Execute StrSql, , adExecuteNoRecords
        Flog.writeline Espacios(Tabulador * 1) & "Borra Areas de interes del tercero para asignarle nuevas"
        
        OpenRecordsetExt "EXEC SP_AreaInteres " & TernroER, rs_Consult, ExtConn
        
        Do While Not rs_Consult.EOF
           
           If CtrlNuloNUM(rs_Consult!intID) <> 0 Then
           
                StrSql = "INSERT INTO interes_ter "
                StrSql = StrSql & "( ternro,intID) "
                StrSql = StrSql & " VALUES "
                StrSql = StrSql & " ( " & TernroRHPro
                StrSql = StrSql & " , " & rs_Consult!intID
                StrSql = StrSql & " )"
                ConnLE.Execute StrSql, , adExecuteNoRecords
                
                Flog.writeline Espacios(Tabulador * 1) & "Area de interes " & rs_Consult!intID & " creada."
...
Public Sub Asociar_AreaInteres(ByVal TernroRHPro As Integer, ByVal TernroER As Integer)
' ---------------------------------------------------------------------------------------------
' Descripcion: Procedimiento encargado de pasar las areas de interes al postulante
' Autor      : JPB
' Fecha      : 11/11/2014
...
Dim TernroTemp As Long
Dim termovilidad As Integer
'Dim terdisponibilidad As Integer
Dim infnro As Integer
Dim terreubicapais As Integer
Dim terfamiliares As Integer
Dim teramigos As Integer
...
MyBeginTrans
    ...
    If TernroRHPro <= 0 Then
      ...
      Else
        Flog.writeline Espacios(Tabulador * 1) & "En Rhpro existe más de un tercero con el mismo Mail  " & Mail
      End If
    ...
    If Not rs_Consult.EOF Then
        ...
        'JPB: Convierto valor de SI/NO de Erecruiting a -1 y 0 de rhpro
        termovilidad = MapeoBooleano(CtrlNuloNUM(rs_Consult!termovilidad))
        'terdisponibilidad = MapeoBooleano(CtrlNuloNUM(rs_Consult!terdisponibilidad))
         
        terreubicapais = MapeoBooleano(CtrlNuloNUM(rs_Consult!terreubicapais))
        terfamiliares = MapeoBooleano(CtrlNuloNUM(rs_Consult!terfamiliares))
        teramigos = MapeoBooleano(CtrlNuloNUM(rs_Consult!teramigos))
        ...
        StrSql = StrSql & " ,posvehiculo = " & termovilidad
        
        StrSql = StrSql & " ,mednro = " & CtrlNuloNUM(rs_Consult!mednro)
         
        '-------------------NUEVOS RAFFO
           StrSql = StrSql & " ,termovilidad = " & termovilidad
           'StrSql = StrSql & " ,terdisponibilidad = " & terdisponibilidad
           StrSql = StrSql & " ,infnro = " & CtrlNuloNUM(rs_Consult!infnro)
           StrSql = StrSql & " ,terreubicapais = " & terreubicapais
           StrSql = StrSql & " ,posactlibre = " & CtrlNuloTXT(rs_Consult!posactlibre, 200)
           StrSql = StrSql & " ,terfamiliares = " & terfamiliares
           StrSql = StrSql & " ,posnomfamiliar = " & CtrlNuloTXT(rs_Consult!posnomfamiliar, 200)
           StrSql = StrSql & " ,teramigos = " & teramigos
           StrSql = StrSql & " ,posnomamigo = " & CtrlNuloTXT(rs_Consult!posnomamigo, 200)
           StrSql = StrSql & " ,intID = " & CtrlNuloNUM(rs_Consult!intID)
        '-------------------
        StrSql = StrSql & "  WHERE ternro = " & TernroRHPro
        objConn.Execute StrSql, , adExecuteNoRecords
        ...
        '---------------------------------------------------
        'JPB: Paso las areas de interes seleccionadas a la estructura 119 asociada al tercero
        'Call Asociar_AreaInteres(119, TernroRHPro, rs_Consult!Ternro, "Area de Interes")
        Call Asociar_AreaInteres(TernroRHPro, rs_Consult!Ternro)
        '---------------------------------------------------
        ...
        If Usa_LE Then
            If Not Misma_BD Then
                ...
                '---------------------------------------------------
                'JPB: Paso las areas de interes seleccionadas a la estructura 119 asociada al tercero
                Call Asociar_Estructura_LE(TernroRHPro, rs_Consult!Ternro)
                '---------------------------------------------------
