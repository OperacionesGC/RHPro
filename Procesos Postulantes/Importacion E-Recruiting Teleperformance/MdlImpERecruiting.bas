...
'Const Version = "1.08"
'Const FechaVersion = "11/02/2011" 'FGZ - Custom Teleperformance R2 (telefonos)
'                                  'EAM- Es un Custom solo para  Teleperformance
''-----------------------------------------------

'Const Version = "1.09"
'Const FechaVersion = "11/02/2011" 'EAM- Se hicieron correciones varias, para la replica de la BD temporal de LE.
'
''-----------------------------------------------
...
StrSql = "SELECT busnro, busdesabr, busdesext"
    ...
    If CantRHBusq <> 0 Then
        ...
        Do While Not rs_RHBusq.EOF
            ...
            If rs_Consult.EOF Then
                ...
                StrSql = StrSql & " ,busq_fec_alta,  busq_fec_fin,activa,destacada,modelo)"
                StrSql = StrSql & " VALUES("
...
Dim rs_terceroTemp As New ADODB.Recordset
    ...
    If (Usa_LE) Then
        ...
        Else
            BuscarTerceroTempXMail = IIf(Not EsNulo(rs_terceroTemp!terceroTemp), rs_terceroTemp!terceroTemp, 0)
        End If
...
MyBeginTrans
    ...
    If rs_Consult.EOF Then
        ...
        Else
            ...
            'EAM- Se comento la linea y la remplace por la de arriba que es el campo tertip (postulante)
            ...
            If rs_Consult.EOF Then
                ...
                CrearTercero = 0
                Flog.writeline Espacios(Tabulador * 2) & "ERROR! El documento tipo " & rs_Post!tidnro & " NRO " & rs_Post!terdoc & " del postulante a insertar " & rs_Post!terape & " " & rs_Post!ternom & " PERTENECE a otro tercero en RHPro (2)"
        ...
        Else
            ...
            Else
                ...
                Else
                    ...
                    'Error, el mail pertenece a un tercero no postulante
                    CrearTercero = 0
                    Flog.writeline Espacios(Tabulador * 2) & "ERROR! El documento de " & rs_Post!teremail & " a insertar " & rs_Post!terape & " " & rs_Post!ternom & " PERTENECE a otro tercero en RHPro (7)"
        ...
        Case 1:
            ...
            If Usa_LE Then
                ...
                Else
                    ...
                    'FGZ - Ojo que es otra conexion debo abrir otra transaccion
                    ...
                    If TernroTemp = -1 Then
                        TernroTemp = getLastIdentityLE("tercero")
                    End If
                    If TernroTemp = -1 Then
                        Flog.writeline Espacios(Tabulador * 2) & "No se pudo replica en la bd temporal "
                        MyRollbackTransLE
                        GoTo FinImp1
                    End If
...
MyBeginTrans
    ...
    If Not rs_Consult.EOF Then
        ...
        If Not rs_DomNro.EOF Then
            Flog.writeline Espacios(Tabulador * 1) & "El postulante ya tiene un Domicilio asociado"
        ...
        Else
            ...
            DomNro = 0
            Flog.writeline Espacios(Tabulador * 1) & "El postulante No tiene un Domicilio asociado"
        End If
        ...
        If DomNro = 0 Then
            ...
            StrSql = StrSql & " ," & CtrlNuloTXT(rs_Consult!calle, 30)
            ...
            If Not EsNulo(rs_Consult!telnro) Then
                'FGZ - 21/01/2011 - le saqué el tipo tel porque en r2 no existe -----
                ...
                StrSql = "INSERT INTO telefono"
                StrSql = StrSql & " (domnro,telnro,teldefault,telfax,telcelular)"
                StrSql = StrSql & " VALUES"
                StrSql = StrSql & " (" & DomNro
                StrSql = StrSql & " ," & CtrlNuloTXT(rs_Consult!telnro, 20)
                StrSql = StrSql & " ,-1,0,0)"
            ...
            'EAM - 04/01/2010
            If (Usa_LE) And (Not Misma_BD) Then
                'EAM- Abre una transacción para la Conexion del LE
                MyBeginTransLE
            
                'Alta de un domicilio nuevo - creo la cabecera del demicilio
                StrSql = "INSERT INTO cabdom(tipnro,ternro,domdefault,tidonro) " & _
                         " VALUES(1," & TernroTemp & ",-1," & CtrlNuloNUM(rs_Consult!tidonro) & ")"
                ConnLE.Execute StrSql, , adExecuteNoRecords
            
                'Obtengo el numero de domicilio en la tabla
                DomNro = CLng(getLastIdentity(ConnLE, "cabdom"))
                If DomNro = -1 Then
                    DomNro = getLastIdentityLE("tercero")
                End If
                If DomNro = -1 Then
                    Flog.writeline Espacios(Tabulador * 2) & "No se pudo replica en la bd temporal "
                    MyRollbackTransLE
                ...
                If Not EsNulo(rs_Consult!telnro) Then
                    ...
                    StrSql = StrSql & " ,-1,0,0)"
                    ConnLE.Execute StrSql, , adExecuteNoRecords
                    Flog.writeline Espacios(Tabulador * 1) & "Inserto el Telefono en la BD LE"
                End If
                
                'EAM- Cierra la transaccion de la Conexión del LE
                MyCommitTransLE
            End If
        ...
        Else
            ...
            StrSql = StrSql & "  calle = " & CtrlNuloTXT(rs_Consult!calle, 30)
            ...
            StrSql = "DELETE FROM telefono WHERE teldefault = -1 AND domnro = " & DomNro
            ConnLE.Execute StrSql, , adExecuteNoRecords
...
MyBeginTrans
    ...
    'EAM- Comento estas líneas porque en LE no se guardan los estudios formales en la tabla de cap_estformal
...
MyBeginTrans
    ...
    'EAM- Comento estas líneas porque en LE no se guardan los estudios formales en la tabla de cap_estinformal
...
MyBeginTrans
    ...
    'EAM- Comento estas líneas porque en LE no se guardan los conocimientos.
    'EAM - Verifica si tiene instalado LE
'    If (Usa_LE) And (Not Misma_BD) Then
'        'Borro todo lo que existe en LE
...
MyBeginTrans
    ...
    'EAM- Comento estas líneas porque en LE no se guardan esos datos
'    'EAM - Verifica si tiene instalado LE
'    If (Usa_LE) And (Not Misma_BD) Then
'        'Borro todo lo que existe en LE
