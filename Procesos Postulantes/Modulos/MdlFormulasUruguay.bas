...
If Not Anual Then
    ...
    Do While Not rs_Ficharet.EOF
        If (Month(rs_Ficharet!Fecha) <= Ret_Mes) And (Year(rs_Ficharet!Fecha) = Ret_Ano) Then
            Otras_Ret = Otras_Ret + rs_Ficharet!importe
        End If
    ...
    'Resultado del impuesto
    Impuesto = Renta - Deduccion - Otras_Ret
    Call InsertarFichaRet(buliq_empleado!ternro, buliq_proceso!profecpago, Impuesto, buliq_proceso!pronro)
Else
