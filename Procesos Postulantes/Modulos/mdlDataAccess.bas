...
Global TransactionLERunning As Boolean
Global T2Running As Boolean
Global Reg_Afected
Global SCHEMA As String

' LAMZ - 17/05/2011 - Se agrego la declaracion de la coneccion
Global ConnLE As New ADODB.Connection ' no borrar por duplicada, borrar de los .bas
...
Public Function getLastIdentityLE(ByVal NombreTabla As String) As Variant
Dim objRs As New ADODB.Recordset
    Dim StrSql As String
    Select Case TipoBD
    Case 1 'db2
    ...
    Case 4 ' Oracle 9
        StrSql = "select SEQ_" & UCase(NombreTabla) & ".CURRVAL as Codigo FROM DUAL"
    End Select
    OpenRecordsetLE StrSql, objRs
    If Not (objRs.EOF And objRs.BOF) Then
        If Not EsNulo(objRs.Fields("codigo").Value) Then
            getLastIdentityLE = objRs.Fields("codigo").Value
        Else
            getLastIdentityLE = -1
        End If
    Else
...
Public Sub OpenRecordsetLE(strSQLQuery As String, ByRef objRs As ADODB.Recordset, Optional lockType As LockTypeEnum = adLockReadOnly)
Dim pos1 As Long
Dim pos2 As Long
Dim aux As String
    ...
    objRs.CacheSize = 500
    objRs.Open strSQLQuery, ConnLE, adOpenDynamic, lockType, adCmdText
    Cantidad_de_OpenRecordset = Cantidad_de_OpenRecordset + 1
End Sub
...
Public Sub MyBeginTransLE()
    If Not TransactionLERunning Then
        ConnLE.BeginTrans
        TransactionLERunning = True
    End If
End Sub
Public Sub MyCommitTransLE()
    If TransactionLERunning Then
        ConnLE.CommitTrans
        TransactionLERunning = False
    End If
End Sub
Public Sub MyRollbackTransLE()
    If TransactionLERunning Then
        ConnLE.RollbackTrans
        TransactionLERunning = False
    End If
End Sub
