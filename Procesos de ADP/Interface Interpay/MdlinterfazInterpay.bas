Attribute VB_Name = "MdlinterfazInterpay"
Option Explicit

'Const Version = "1.00"
'Const FechaVersion = "21/11/2013" ' CAS-21976 - BDO - GE - Interface Interpay - Deluchi Ezequiel - Version Inicial

'Const Version = "1.01"
'Const FechaVersion = "29/04/2015" ' LED - CAS-29842 - GE - Interpay Nueva Versión
                                  'Cambio en el nombre del proceso y cambio en los campos de los todos los movimientos

'Const Version = "1.02"
'Const FechaVersion = "26/06/2015" ' LED - CAS-29842 - GE - Interpay Nueva Versión [Entrega 2]
                                  'Movimiento SYNCH cambio en el formato de fecha ddmmyyyy, Montos 2 decimales,
                                  'campo position title descripcion del puesto, solo se exporta empleados activos (empest=-1)
                                  'Cambios en el movimiento Synch

'Const Version = "1.03"
'Const FechaVersion = "30/06/2015" ' LED - CAS-29842 - GE - Interpay Nueva Versión [Entrega 3]
                                  'Cambios en el movimiento Synch - Se invierten los datos que se muestran en la columna 12 y 13.

'Const Version = "1.04"
'Const FechaVersion = "16/07/2015" ' LED - CAS-29842 - GE - Interpay Nueva Versión [Entrega 4]
                                  ' Cambios en el movimiento ADDR - Se muestra en la columna 26 provincia y en la 27 localidad.
                                  ' Cambios en el movimiento Synch - Se informa en la columna 21 provincia y en la 22 localidad.
                                  ' Se agrego filtro de empleados por empresas configuradas en el confrep columna 21 (puede ser mas  de 1)

Const Version = "1.05"
Const FechaVersion = "31/08/2015" ' LED - CAS-29842 - GE - Interpay Nueva Versión [Corrección]
                                  ' se corrigio comando de ejecucion en la funcion cerrarFase()
...
Dim ArrParametros
    ...
    Nombre_Arch = PathFLog & "InterfaceInterpay_" & NroProcesoBatch & ".log"
...
'--------------
    ...
    'busco las empresas configuradas
    StrSql = " SELECT confnrocol, confval, confval2 FROM confrep WHERE repnro = 480 AND confnrocol in (21)"
    ...
    If Not rsConfrep.EOF Then
        ...
        Do While Not rsConfrep.EOF
            Select Case CLng(rsConfrep!confnrocol)
                ...
                Case 21: 'Codigo de estructura de Empresa
                    listaEmpresa = listaEmpresa & "," & rsConfrep!confval2 & ""
        ...
        Loop
    Else
        Flog.writeline Espacios(Tabulador * 1) & "No hay empresa configuradas, se buscaran todos los empleados"
    End If
    
    If CStr(listaEmpresa) = "0" Then
        StrSql = " SELECT empleado.ternro FROM empleado WHERE empest = -1 "
    Else
        StrSql = " SELECT empleado.ternro FROM empleado " & _
                 " INNER JOIN his_estructura ON his_estructura.ternro = empleado.ternro AND his_estructura.tenro = 10 AND his_estructura.estrnro in (" & listaEmpresa & ") " & _
                 " WHERE empest = -1 AND (his_estructura.htetdesde <= " & cambiaFecha(Date) & " AND (his_estructura.htethasta >= " & cambiaFecha(Date) & " OR his_estructura.htethasta is null)) "
    End If
    'StrSql = " SELECT * FROM empleado WHERE empleg in (1,2,3) "
...
Dim nroLinea As Long
   ...
   'Abro el archivo
    ...
    objStream.Open
    objStream.LoadFromFile (nombreArchivo)
    
    str_error = ""
    nroLinea = 0
    
    Do Until objStream.EOS
        strLinea = objStream.ReadText(-2)
        nroLinea = nroLinea + 1
