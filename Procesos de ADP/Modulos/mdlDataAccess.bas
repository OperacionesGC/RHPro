...
Global TransactionRunningExt As Boolean
...
Public Sub OpenRecordsetRH(strSQLQuery As String, ByRef objRs As ADODB.Recordset, Optional lockType As LockTypeEnum = adLockReadOnly)
    ...
    objRs.Open strSQLQuery, objConn, adOpenDynamic, lockType, adCmdText
End Sub
Public Sub MyBeginTransExt(ByVal objConnE As ADODB.Connection)
    If Not TransactionRunningExt Then
        objConnE.BeginTrans
        TransactionRunningExt = True
    End If
End Sub
Public Sub MyCommitTransExt(ByVal objConnE As ADODB.Connection)
    If TransactionRunningExt Then
        objConnE.CommitTrans
        TransactionRunningExt = False
    End If
End Sub
...
Public Sub CreaVistaEmpleadoProceso(ByVal NombreTabla As String, ByVal usuario As String)
 ...
 On Error GoTo CE
    ...
    Flog.writeline Espacios(Tabulador * 0) & "Vista Utilizada: " & Sql_VistaEmpleado
