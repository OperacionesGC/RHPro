...
Const Version = 1.03
Const FechaVersion = "01/04/2015"
'Modificaciones: LED - Ruiz Miriam - Monresa - Sanciones-CAS-26972 - RH Pro (Producto) - Tipo de estructura fijo en Sanciones
    ' si el criterio tiene mas de una condición descuenta solo cuando las dos se cumplen
    ' se corrigió la cantidad acumulada
    ' se cambió el calculo de la fecha desde para que tenga en cuenta la fecha desde del criterio
    ' ahora evalua el orden despues de insertar todas las sanciones
    ' en el criterio por acumulación cuando llega al máximo solo inserta una sanción
    ' regenera todos los ordenes a partir de la menor fecha desde procesada
'---------------------------------------------------------------------------
'---------------------------------------------------------------------------
...
Dim rsorden2 As New ADODB.Recordset
...
Dim arrParametros
Dim gradorden
Dim FechaDAux   ' guarda la fecha menor del período evaluado
    ...
    Flog.writeline "Fecha Ultima Modificacion(No liberada al 10/03/2015): " & FechaVersion
        ...
        Do While Not rsEmpCausal.EOF
            ...
            Do While Not rsCriterio.EOF
                ...
                Do While Not rsCondicion.EOF
                    ...
                    Call buscarFechaPeriodo(rsCondicion!perievnro, FechaProc, FechaDesde, FechaHasta, rsEmpCausal!ternro, rsCriterio!critdesde)
                    ...
                    Else
                        ...
                        If FechaDAux > FechaDesde Then
                             FechaDAux = FechaDesde
                        End If
...
'            Else
            ...
            Loop
            ' MR desde acá
                'StrSql = "SELECT * FROM dis_his_sancion WHERE gradorden=999 and ternro = " & rsEmpCausal("ternro")
                
                StrSql = "SELECT top 1 gradorden FROM dis_his_sancion WHERE hsandesde < " & ConvFecha(FechaDAux)
                StrSql = StrSql & " AND causnro = " & rsCausal("causnro")
                StrSql = StrSql & " AND ternro = " & rsEmpCausal("ternro")
                StrSql = StrSql & " ORDER BY hsanfecinfra DESC "
                OpenRecordset StrSql, rsorden
                If Not rsorden.EOF Then
                   gradorden = rsorden!gradorden
                Else
                   gradorden = 0
                End If
                rsorden.Close
                ...
                StrSql = "SELECT * FROM dis_his_sancion WHERE hsandesde >= " & ConvFecha(FechaDAux)
                StrSql = StrSql & " AND causnro = " & rsCausal("causnro")
                StrSql = StrSql & " AND ternro = " & rsEmpCausal("ternro")
                StrSql = StrSql & " ORDER BY hsanfecinfra "
                OpenRecordset StrSql, rsorden
                
                If Not rsorden.EOF Then
                   
                If gradorden > 0 Then
                        StrSql = "UPDATE dis_grad_san_emp SET gradorden= " & gradorden & _
                                 " WHERE ternro= " & rsorden!ternro & " AND causnro= " & rsorden!causnro
                        objConn.Execute StrSql, , adExecuteNoRecords
                        Flog.writeline Espacios(Tabulador * 1) & "Se restauro despues de la eliminación al grado: " & gradorden
                 Else
                        StrSql = "DELETE FROM dis_grad_san_emp " & _
                                 " WHERE ternro= " & rsorden!ternro & " AND causnro= " & rsorden!causnro
                        objConn.Execute StrSql, , adExecuteNoRecords
                
                End If

                        ' Call ReducirGraduacionSancion(rsorden!causnro, rsorden!condnro, rsorden!ternro, rsorden!hsanfecinfra)
                 
                    Do While Not rsorden.EOF
                        ' regenera el orden desde la menor fecha desde
                              Call ReducirGraduacionSancion(rsorden!causnro, rsorden!condnro, rsorden!ternro, rsorden!hsanfecinfra)
                            ...
                            " WHERE dis_condicion.condnro = " & rsorden!condnro
                            OpenRecordset StrSql, rsdatos
                          gradorden = ActualizarGradoSancion(rsorden!ternro, rsorden!causnro, rsdatos!gradnro, 1, 0)
                          
                          If gradorden = "" Then
                            gradorden = 0
                          End If
                          StrSql = "UPDATE dis_his_sancion SET gradorden= " & gradorden & _
                          " WHERE hsannro= " & rsorden!hsannro
                          objConn.Execute StrSql, , adExecuteNoRecords
...
Private Sub buscarFechaPeriodo(ByVal perievnro, ByVal fdesdeProc As Date, ByRef fdesde As Date, ByRef fhasta As Date, ByVal ternro As Long, ByVal critdesde As Date)
            ...
            Case Is = -1: 'Zafra
                    ...
                    If Not rsPeriEv.EOF Then
                        If Not IsNull(rsPeriEv!bajfec) Then
                            If critdesde > rsPeriEv!altfec Then
                                fdesde = critdesde
                            Else
...
Private Sub GenerarSancion(ByVal Hora As String, ByVal condnro As Long, ByVal fdesde As Date, ByVal fhasta As Date, _
 ...
 Dim fecInfraccSancionAUX
    ...
    Do While Not rsAD.EOF
        ...
        If rsdatos!cantmax > 0 Then
            ...
            If (cantInfraccion > rsdatos!cantmax) And (ConteplarInfraccion) Then
                ...
                If (Not aplicoReduccion) Then
                   'MR Call ReducirGraduacionSancion(rsDatos!causnro, rsDatos!condorigennro, ternro, rsAD!adfecha)
              ...
              'MR  gradorden = ActualizarGradoSancion(ternro, rsDatos!causnro, rsDatos!gradnro, 1, 0)
               ...
               'MR StrSql = "INSERT INTO dis_his_sancion (ternro,hsananio,fasnro,causnro,condnro,hsandesde,hsanhasta,estsancodigo " & _
                ...
                StrSql = "INSERT INTO dis_his_sancion (ternro,hsananio,fasnro,causnro,condnro,hsandesde,hsanhasta,estsancodigo " & _
                        ",hsanfecinfra,hsanfechagen,gradorden,hsanhora,hsantipocond )" & _
                        " VALUES (" & ternro & "," & Year(fdesde) & ",''," & rsdatos!causnro & "," & condnro & "," & ConvFecha(fdesde) & _
                        "," & ConvFecha(fhasta) & "," & codEstado & "," & ConvFecha(rsAD!adfecha) & "," & ConvFecha(Now) & ",999," & Hora & ",1)"
        ...
        If rsdatos!valmax > 0 And (ConteplarInfraccion) Then
            ...
            If ((cantAcumulada * 60) > rsdatos!valmax) Then
                ...
                'For i = 1 To cantSanciones
                    'EAM- Actualiza el grado de sancion para generar la nueva sancion
                   'MR gradorden = ActualizarGradoSancion(ternro, rsDatos!causnro, rsDatos!gradnro, 2, 0)
                   ...
                   'MR StrSql = "INSERT INTO dis_his_sancion (ternro,hsananio,fasnro,causnro,condnro,hsandesde,hsanhasta,estsancodigo " & _
                    ...
                    StrSql = "INSERT INTO dis_his_sancion (ternro,hsananio,fasnro,causnro,condnro,hsandesde,hsanhasta,estsancodigo " & _
                            ",hsanfecinfra,hsanfechagen,gradorden,hsanhora,hsantipocond )" & _
                            " VALUES (" & ternro & "," & Year(fdesde) & ",''," & rsdatos!causnro & "," & condnro & "," & ConvFecha(fdesde) & _
                            "," & ConvFecha(fhasta) & "," & codEstado & "," & ConvFecha(rsAD!adfecha) & "," & ConvFecha(Now) & ",999," & Hora & ",2)"
...
Private Sub ReducirGraduacionSancion(ByVal causnro As Long, ByVal condorigennro As Long, ByVal ternro As Long, ByVal fechasta As Date)
 ...
 Dim causreducniv As Long
  ...
  StrSql = "SELECT hsanfecinfra FROM  dis_his_sancion WHERE causnro=" & causnro & "  AND ternro= " & ternro & "  AND hsanfecinfra < " & ConvFecha(fechasta) & " ORDER BY hsanfecinfra DESC"
    OpenRecordset StrSql, rsAux
    If Not rsAux.EOF Then
