...
''                                       ----------------------------------------------------------------------------
''                                       Se cambió el sub PuedeEjecutar.
''                                             controla por procesos en estado iniciando
''                                       Se cambió el sub EliminarProcesosNoInician
''                                           Aborta procesos que no inician

'Const Version = "2.18"
'Const FechaVersion = "03/01/2012"   'FGZ
''                                       ----------------------------------------------------------------------------
''                                       Se le sacaron los mensajes de log cuando todavia no es la hora de ejecucion (así no crece tanto el archivo de log).
...
''                                       ----------------------------------------------------------------------------
''                                       Se recompiló el proceso para que utilice MDAC 2.8.

'Const Version = "3.01"
'Const FechaVersion = "08/08/2012"   'FGZ
''                                       ----------------------------------------------------------------------------
''                                       Se modificó el control de incompatibilidades para los procesos de Simulacion
...
''                                       ----------------------------------------------------------------------------
''                                       Se modificó el formato de fecha interno (sin Hora) para el control de procesos incompatibles
'
''                                       Se redefinió el OpenRecordset por los problemas de corte de conexion. Ahora en este proceso se usa OpenrecordsetAS
'
''                                       Se agregó funcionalidad de Depuracion de carpeta de logs. Los parametros salen
''                                       FGZ - se agregaron 2 parametros en defaults.ini
''                                       ----------------------------------------------------------------------------
''                                       Esta modificacion requiere agregar un parametros en RHProappSrvDefaults.ini
''                                       ----------------------------------------------------------------------------
...
''                                       ----------------------------------------------------------------------------
''                                       Se agregó funcionalidad de verificacion y notificacion de Alertas activas y no planificadas
''#Const Version = "3.0.4"
...
'                                   Miriam Ruiz- CAS-29182 - BDO Base Kraft CHILE - BUG EN PROCESAR- se libera la versión
...
Const Version = "3.10"
Const FechaVersion = "14/07/2015" ' Miriam Ruiz - CAS-31836 - Rhpro - Inconvenientes con opcion borrado de appsrv - Se arregló el borrado de logs
            ...
            Do While (Actual <= Ultimo) And seguir
                If PuedeEjecutar(Pendientes(Actual).Proceso, Pendientes(Actual).TipoProceso) Then
                    If Format(Pendientes(Actual).Fecha, FormatoInternoFecha) = Format(Date, FormatoInternoFecha) Then
                        ...
                        Else
                            'FGZ - 03/01/2012
                            ' le saqué los mensajes de log para que no crezca tanto el archivo de log
...
Dim Aplica As Boolean
    ...
    Flog.writeline " Depuración de logs ... "
    If Accion = 0 Then
    ...
    Do While Not rsEj.EOF
        ...
        If Ejecutando(MiIndice).Progreso = rsEj!bprcprogreso Then
           ...
           If DateDiff("n", Format(Ejecutando(MiIndice).HoraFinEj, FormatoInternoHora), Format(Time, FormatoInternoHora)) > TiempoDeEsperaSinProgreso Then
                Flog.writeline "No avanzó el progreso en " & TiempoDeEsperaSinProgreso & " minutos. Pone Proceso " & rsEj!bpronro & " en estado NO RESPONDE - " & Format(Now, FormatoInternoHora)
        ...
        Else
            ...
            If IsNull(rsEj!bprcprogreso) Then
                ...
                If hProc = 0 Then
                    ...
                    Flog.writeline "Proceso " & rsEj!bpronro & " se aborta porque No está en memoria. Estado --> ERROR " & Format(Now, FormatoInternoHora)
                Else
...
Do While i <= UBound(Pendientes)
    ...
    'EAM(v3.08) - Se comento la inicialización del arreglo ejecuntando porque no se debe setear ya que da error de abortado por el usuario
'    Ejecutando(i).pid = 0
'    Ejecutando(i).Proceso = 0
'    Ejecutando(i).Progreso = 0
...
Continuar:
    ...
    If CLng(Err.Number) <> 0 Then
        ...
        If ((CLng(Err.Number) = 2147467259) Or (CLng(Err.Number) = -2147467259)) Or (CLng(Err.Number) = 3709) Then
            ...
            Do While Err.Number <> 0
                Err.Clear
                Flog.writeline "       Reintentando conexión en " & TiempodeDormida & " segundos."
                Sleep (TiempodeDormida * 1000)
                Flog.writeline
                Flog.writeline "Intentando establecer conexión nuevamente ..."
            ...
            If NotificaError Then
                ...
                If objConn.State = adStateOpen Then
                    ...
                    Mensaje = " Se reestableció la conexión del servicio"
                    Call NotificarError(Asunto, Mensaje)
...
Dim HayAlertas As Boolean
    ...
    If NotificaError And HayAlertas Then
        Flog.writeline "Notificacion de Planificación de alertas" & Format(Now, FormatoInternoFecha)
    
        Asunto = "RHPRO - Warning de Planificación"
