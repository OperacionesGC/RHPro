...
Public Sub ConfReg_Usr(ByVal USER As String)
Dim Ret As String
Dim buffer As String
    
    buffer = String$(256, 0)
    'Configuración del número
    Ret = GetLocaleInfo(USER, LOCALE_SDECIMAL, buffer, Len(buffer))
    If Ret > 0 Then
        NumeroSeparadorDecimal = Left$(buffer, Ret - 1)
    End If
    Ret = GetLocaleInfo(USER, LOCALE_STHOUSAND, buffer, Len(buffer))
    If Ret > 0 Then
        NumeroSeparadorMiles = Left$(buffer, Ret - 1)
    End If
    'Configuración de la moneda
    Ret = GetLocaleInfo(USER, LOCALE_SMONDECIMALSEP, buffer, Len(buffer))
    If Ret > 0 Then
        MonedaSeparadorDecimal = Left$(buffer, Ret - 1)
    End If
    
    Ret = GetLocaleInfo(USER, LOCALE_SMONTHOUSANDSEP, buffer, Len(buffer))
    If Ret > 0 Then
        MonedaSeparadorMiles = Left$(buffer, Ret - 1)
    End If
    
    Ret = GetLocaleInfo(USER, LOCALE_SSHORTDATE, buffer, Len(buffer))
    If Ret > 0 Then
        FormatoDeFechaCorto = Left$(buffer, Ret - 1)
    End If
End Sub
