...
Global NroVacAux As Long    'EAM- (v3.28)
...
Global TipDiaPago2 As Integer
Global TipDiaDescuento2 As Integer
...
Global FeriadoConControldeAnormalidades  As Boolean
Global FP_Feriado As Long
...
Global Arr_Oblig(1 To 6) As TRegOblig
'FGZ - 01/08/2014 -----------------------
...
Global listEmpLic As String    'EAM- (v3.28)
...
Case 1: 'Horario cumplido
    ...
    If Version >= "5.48" Then
        ''/*Nueva Tabla*/
        'CREATE TABLE [dbo].[gti_desgloce_hc](
        '    [deshcnro] [int] IDENTITY(1,1) NOT NULL,
        ...
        'GO
        Texto = ""
        Texto = Texto & " gti_desgloce_hc"
        StrSql = "Select deshcnro FROM gti_desgloce_hc WHERE deshcnro = 1"
        OpenRecordset StrSql, rs
        '/*-----------------------------*/
...
Case 2: 'Acumulado Diarios
    ...
    'LED - 14/05/2011 - Relevos de Estructuras
    If Version >= "5.35" Then
        'Relevos de Estructuras (Politica 589 V2)
        'CREATE TABLE [dbo].[gti_relevo_estruc](
        '    [relestnro] [int] IDENTITY(1,1) NOT NULL,       --Codigo de la distribucion de horas
        '    [gcpnro] [int] NOT NULL,                    --Codigo de la cabecera del parte
        '    [relestfecdesde] [datetime] NULL,               --Fecha Desde
        '    [relestfechasta] [datetime] NULL,               --Fecha Hasta
        '    [ternro] [int] NOT NULL,                    --Numero de tercero del empleado
        '    [tenro] [int] NOT NULL,                     --Codigo del tipo de estructura
        '    [estrnro] [int] NOT NULL                    --Codigo de la estructura
        ') ON [PRIMARY]
        'GO
        
        Texto = ""
        Texto = Texto & " gti_relevo_estruc "
        StrSql = "Select relestnro FROM gti_relevo_estruc WHERE relestnro = 1"
        OpenRecordset StrSql, rs
        
        'CREATE TABLE [dbo].[gti_relevo_estruc_det](
        '    [relestdetnro] [int] IDENTITY(1,1) NOT NULL,        --Codigo del detalle
        '    [relestnro] [int] NOT NULL,                         --Codigo de la cabecera
        '    [fecha] [datetime] NULL,                            --fecha del detalle
        '    [estrnro] [int] NOT NULL                            --Codigo de la estructura
        ') ON [PRIMARY]
        'GO

        Texto = ""
        Texto = Texto & " gti_relevo_estruc_det "
        StrSql = "Select relestdetnro FROM gti_relevo_estruc_det WHERE relestdetnro = 1"
        OpenRecordset StrSql, rs

        'CREATE TABLE [dbo].[gti_relevo_estruc_det_hs](
        '    [relestdethsnro] [int] IDENTITY(1,1) NOT NULL,      --Codigo del detalle de hs
        '    [relestdetnro] [int] NOT NULL,                      --Codigo del detalle
        '    [thnro] [int] NOT NULL,                                 --Tipo de hora
        '    [canths] [decimal](5, 2) NULL,                          --Cantidad de Horas
        '    [estrnro] [int] NOT NULL                            --Codigo de la estructura
        ') ON [PRIMARY]
        'GO
        Texto = ""
        Texto = Texto & " gti_relevo_estruc_det_hs "
        StrSql = "Select relestdethsnro FROM gti_relevo_estruc_det_hs WHERE relestdethsnro = 1"
        OpenRecordset StrSql, rs
        
        V = True
    End If
    'LED - 01/08/2013 - CAS-11450 - H&A - Adecuación Conversiones (nivelacion R3 - R4)
    If Version >= "5.36" Then

        '/*AGREGADOS DE COLUMNA*/
        'ALTER TABLE [dbo].[gti_config_horadia] ADD [hd_alcance] [smallint] NULL
        'ALTER TABLE [dbo].[gti_config_horadia] ADD [ws_diassem] [varchar](30) NULL
        Texto = ""
        Texto = Texto & " hd_alcance, ws_diassem "
        StrSql = "Select hd_alcance, ws_diassem FROM gti_config_horadia WHERE hd_nro = 1"
        OpenRecordset StrSql, rs
        '/*-----------------------------*/


        '/*TABLA DE EMPLEADOS ASOCIADOS A UNA CONFIGURACION DE HS.*/
        'CREATE TABLE [dbo].[gti_config_horadia_empleados](
        '[hd_nro] [int] NOT NULL,
        '    [ternro] [int] NOT NULL,
        ') ON [PRIMARY]
        '/*-----------------------------*/
        Texto = ""
        Texto = Texto & " gti_config_horadia_empleados "
        StrSql = "Select hd_nro FROM gti_config_horadia_empleados WHERE hd_nro = 1"
        OpenRecordset StrSql, rs

        '/*TABLA DE ESTRUCTURAS ASOCIADOS A UNA CONFIGURACION DE HS.*/
        'CREATE TABLE [dbo].[gti_config_horadia_estr](
        '    [hd_nro] [int] NOT NULL,
        '    [tenro] [int] NOT NULL,
        '    [estrnro] [int] NOT NULL,
        ') ON [PRIMARY]
        ...
        Texto = ""
        Texto = Texto & " gti_config_horadia_empleados "
        StrSql = "Select hd_nro FROM gti_config_horadia_estr WHERE hd_nro = 1"
        OpenRecordset StrSql, rs


        '/*LISTADO DE PROGRAMA - CONVERSION DE HORAS */
        'CREATE TABLE [dbo].[gti_conversion_prog](
        '    [conv_nro] [int] IDENTITY(1,1) NOT NULL,
        '    [conv_prog] [int] NOT NULL,
        '    [conv_nombre] [VARCHAR](50) NOT NULL,
        '    [conv_desc] [VARCHAR](200) NULL,
        '    [conv_doc] [VARCHAR](50) NULL,
        '    [conv_custom] [VARCHAR](100) NULL,
        '    [conv_visible][smallint] NOT NULL,
        ') ON [PRIMARY]
        Texto = ""
        Texto = Texto & " gti_conversion_prog "
        StrSql = "Select conv_nro FROM gti_conversion_prog WHERE conv_nro = 1"
        OpenRecordset StrSql, rs
