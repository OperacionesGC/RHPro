Attribute VB_Name = "mdlConversionesv3"
Option Explicit
...
'EAM- Crea el tipo para suplantar la tabla temporal
Public Type THsAD
    thnro As Long
    Horas As String
    Cant_hs As Single
...
Public Sub AD_01(ByVal hd_nro As Long, A_HorNro As Long, thdestino As Long, Horas As Single, ByRef Hora_Dest As Single)
'------------------------------------------------------------------------------
...
StrSql = "SELECT hd_programa,hd_minimo,hd_maximo,hd_excede,hd_fijo,hd_pje " & _
        " FROM gti_config_horadia " & _
        " WHERE turnro= " & Nro_Turno & " AND hd_thorigen= " & A_HorNro & " AND   hd_thdestino = " & thdestino
'FGZ - 15/12/2008 - le agregué estos controles extras
StrSql = StrSql & " AND ( hd_programa is null OR hd_programa = '' OR hd_programa = 0)"
'FGZ - 04/09/2014 -----------------
StrSql = StrSql & " AND hd_nro = " & hd_nro
'FGZ - 04/09/2014 -----------------
...
Do While Not objRsCFG.EOF
    ...
    If objRsCFG!hd_programa <> "" And objRsCFG!hd_programa <> 0 Then
        If depurar Then
    ...
    If objRsCFG!hd_fijo >= 0 Then  'aca antes era: objRsCFG!hd_fijo > 0
        Hora_Dest = objRsCFG!hd_fijo
    Else
        Hora_Dest = Horas * objRsCFG!hd_pje / 100
    End If
...
Public Sub AD_Antes_Autorizar(ByVal traza As Boolean, ByVal P_NroTer As Long, ByVal P_NroTur As Long, ByVal P_NroGru As Long, ByVal P_HorNro As Long, ByVal Fecha_Inicio As Date, ByRef arrTempHsAD() As THsAD)
'------------------------------------------------------------------------------------------
'Descripcion:   Verifica si hay que convertir Horas o No. Si convierte devuelve un array con los resultados de las Hs convertidas.
'Autor:         EAM
'Fecha:         22/02/2012
'Ult. Modif:
'------------------------------------------------------------------------------------------
...
Dim acumula As Boolean  'Indica si va acumulando horas o si va reg. a reg.
                        ...
                        'No servia para nada
Dim auxPosHsADTemp, i As Long
Dim Horas_Acum_HS As String
Dim THOrigen As Long
                  
If depurar Then
    Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "AD_Antes_Autorizar - Conversion Antes de Autorizar"
End If
...
If Not objrsHC.EOF Then
    ...
    'EAM- Busca si el Tipo de Hora tiene alguna configuracion de conversion de Hs
    If depurar Then
        Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Busca si el Tipo de Hora tiene alguna configuracion"
    End If

    'EAM- 12/10/2012 - Se modifico la sql para r3
    'Versión r4 -->StrSql = "SELECT hd_nro, hd_thorigen, hd_thdestino,hd_feriados,hd_nolaborables,hd_laborable,hd_franja,hd_alcance,ws_diassem FROM gti_config_horadia "
    'Versión r3 -->StrSql = "SELECT hd_nro, hd_thorigen, hd_thdestino,hd_feriados,hd_nolaborables,hd_laborable,hd_franja FROM gti_config_horadia "
    StrSql = "SELECT hd_nro, hd_thorigen, hd_thdestino,hd_feriados,hd_nolaborables,hd_laborable,hd_franja,hd_alcance,ws_diassem FROM gti_config_horadia " & _
            " WHERE hd_thorigen = " & objrsHC!thnro & " AND  turnro = " & P_NroTur & " AND hd_ordenproc = 1 AND ( hd_programa is null OR hd_programa = '' OR hd_programa = 0) ORDER BY hd_nro"
    OpenRecordset StrSql, objRsConfig
            
    'EAM- Verifica si tiene configurado alguna conversion de Hs
    If objRsConfig.EOF Then
            If depurar Then
            Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "La Hora Nro: " & objrsHC!thnro & " No tiene configurado ninguna conversión de Hora"
        End If
    Else
    
        THOrigen = objRsConfig!hd_thorigen
        ...
        Do While Not objRsConfig.EOF
            thdestino = objRsConfig!hd_thdestino
            ...
            Else
                ...
                Else
                    ...
                    If Not Dia_Libre And Not (esFeriado And objRsConfig!hd_feriados) And Not objRsConfig!hd_laborable Then
                        GoTo Siguiente
                    Else
                        
                        'EAM- 12/10/2012 - esto se comento porque se aplica para R4. En r3 no se analiza que dia aplica
                        'EAM- Verifica si aplica para ese el dia que se esta procesando o no
                       'LED - 31/07/2013 - Se descomento para r4
                       If Not AplicaConversionEnDia(p_fecha, objRsConfig!ws_diassem, objRsConfig!hd_alcance, P_NroTer, objRsConfig!hd_nro) Then
                            If depurar Then
                                Flog.writeline Espacios(Tabulador * (Nivel_Tab_Log + 1)) & "Conversion del Tipo de Hora " & objRsConfig!hd_thorigen & " No aplica para el día " & p_fecha
                            End If
                            GoTo Siguiente
                        End If
                        
                        If depurar Then
                            Flog.writeline Espacios(Tabulador * (Nivel_Tab_Log + 1)) & "Conversion del Tipo de Hora " & objRsConfig!hd_thorigen & " al Tipo de Hora " & objRsConfig!hd_thdestino
                        End If
                        'LED - 31/07/2013 - Se descomento para r4
                        
                        acumula = Not objRsConfig!hd_franja
                        If acumula Then
                            'EAM- Si Acumula, suma todas las horas que tienen el mismo Tipo de hora destino y las gurada en -1. al final de esta func. Le aplica la conversion
                            '       y setea el registro en 0 y por cada hora va actualizando la conversión.
                            
                            'EAM- Busca el tipo de hora configurada a convertir
                            auxPosHsADTemp = BuscarHsADTemp(arrTempHsAD, objRsConfig!hd_thdestino, -1)
                        
                            'EAM- Si encuentra el tipo de hora lo va sumando y acumulando en el registro del tipo de hora
                            If (auxPosHsADTemp = 0) Then
                                ReDim Preserve arrTempHsAD(UBound(arrTempHsAD) + 1)
                                arrTempHsAD(UBound(arrTempHsAD)).thnro = objRsConfig!hd_thdestino
                                arrTempHsAD(UBound(arrTempHsAD)).Horas = objrsHC!Horas
                                arrTempHsAD(UBound(arrTempHsAD)).Cant_hs = objrsHC!horcant
                                arrTempHsAD(UBound(arrTempHsAD)).acumula = -1
                                'FGZ - 04/09/2014 --------------------------------------------
                                arrTempHsAD(UBound(arrTempHsAD)).hd_nro = objRsConfig!hd_nro
                                'FGZ - 04/09/2014 --------------------------------------------
                                creado = True
                            Else
                                If Not creado Then
                                    'EAM- Si entra por aca es porque el tipo de hora ya exite y le suma a lo acumulado los nuevos valores para convertir al final.
                                    Call SHoras(arrTempHsAD(auxPosHsADTemp).Horas, objrsHC!Horas, TotHorHHMM)
                                    arrTempHsAD(auxPosHsADTemp).Horas = TotHorHHMM
                                    arrTempHsAD(auxPosHsADTemp).Cant_hs = arrTempHsAD(auxPosHsADTemp).Cant_hs + objrsHC!horcant
                                    creado = True
                                End If
                            End If
                        Else
                            'EAM- Entra si no acumula las hs, es por Franja. convierte y va acumulando cada conversión.
                            'FGZ - 04/09/2014 ---------------------------------------------------------------
                            'Call AD_01(THOrigen, thdestino, objrsHC!horcant, horas_conv)
                            Call AD_01(objRsConfig!hd_nro, THOrigen, thdestino, objrsHC!horcant, horas_conv)
                            'FGZ - 04/09/2014 ---------------------------------------------------------------
                            auxPosHsADTemp = BuscarHsADTemp(arrTempHsAD, thdestino, 0)
                            TotHorHHMM = CHoras(horas_conv, 60)
                            If (auxPosHsADTemp = 0) Then
                                ReDim Preserve arrTempHsAD(UBound(arrTempHsAD) + 1)
                                arrTempHsAD(UBound(arrTempHsAD)).thnro = objRsConfig!hd_thdestino
                                arrTempHsAD(UBound(arrTempHsAD)).Horas = Replace(TotHorHHMM, "'", "")
                                arrTempHsAD(UBound(arrTempHsAD)).Cant_hs = horas_conv
                                arrTempHsAD(UBound(arrTempHsAD)).acumula = 0
                                'FGZ - 04/09/2014 --------------------------------------------
                                arrTempHsAD(UBound(arrTempHsAD)).hd_nro = objRsConfig!hd_nro
                                'FGZ - 04/09/2014 --------------------------------------------
                            Else
                                Horas_Acum_HS = "00:00"
                                TotHorHHMM = Replace(TotHorHHMM, "'", "")
                                Call SHoras(TotHorHHMM, IIf(IsNull(arrTempHsAD(auxPosHsADTemp).Horas), "00:00", arrTempHsAD(auxPosHsADTemp).Horas), Horas_Acum_HS)
                                arrTempHsAD(UBound(arrTempHsAD)).thnro = objRsConfig!hd_thdestino
                                arrTempHsAD(UBound(arrTempHsAD)).Horas = Replace(Horas_Acum_HS, "'", "")
                                arrTempHsAD(UBound(arrTempHsAD)).Cant_hs = arrTempHsAD(UBound(arrTempHsAD)).Cant_hs + horas_conv
                                'FGZ - 04/09/2014 --------------------------------------------
                                arrTempHsAD(UBound(arrTempHsAD)).hd_nro = objRsConfig!hd_nro
                                'FGZ - 04/09/2014 --------------------------------------------
                            End If
...
End If
    ...
    'EAM- Busca el tipo de hora configurada para convertir
    For i = 1 To UBound(arrTempHsAD)
        If arrTempHsAD(i).acumula = -1 Then
            Horas_Acum_HS = "00:00"
            Call SHoras(Horas_Acum_HS, IIf(IsNull(arrTempHsAD(i).Horas), "00:00", arrTempHsAD(i).Horas), Horas_Acum_HS)
            thdestino = arrTempHsAD(i).thnro
    
            'Valida y convierte la cantidad
            'FGZ - 04/09/2014 ---------------------------------------------------------------
            'Call AD_01(THOrigen, arrTempHsAD(i).thnro, arrTempHsAD(i).Cant_hs, horas_conv)
            Call AD_01(arrTempHsAD(i).hd_nro, THOrigen, arrTempHsAD(i).thnro, arrTempHsAD(i).Cant_hs, horas_conv)
            'FGZ - 04/09/2014 ---------------------------------------------------------------
    
            'If horas_conv <> 0 Then mdf 20/05/2015
            If horas_conv > 0 Then
                'EAM- Busca el tipo de hora configurada a convertir
                auxPosHsADTemp = BuscarHsADTemp(arrTempHsAD, arrTempHsAD(i).thnro, 0)
    
                If auxPosHsADTemp = 0 Then
                    TotHorHHMM = CHoras(horas_conv, 60)
    
                    ReDim Preserve arrTempHsAD(UBound(arrTempHsAD) + 1)
                    arrTempHsAD(UBound(arrTempHsAD)).thnro = thdestino
                    arrTempHsAD(UBound(arrTempHsAD)).Horas = Replace(TotHorHHMM, "'", "")
                    arrTempHsAD(UBound(arrTempHsAD)).Cant_hs = horas_conv
                    arrTempHsAD(UBound(arrTempHsAD)).acumula = 0
                    'FGZ - 04/09/2014 --------------------------------------------
                    arrTempHsAD(UBound(arrTempHsAD)).hd_nro = arrTempHsAD(i).hd_nro
                    'FGZ - 04/09/2014 --------------------------------------------
                Else
                    TotHorHHMM = CHoras(horas_conv, 60)
                    
                    arrTempHsAD(auxPosHsADTemp).thnro = thdestino
                    arrTempHsAD(auxPosHsADTemp).Horas = Replace(TotHorHHMM, "'", "")
                    arrTempHsAD(auxPosHsADTemp).Cant_hs = horas_conv
                    arrTempHsAD(auxPosHsADTemp).acumula = 0
                End If
    
                    If depurar Then
                        Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & " Se agrego el dato: " & horas_conv
                    End If
            End If
        End If
    Next
...
Public Sub AD_02(ByVal traza As Boolean, ByVal P_NroTer As Long, ByVal P_NroTur As Long, ByVal P_NroGru As Long, ByVal P_HorNro As Long, ByVal Fecha_Inicio As Date, ByRef arrTempHsAD() As THsAD)
'------------------------------------------------------------------------------------------
...
If Not objrsHC.EOF Then
    ...
    If depurar Then
        Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Busca si el Tipo de Hora tiene alguna configuracion"
    End If
'    StrSql = "SELECT hd_thdestino FROM gti_config_horadia WHERE hd_thorigen = " & objrsHC!thnro & " AND  turnro = " & P_NroTur & _
'             " AND ( hd_programa is null OR hd_programa = '') ORDER BY hd_nro"
    StrSql = "SELECT hd_nro, hd_thorigen, hd_thdestino,hd_feriados,hd_nolaborables,hd_laborable,hd_franja FROM gti_config_horadia " & _
            " WHERE hd_thorigen = " & objrsHC!thnro & " AND  turnro = " & P_NroTur & " AND ( hd_programa is null OR hd_programa = '' OR hd_programa = 0) ORDER BY hd_nro"
    OpenRecordset StrSql, objRsConfig
            ...
            Else
                ...
                Else
                    ...
                    Else
                        ...
                        Call AD_01(objRsConfig!hd_nro, P_HorNro, thdestino, objrsHC!horcant, horas_conv)
                        'FGZ - 04/09/2014 ---------------------------------------------------------------
                        ...
                        ReDim Preserve arrTempHsAD(UBound(arrTempHsAD) + 1)
                        arrTempHsAD(UBound(arrTempHsAD)).thnro = objRsConfig!hd_thdestino
                        arrTempHsAD(UBound(arrTempHsAD)).Horas = TotHorHHMM
                        arrTempHsAD(UBound(arrTempHsAD)).Cant_hs = horas_conv
                        arrTempHsAD(UBound(arrTempHsAD)).acumula = -1
                        'FGZ - 04/09/2014 --------------------------------------------
                        arrTempHsAD(UBound(arrTempHsAD)).hd_nro = objRsConfig!hd_nro
...
Public Sub AD_05(hornro As Long, THOrigen As Long, nrotur As Long, NroTer As Long, Nro_Grupo As Long, Fecha_Inicio As Date, depurar As Boolean, ByRef arrTempHsAD() As THsAD)
'------------------------------------------------------------------------------
'Archivo:   gtiad05.p
...
If Not objrsHC.EOF Then
    ...
    If depurar Then
        Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Busca si el Tipo de Hora tiene alguna configuracion"
    End If
    StrSql = "SELECT hd_nro,hd_thdestino,hd_feriados,hd_nolaborables,hd_laborable FROM gti_config_horadia "
    ...
    Do While Not objrsCFG_H.EOF
        ...
        Else
            ...
            Else
                ...
                'EAM- Busca el tipo de hora configurada a convertir
                For i = 1 To UBound(arrTempHsAD)
                    If (arrTempHsAD(i).thnro = thdestino) And (arrTempHsAD(i).acumula = -1) Then
                        Horas_Acum = Horas_Acum + arrTempHsAD(i).Cant_hs
                        Call SHoras(Horas_Acum_HS, IIf(IsNull(arrTempHsAD(i).Horas), "00:00", arrTempHsAD(i).Horas), Horas_Acum_HS)
                        Exit For
                    End If
                Next
                ...
                Call AD_01(objrsCFG_H!hd_nro, THOrigen, thdestino, Horas_Acum, horas_conv)
                'FGZ - 04/09/2014 ---------------------------------------------------------------
                ...
                If horas_conv <> 0 Then
                        ...
                        ReDim Preserve arrTempHsAD(UBound(arrTempHsAD) + 1)
                        arrTempHsAD(UBound(arrTempHsAD)).thnro = thdestino
                        arrTempHsAD(UBound(arrTempHsAD)).Horas = Horas_Acum_HS
                        arrTempHsAD(UBound(arrTempHsAD)).Cant_hs = horas_conv
                        arrTempHsAD(UBound(arrTempHsAD)).acumula = 0
                        'FGZ - 04/09/2014 --------------------------------------------
                        arrTempHsAD(UBound(arrTempHsAD)).hd_nro = objrsCFG_H!hd_nro
                        'FGZ - 04/09/2014 --------------------------------------------
...
Public Sub AD_06(p_fecha As Date, NroTer As Long, depura As Boolean, ByRef arrTempHsAD() As THsAD)
'------------------------------------------------------------------------------
'Archivo:   gtiad05.p
...
'------------------------------------------------------------------------------
  ...
  Dim auxPosHsADTemp As Long
    ...
    'EAM- Limpia el arreglo para volver a analizar las horas
    ReDim arrTempHsAD(0)
   ...
   'Busca el dia del empleado, es decir si es un d¡a laborable o no
    ...
    Do While Not objrsAC_D.EOF
        ...
        'Busca la Config. del tipo de hora a convertir
        'EAM- 12/10/2012 - Se modifico la sql para r3
        'r4 --> StrSql = "SELECT hd_nro,hd_thorigen,hd_thdestino, hd_feriados,hd_nolaborables,hd_laborable,hd_programa,hd_franja,hd_alcance,ws_diassem "
        'r3 --> StrSql = "SELECT hd_nro,hd_thorigen,hd_thdestino, hd_feriados,hd_nolaborables,hd_laborable,hd_programa,hd_franja "
        StrSql = "SELECT hd_nro,hd_thorigen,hd_thdestino, hd_feriados,hd_nolaborables,hd_laborable,hd_programa,hd_franja,hd_alcance,ws_diassem " & _
                 " FROM gti_config_horadia " & _
                 " WHERE turnro = " & Nro_Turno & " AND   hd_thorigen  = " & objrsAC_D!thnro & _
                 " AND   hd_ordenproc = 2 "
...
'            End If
            ...
            'EAM- Verifica si aplica para ese el dia que se esta procesando o no
            'LED - 31/07/2013 - Se descomento para r4
            If Not AplicaConversionEnDia(p_fecha, objRsConfig!ws_diassem, objRsConfig!hd_alcance, NroTer, objRsConfig!hd_nro) Then
                If depurar Then
                    Flog.writeline Espacios(Tabulador * (Nivel_Tab_Log)) & "Conversion del Tipo de Hora " & objRsConfig!hd_thorigen & " No aplica para el día " & p_fecha
                End If
                Continua = False
            Else
                Continua = True
            ...
            If (objRsConfig!hd_programa <> "" And objRsConfig!hd_programa <> 0) Then GoTo SiguienteConfig
            'FGZ - 22/08/2014 ----------------------
            ...
            Call AD_01(objRsConfig!hd_nro, objRsConfig!hd_thorigen, objRsConfig!hd_thdestino, objrsAC_D!adcanthoras, horas_conv)
            'FGZ - 04/09/2014 --------------------------------------------------------------------------------
            ...
            If horas_conv <> 0 Then
                ...
                Else
                    ...
                    Call SHoras(TotHorHHMM, IIf(IsNull(arrTempHsAD(auxPosHsADTemp).Horas), "00:00", arrTempHsAD(auxPosHsADTemp).Horas), TotHorHHMM)
                    
                    arrTempHsAD(auxPosHsADTemp).Horas = TotHorHHMM
                    arrTempHsAD(auxPosHsADTemp).Cant_hs = arrTempHsAD(auxPosHsADTemp).Cant_hs + horas_conv
...
'End If
Call CrearAD(NroTer, arrTempHsAD())
'If depurar Then
...
'  Ult Modif: FGZ - 30/05/2014 - se agrego la Customizacion 32 para LUCAIOLI.
'  --------------------------------------------------------------------------------------------------
'  Programas Validos:
'  ------------------
'   1.  Conversion              : Conversion estandar a Jornada produccion
...
'   25. NOC_Sykes_CR            : Customizacion para Sykes - CR.
'   26. DESC_LUNCH              : Customizacion para Sykes - CR.
'   27. SABADOS                 : Customizacion para Gral Mills.
'   28. ConvMedioFeriadosSaDo   : Customizacion para AMR (Asociacion Medica de Rosario).
'   29. DESC_CITA_MED           : Customizacion para Sykes - SV.
'   29. DESTINO                 : Customizacion para Lucaioli.
'  --------------------------------------------------------------------------------------------------
Dim Horas_Oblig As Single
Dim HorasRes As Single
Dim TotHor As Single
...
Dim Hora_Lunch As Single
Dim rs_Prog As New ADODB.Recordset
Dim Aux_Programa As String
...
'EAM- 12/10/2012 - Se modifico la sql para r3
'R3 --> StrSql = StrSql & " , hd_feriados, hd_nolaborables, hd_laborable, hd_nro,hd_franja "
'21/11/2007 - Diego Rosso
'Agrege  gti_config_horadia.hd_maximo y gti_config_horadia.hd_maximo en el select
'*********************************************************
...
StrSql = StrSql & " , hd_feriados, hd_nolaborables, hd_laborable, hd_nro,hd_franja,hd_alcance,ws_diassem "
...
Do While Not objRsCFG.EOF
    ...
    'FGZ - 07/03/2014 ----------
    If IsNumeric(objRsCFG!hd_programa) Then
        StrSql = " Select conv_nombre FROM gti_conversion_prog WHERE conv_nro  = " & objRsCFG!hd_programa
        OpenRecordset StrSql, rs_Prog
        If Not rs_Prog.EOF Then
            Aux_Programa = rs_Prog!conv_nombre
        Else
            Aux_Programa = ""
        End If
    Else
        Aux_Programa = objRsCFG!hd_programa
    End If

    'FGZ - 07/03/2014 ----------
    
    'gti_config_horadia.hd_programa
    
    
    If UCase(Aux_Programa) = UCase("Conversion") Then
        Call Prog_1_Conversion(p_ternro, p_fecha, objRsCFG!hd_thorigen, objRsCFG!hd_thdestino, objRsCFG!adcanthoras)
    End If
    
    'FGZ - 08/09/2006 - Customizacion para Moño Azul
    If UCase(Aux_Programa) = UCase("ConversionProd") Then
        'Programa que convierte de la Cantida de Horas en Cantidad de Días para el turno del Empleado
    ...
    'Esta es una conversión que se aplica en TMK
    If UCase(Aux_Programa) = UCase("SACO1HORA") Then
        Call Prog_SACO1HORA(p_ternro, p_fecha, objRsCFG!hd_thorigen, objRsCFG!hd_thdestino, objRsCFG!adcanthoras)
    End If
    

    'FGZ - 23/09/2004
    If UCase(Aux_Programa) = "REDONDEO" Then
        StrSql = " SELECT * FROM gti_acumdiario WHERE thnro = " & objRsCFG!hd_thdestino & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
        OpenRecordset StrSql, objRsAD
    ...
    'FGZ - 28/06/2005
    If UCase(Aux_Programa) = "SABADOS SCHERING" Then
        If Weekday(p_fecha) = vbSaturday Then
            TipoHora50 = 1
            TipoHora100 = 2
    ...
    'FGZ - 27/07/2005
    'Nueva Politica de Convrersion para Estrada
    If Aux_Programa = "NormalesEstrada" Then
        'Programa que convierte de la Cantida de Horas en Cantidad de Días para el turno del Empleado
        StrSql = "SELECT * FROM gti_dias WHERE dianro = " & Nro_Dia
        StrSql = StrSql & " ORDER BY diaorden"
        OpenRecordset StrSql, objRs
    ...
    'CAT - 14/02/2006
    If UCase(Aux_Programa) = "H100DIVINO" Then
            TipoHora100 = 6
            TipoHora150 = 38
            TipoNocturna = 8
    ...
    'FGZ - 22/09/2006 - Estas conversiones las encontre en fuentes viejos (en teoria son para ICI)
    If Aux_Programa = "ConvNormales" Then
        If Weekday(p_fecha) = 1 Then
            StrSql = " SELECT * FROM gti_horcumplido WHERE "
            StrSql = StrSql & " gti_horcumplido.ternro = " & p_ternro & " AND "
            StrSql = StrSql & " horfecrep = " & ConvFecha(p_fecha) & " AND "
    ...
    'FGZ - 27/11/2006 Nueva Politica de Convrersion para AGD
    If UCase(Aux_Programa) = UCase("Feriados") Then
        Set objFeriado.Conexion = objConn
        Set objFeriado.ConexionTraza = CnTraza
        esFeriado = objFeriado.Feriado(p_fecha, Empleado.Ternro, depurar)
        If depurar Then
        ...
        If Not objRsAD.EOF Then
            ...
            If esFeriado Then
                ...
                If Not Hay_Licencia Then
                    ...
                    'Si Origen = destino ==> Quedan como estan Sino Creo el destino
                    If objRsCFG!hd_thdestino <> objRsCFG!hd_thorigen Then
                        'Inserto el tipo de hora
                        TotHorHHMM = CHoras(Horas_Oblig, 60)
    ...
    'FGZ - 14/11/2006
    'Nueva Politica de Convrersion para AGD
    If UCase(Aux_Programa) = UCase("Feriados_Estr") Then
        StrSql = " SELECT * FROM gti_acumdiario WHERE thnro = " & objRsCFG!hd_thdestino & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
        OpenRecordset StrSql, objRsAD
        If Not objRsAD.EOF Then
            'Verifico las condiciones
    ...
    'FGZ - 27/11/2006
    'Nueva Politica de Convrersion para AGD
    If UCase(Aux_Programa) = UCase("Feriados_Trabajados") Then
        'las obligatorias serian las de horigen
        'Las excedentes serian las destino
    ...
    'FGZ - 27/11/2006
    'Nueva Politica de Convrersion para AGD
    If UCase(Aux_Programa) = UCase("Feriados_Trabajados_SD") Then
        'las obligatorias serian las de horigen
        'Las excedentes serian las destino
        If Weekday(p_fecha) > 1 And Weekday(p_fecha) < 7 Then 'Dia de semana
            'Calculo la cantidad de horas Obligatorias
    ...
    'FGZ - 01/02/2007
    'Nueva Convrersion para Frigorifico Gorina ---------------------------------------
    If UCase(Aux_Programa) = UCase("HORASDESTAJO") Then
            If depurar Then
                Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "HORASDESTAJO -->"
            End If
            'Busco Centro de Costo y el Sector
    ...
    'FGZ - 01/02/2007
    'Nueva Convrersion para Frigorifico Gorina ---------------------------------------
    If UCase(Aux_Programa) = UCase("ADICALMUERZO") Then
            If depurar Then
                Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "ADICALMUERZO -->"
            End If
            'Busco Centro de Costo y el Sector
        ...
        'FGZ - 01/02/2007
        'Nueva Convrersion para Frigorifico Gorina ---------------------------------------
        If UCase(Aux_Programa) = UCase("PEFICIENCIA") Then
            If depurar Then
                Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "PEFICIENCIA -->"
            End If
            'Busco Centro de Costo y el Sector
    ...
    'en los dias Configurados.
    'Tiene en cuenta el mínimo de Horas Normales realizadas para que esto tenga efecto.
    If UCase(Aux_Programa) = UCase("Completar") Then
        'Llamo a la politica
        Call Politica(571)
        TH_Anormalidad = st_TipoHora1
    ...
    'Sabado hasta las 13 hs  genere Hs adicionales Aut. y después del sabado a las 13 y hasta el domingo genera Hs 100%  según corresponda.
    '*********************************************************************************************************************************************************************
    If UCase(Aux_Programa) = "SABADODOMINGO MV" Then
    ...
    If UCase(Aux_Programa) = UCase("TopeMinimo") Then
        Continua = False
        If esFeriado Then
        ...
        'EAM- Verifica si aplica para ese el dia que se esta procesando o no
        'LED - 31/07/2013  Se descomento para R4
        If Not AplicaConversionEnDia(p_fecha, objRsCFG!ws_diassem, objRsCFG!hd_alcance, p_ternro, objRsCFG!hd_nro) Then
            If depurar Then
                Flog.writeline Espacios(Tabulador * (Nivel_Tab_Log)) & "Conversion del Tipo de Hora " & objRsCFG!hd_thorigen & " No aplica para el día " & p_fecha
            End If
            Continua = False
        Else
            Continua = True
    ...
    'Deja la cantidad minima como Origen y la diferencia entre la cantidad y el minimo como destino
    If UCase(Aux_Programa) = UCase("TopeMinimo_LV") Then
        Continua = False
        If esFeriado Then
            If objRsCFG!hd_feriados Then
                Continua = True
...
'        'EAM- Verifica si aplica para ese el dia que se esta procesando o no
    ...
    'FGZ - 16/12/2007 - Tope de Hs
    'Deja la cantidad minima como Origen y la diferencia entre la cantidad y el minimo como destino
    If UCase(Aux_Programa) = UCase("TopeMinimo_SD") Then
        Continua = False
        If esFeriado Then
            If objRsCFG!hd_feriados Then
                Continua = True
    ...
    If UCase(Aux_Programa) = UCase("VALES_SAT") Then
    ...
    If UCase(Aux_Programa) = UCase("TURNO_PLUS") Then
    ...
    If UCase(Aux_Programa) = UCase("Feriados_MV") Then
    ...
    'NOC_Sykes_CR ---------------------------------------------------------------------------
    'Esta es una conversión que se aplica en Sykes Costa Rica
    'Modifica la cantidad de hs origen a la cantidad de hs destino. El tipo de hora sigue siendo el mismo, solo se modifica la cantidad
    If UCase(Aux_Programa) = UCase("NOC_Sykes_CR") Then
        'Chequeo que las horas que hizo el empleado esten dentro del minimo y maximo configurado para la conversion
        If (objRsCFG!adcanthoras <= objRsCFG!hd_maximo) And (objRsCFG!adcanthoras >= objRsCFG!hd_minimo) Then
        ...
        End If
        If depurar Then
            Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Cantidad de horas destino " & Hora_Ori
        End If

        If Hora_Ori > 0 Then
           'HJI - 31/01/2012 -----------------------------------------------------------------
           'La conversión debe ver si ya existen horas de lunch.
            StrSql = " SELECT * FROM gti_acumdiario WHERE thnro = 45 AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
            objRsAD.Close
            OpenRecordset StrSql, objRsAD
            If objRsAD.EOF Then
                Hora_Lunch = 0
            Else
                Hora_Lunch = CLng(Tiempo_de_Lunch(p_fecha) / 60)
            End If
            'HJI - 31/01/2012 -----------------------------------------------------------------

            If (Hora_Ori > Hora_Lunch) Then
                Hora_Ori = Hora_Ori - Hora_Lunch
            End If

            TotHorHHMM = CHoras(Hora_Ori, 60)
            StrSql = " UPDATE gti_acumdiario SET horas =" & TotHorHHMM & ",adcanthoras = " & Round(Hora_Ori, 3)
            StrSql = StrSql & " WHERE thnro = " & objRsCFG!hd_thorigen & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
            objConn.Execute StrSql, , adExecuteNoRecords
            If depurar Then
                Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Update " & StrSql
        ...
        Else
            ...
            objConn.Execute StrSql, , adExecuteNoRecords
            If depurar Then
                Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Elimino las hs de origern "
            End If
        End If
        End If
    End If
    
    Call DESC_LUNCH(p_ternro, Aux_Programa, Hora_Dest, Hora_Lunch)
    'NOC_Sykes_CR ---------------------------------------------------------------------------

'    'DESC_LUNCH ---------------------------------------------------------------------------
'    'Esta es una conversión que se aplica en Sykes
'    'Descuenta la cantidad del tipo de hora destino de la conversion en el tipo de hora origen
'    If UCase(objRsCFG!hd_programa) = UCase("DESC_LUNCH") Then
'
'        Horas_Oblig = 0
'
'        StrSql = "SELECT * FROM gti_detturtemp WHERE (ternro =" & p_ternro & " ) and (" & _
...
'        End If
'
'        Hora_Dest = objRsCFG!adcanthoras
'
'        'FGZ - 25/10/2011 -----------------------------------------------------------------
'        'La conversión debe ver si ya existen horas de lunch ==> no resta nada.
'        'Sino si las horas origen son por día completo entonces resta horas de lunch (teoricas).
'        StrSql = " SELECT * FROM gti_acumdiario WHERE thnro = " & objRsCFG!hd_thdestino & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
'        OpenRecordset StrSql, objRsAD
'        If Not objRsAD.EOF Then
'            Hora_Lunch = 0
'        Else
'            'si las horas origen son por día completo entonces resta horas de lunch (teoricas).
'            If Hora_Dest >= Horas_Oblig Then
'                Hora_Lunch = CLng(Tiempo_de_Lunch(p_fecha) / 60)
'            Else
'                Hora_Lunch = 0
'            End If
...
'
'        If Hora_Dest >= Horas_Oblig Then
'             Hora_Dest = Hora_Dest - Hora_Lunch
'        Else
'             Hora_Dest = Hora_Dest
'        End If
'
...
'        StrSql = StrSql & " WHERE thnro = " & objRsCFG!hd_thorigen & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
'        objConn.Execute StrSql, , adExecuteNoRecords
'    End If 'esta es una conversion que se aplica en Sykes
'    'DESC_LUNCH ---------------------------------------------------------------------------

    'SABADOS ------------------------------------------------------------------------------
    If UCase(Aux_Programa) = UCase("SABADOS") Then
        Call Prog_Sabados(p_ternro, p_fecha, objRsCFG!hd_thorigen, objRsCFG!hd_thdestino, objRsCFG!adcanthoras)
    End If
    'SABADOS ------------------------------------------------------------------------------

    'EAM- 16553 - (ARM -Asociacion Médica de Rosario)
    'Conversión, para que transforme las hs de origen en destino dividiendo por la cantidad de hs teóricas del día a excepción de los sábados que se debe dividir por las teóricas por 2.
    If (UCase(Aux_Programa) = UCase("ConvMedioFeriadosSaDo")) Then
        Call Prog_Conv_Feriado_Sa_DO(p_ternro, p_fecha, objRsCFG!hd_thorigen, objRsCFG!hd_thdestino, objRsCFG!adcanthoras)
    End If

    'FGZ - 07/03/2014 ---------------------------------------------
    If (UCase(Aux_Programa) = UCase("DESC_CITA_MED")) Then
        Call Prog_DESC_CITA_MED(p_ternro, p_fecha, objRsCFG!hd_thorigen, objRsCFG!hd_thdestino, objRsCFG!adcanthoras, objRsCFG!hd_minimo, objRsCFG!hd_maximo)
    End If
    'FGZ - 07/03/2014 ---------------------------------------------

    'FGZ - 30/05/2014 ---------------------------------------------
    If (UCase(Aux_Programa) = UCase("DESTINO")) Then
        Call Prog_DESTINO(p_ternro, p_fecha, objRsCFG!hd_thorigen, objRsCFG!hd_thdestino, objRsCFG!adcanthoras, objRsCFG!hd_minimo, objRsCFG!hd_maximo)
    End If
    'FGZ - 30/05/2014 ---------------------------------------------
...
Public Sub CrearAD(P_NroTer As Long, ByRef arrTempHsAD() As THsAD)
Dim Acum As Double
...
For i = 1 To UBound(arrTempHsAD)
    ...
    'EAM- Si es distinto de -1 son los valores calculados que hay que Insertar
    'If arrTempHsAD(i).acumula = -1 Then   ' MDF-30/09/2015 -  aca habia un <>, pero tiene q ser igual----
    If arrTempHsAD(i).acumula <> -1 Then   ' MDF-07/10/2015 - iba un <>, habia un problema de configuracion q no detecte y ahora lo corrijo
        If arrTempHsAD(i).thnro <> AuxThnro Then
            Acum = 0
            Acum_HS = "00:00"
        End If
        
        Acum = Acum + arrTempHsAD(i).Cant_hs
        Call SHoras(Acum_HS, IIf(IsNull(arrTempHsAD(i).Horas), "00:00", arrTempHsAD(i).Horas), Acum_HS)
        
        AuxThnro = arrTempHsAD(i).thnro
        
        StrSql = "SELECT * FROM gti_acumdiario WHERE adfecha = " & ConvFecha(p_fecha) & _
            " AND ternro = " & P_NroTer & " AND thnro = " & arrTempHsAD(i).thnro
        OpenRecordset StrSql, objrsAC_D
        
        If objrsAC_D.EOF Then
            'inserto
            StrSql = "INSERT INTO gti_acumdiario (ternro,thnro,adfecha,horas, adcanthoras, admanual) VALUES (" & _
                    P_NroTer & "," & arrTempHsAD(i).thnro & "," & ConvFecha(p_fecha) & ",'" & Acum_HS & "'," & Acum & ", 0)"
        Else
            'modifico el campo de cantidad de horas
            'Call SHoras(Acum_HS, objrsWFAd!Horas, Aux_HS)
            
            StrSql = "UPDATE gti_acumdiario SET horas = '" & arrTempHsAD(i).Horas & "', adcanthoras = " & arrTempHsAD(i).Cant_hs & ", advalido = -1 WHERE adfecha = " & ConvFecha(p_fecha) & _
                " AND ternro = " & P_NroTer & " AND thnro = " & arrTempHsAD(i).thnro
        End If
        objConn.Execute StrSql, , adExecuteNoRecords
...
'OrigEAM- Lo comente porque no se va a usar mas la tabla
'Private Sub Limpia()
...
Public Sub Prog_Sabados(ByVal p_ternro As Long, ByVal p_fecha As Date, ByVal hd_thorigen As Long, ByVal hd_thdestino As Long, ByVal Cantidad As Single)
'  --------------------------------------------------------------------------------------------------
'  Descripción: Conversion CUSTOM Sabados (Gral Mills)
'  Autor : FGZ - 14/11/2011
'  El empleado puede ingresar en cualquier hora de un día sábado,
'   siendo las primeras 4 hs obligatorias y normales y el excedente (más de 4 hs) es extra con la siguiente condición:
'       1-  Es al 50 % si esas excedentes están en el rango de 00 a 13 hs.
'       2-  Es al 100 % si esas excedentes están en el rango de 13 a 24 hs.
'------------
'   Se espera que se configure la FP para que las primeras 4 hs sean normales (es decir, por cantidad, entre 0 y 4 Normales)
'       y es resto sea %50.
' Luego la conversion se debe configurar como Origen %50 y destino %100.
'   y luego de ejecutar dejará como hs de origen todas las hechas antes de las 13 y como hs destino posteriores a las 13 hs
'  --------------------------------------------------------------------------------------------------
'  Ult Modif:
'  --------------------------------------------------------------------------------------------------
Dim Horas_Oblig As Single
...
Dim TotHorHHMM As String
    ...
    If depurar Then
        Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Conversion Sabados"
    End If
    On Error GoTo ME_Conv
                ...
                If Weekday(p_fecha) = vbSaturday Or Weekday(p_fecha) = vbSunday Then
                    TipoHora100 = hd_thdestino
                    THOrigen = hd_thorigen
                        ...
                        'Actualizo las hs destino
                            If Total100 <> 0 Then
                                ...
                                If Total100 = Cantidad Then
                                 'Borro original
                                     StrSql = " DELETE From gti_acumdiario "
                                     StrSql = StrSql & " WHERE thnro = " & hd_thorigen & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
                                    objConn.Execute StrSql, , adExecuteNoRecords
                                Else
                                    'Update original de objRsCFG!adcanthoras  -total100
                                    TotHorHHMM = CHoras(Cantidad - Total100, 60)
                                    StrSql = " UPDATE gti_acumdiario SET horas = " & TotHorHHMM & ",adcanthoras = " & Round(Cantidad - Total100, 3)
                                    StrSql = StrSql & " WHERE thnro = " & hd_thorigen & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
                                    objConn.Execute StrSql, , adExecuteNoRecords
                                End If
                            End If
...
Public Sub Prog_SACO1HORA(ByVal p_ternro As Long, ByVal p_fecha As Date, ByVal hd_thorigen As Long, ByVal hd_thdestino As Long, ByVal Cantidad As Single)
'  --------------------------------------------------------------------------------------------------
'  Descripción: Conversion CUSTOM ICI
'  Autor :
'  --------------------------------------------------------------------------------------------------
'  Ult Modif: FGZ - 14/11/2011 - Se pasó a procedimiento
'  --------------------------------------------------------------------------------------------------
Dim Horas_Oblig As Single
Dim HorasRes As Single
Dim TotHor As Single
...
Dim TotHorHHMM As String
    ...
    If depurar Then
        Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Conversion SACO1HORA"
    End If
    On Error GoTo ME_Conv
            ...
            Else
                TotHorHHMM = CHoras(Hora_Dest, 60)
                StrSql = " INSERT INTO gti_acumdiario(adfecha,ternro,thnro,horas,adcanthoras,admanual,advalido) " & _
                         " VALUES (" & ConvFecha(p_fecha) & "," & p_ternro & "," & hd_thdestino & "," & TotHorHHMM & "," & Hora_Dest & "," & _
                         CInt(False) & "," & CInt(True) & ")"
                objConn.Execute StrSql, , adExecuteNoRecords
            End If
        ...
        If Horas_Oblig > 0 Then
            ...
            If Not objRsAD.EOF Then
                ...
                Else
                    'FGZ - 07/10/2014 --------------
                    'HorasRes = objRsAD!adcanthoras / Horas_Oblig
                    HorasRes = Round(objRsAD!adcanthoras / Horas_Oblig, 3)
                End If
            ...
            Else
                'FGZ - 07/10/2014 --------------
                HorasRes = Round(Cantidad / Horas_Oblig, 3)
                TotHorHHMM = CHoras(HorasRes, 60)
                ...
                StrSql = " INSERT INTO gti_acumdiario(adfecha,ternro,thnro, horas,adcanthoras,admanual,advalido) " & _
                         " VALUES (" & ConvFecha(p_fecha) & "," & p_ternro & "," & hd_thdestino & "," & TotHorHHMM & "," & HorasRes & "," & _
                         CInt(False) & "," & CInt(True) & ")"
                'FGZ - 07/10/2014 --------------
                objConn.Execute StrSql, , adExecuteNoRecords
            End If
...
'EAM- Busca las horas de un derterminado tipo y devuelve la posicion del registro.
Private Function BuscarHsADTemp(datos() As THsAD, ByVal thnro As Integer, ByVal acumula As Integer) As Long
 Dim i, posicion As Long
 
    posicion = 0
    For i = 0 To UBound(datos)
        If (datos(i).thnro = thnro) And (datos(i).acumula = acumula) Then
            posicion = i
            Exit For
        End If
    Next
...
'EAM(11450)- Esta función determina segun lo configurado si aplica la conversion para ese dia o no
Private Function AplicaConversionEnDia(Fecha As Date, ByVal diasAplica As String, ByVal Alcance As Integer, ByVal Ternro As Long, ByVal hd_nro As Long) As Boolean
 Dim arrDias() As String
 Dim aplica As Boolean
 Dim objRs As New ADODB.Recordset
    
    arrDias = Split(diasAplica, ",")
    aplica = False
    
    Select Case Alcance
        Case 1:
            If (CInt(arrDias(Weekday(Fecha) - 1)) = 1) Then
                aplica = True
            End If
        Case 2:
            StrSql = "SELECT * FROM gti_config_horadia_estr " & _
                    " INNER JOIN his_estructura ON his_estructura.estrnro = gti_config_horadia_estr.estrnro " & _
                    " WHERE  hd_nro= " & hd_nro & " AND ternro= " & Ternro & _
                    " AND (htetdesde <= " & ConvFecha(p_fecha) & ")" & _
                    " AND ((" & ConvFecha(p_fecha) & " <= htethasta) or (htethasta is null))"
            OpenRecordset StrSql, objRs
        ...
        Case 3:
            StrSql = "SELECT * FROM gti_config_horadia_empleados " & _
                    " WHERE  hd_nro= " & hd_nro & " AND gti_config_horadia_empleados.ternro= " & Ternro
            OpenRecordset StrSql, objRs
            ...
            End If
        Case Else
            'No aplico mal configurado el alcance
    End Select
    
    AplicaConversionEnDia = aplica
End Function



Public Sub Prog_Conv_Feriado_Sa_DO(ByVal p_ternro As Long, ByVal p_fecha As Date, ByVal hd_thorigen As Long, ByVal hd_thdestino As Long, ByVal Cantidad As Single)
'  --------------------------------------------------------------------------------------------------
'  Descripción: Conversion CUSTOM AMR(Asociación Médica de Rosario)
'  Autor : EAM - 27/11/2012
'  Si los empleados que tiene el turno full-time y trabaja un sabado y domingo se le debe abonar medio feriado
'  --------------------------------------------------------------------------------------------------
Dim rs_Estructura As New ADODB.Recordset
Dim objRsAD As New ADODB.Recordset
Dim esFeriado As Boolean
...
Dim TotHorHHMM As String
    ...
    estrnro = 1245
    If depurar Then
        Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Conversión a Medio feriado para Sabados y domingo"
    End If
    On Error GoTo ME_Conv

    Call buscar_horas_turno(Horas_Oblig, horas_max, horas_min)
    '
    'EAM- DETERMINAR SI ES FULL TIME TIPO DE ESTRUCTURA
    StrSql = " SELECT estructura.estrnro, estructura.estrcodext FROM his_estructura " & _
            ...
            " WHERE his_estructura.ternro = " & p_ternro & " AND estructura.tenro= " & Tenro & " AND estructura.estrnro= " & estrnro & _
            " AND (his_estructura.htetdesde <= " & ConvFecha(p_fecha) & ") " & " AND ((" & ConvFecha(p_fecha) & " <= his_estructura.htethasta) or (his_estructura.htethasta is null))"
    OpenRecordset StrSql, rs_Estructura
    
    If Not rs_Estructura.EOF Then
        'EAM- Si no es fin de archivo es porque tiene la estructura de regimen horario de Full Time
        turnoFullTime = True
        If depurar Then
            Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Turno " & rs_Estructura!estrnro & " Full Time."
        End If
    Else
        'Empleado jornalero. No es full time
        turnoFullTime = False
        If depurar Then
            Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "El tercero: " & p_ternro & " no tiene regimen horario configurado (21)."
        End If
    End If
    
    If turnoFullTime Then
        esFeriado = objFeriado.Feriado(p_fecha, p_ternro, True)
        
        If ((Weekday(p_fecha) = vbSaturday) Or (Weekday(p_fecha) = vbSunday)) And (esFeriado = True) Then
            If depurar Then
                Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Conversión de un feriado para un Turno Full-Time en un día Sabado o Domingo"
            End If
            
            canthoras = (Round(Cantidad / Horas_Oblig, 3) * 1 / 2)
        Else
            If depurar Then
                Flog.writeline Espacios(Tabulador * Nivel_Tab_Log) & "Conversión de Turno Full-Time en un día laborable"
            End If
            
            canthoras = Round(Cantidad / Horas_Oblig, 3)
        End If
    Else
        'Conversión normal
        canthoras = Round(Cantidad / Horas_Oblig, 3)
    ...
    If Not objRsAD.EOF Then
        ...
        StrSql = " UPDATE gti_acumdiario SET adcanthoras = " & canthoras & ", horas =" & TotHorHHMM & _
                " WHERE thnro = " & hd_thdestino & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
        objConn.Execute StrSql, , adExecuteNoRecords
    Else
        TotHorHHMM = CHoras(canthoras, 60)
        StrSql = " INSERT INTO gti_acumdiario(adfecha,ternro,thnro,horas,adcanthoras,admanual,advalido) " & _
                " VALUES (" & ConvFecha(p_fecha) & "," & p_ternro & "," & hd_thdestino & "," & TotHorHHMM & "," & canthoras & "," & _
                CInt(False) & "," & CInt(True) & ")"
        objConn.Execute StrSql, , adExecuteNoRecords
    End If
FIN:
...
Public Sub DESC_LUNCH(p_ternro As Long, programa, ByRef Hora_Dest As Single, ByRef Hora_Lunch As Single)
'DESC_LUNCH ---------------------------------------------------------------------------
'Esta es una conversión que se aplica en Sykes
'Descuenta la cantidad del tipo de hora destino de la conversion en el tipo de hora origen
Dim objRsCFG As New ADODB.Recordset
    ...
    If UCase(programa) = UCase("DESC_LUNCH") Then

        Horas_Oblig = 0

        StrSql = "SELECT * FROM gti_detturtemp WHERE (ternro =" & p_ternro & " ) and (" & _
                 "gttempdesde <= " & ConvFecha(p_fecha) & ") and (" & ConvFecha(p_fecha) & " <= gttemphasta)"
        OpenRecordset StrSql, objRs
        ...
        If Horas_Oblig = 0 Then
            'Programa que convierte de la Cantida de Horas en Cantidad de Días para el turno del Empleado
            StrSql = "SELECT * FROM gti_dias WHERE subturnro = " & Nro_Subturno & " ORDER BY diaorden"
            OpenRecordset StrSql, objRs
            If Not objRs.EOF Then
                Horas_Oblig = objRs!diacanthoras
            End If
        ...
        StrSql = " UPDATE gti_acumdiario SET adcanthoras = " & Hora_Dest & _
                " WHERE thnro = " & objRsCFG!hd_thorigen & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
        objConn.Execute StrSql, , adExecuteNoRecords
    End If
    'esta es una conversion que se aplica en Sykes
    'DESC_LUNCH ---------------------------------------------------------------------------
End Sub



Public Sub Prog_DESC_CITA_MED(ByVal p_ternro As Long, ByVal p_fecha As Date, ByVal hd_thorigen As Long, ByVal hd_thdestino As Long, ByVal Cantidad As Single, ByVal MinimoHS As Single, ByVal MaximoHS As Single)
'--------------------------------------------------------------------------------------------------------------
'Esta es una conversión que se aplica en Sykes SV
'Convierte las hs de origen a destino topeando a 8 hs y si la cant de origen es mayor a 4 ==> calcula la diferencia entre la cant origen y 4.
Dim objRsCFG As New ADODB.Recordset
Dim objRsAD As New ADODB.Recordset
Dim canthoras As Single
Dim TotHorHHMM As String
        ...
        If Cantidad >= MinimoHS Then
            ...
            If Horas_Oblig > 0 Then
                If Horas_Oblig > MaximoHS Then
                    Horas_Oblig = MaximoHS
                End If
                
                If Cantidad > Horas_Oblig Then
                    Cantidad = Horas_Oblig
                End If
                            
                canthoras = Cantidad - MinimoHS
                'canthoras = Horas_Oblig - Cantidad
...
Public Sub Prog_DESTINO(ByVal p_ternro As Long, ByVal p_fecha As Date, ByVal hd_thorigen As Long, ByVal hd_thdestino As Long, ByVal Cantidad As Single, ByVal MinimoHS As Single, ByVal MaximoHS As Single)
'--------------------------------------------------------------------------------------------------------------
'Esta es una conversión que se aplica en Lucaioli
'Convierte las hs de origen a destino si ya existen hs de Destino. Ejemplo Convierta las hs Extras al 50% en Horas extras al 100% si hay horas al 100%
Dim objRsCFG As New ADODB.Recordset
Dim objRsAD As New ADODB.Recordset
Dim canthoras As Single
Dim TotHorHHMM As String
    
        StrSql = " SELECT adcanthoras FROM gti_acumdiario WHERE thnro = " & hd_thdestino & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
        OpenRecordset StrSql, objRsAD
        If Not objRsAD.EOF Then
            canthoras = Cantidad + objRsAD!adcanthoras
            TotHorHHMM = CHoras(canthoras, 60)
            StrSql = " UPDATE gti_acumdiario SET adcanthoras = " & canthoras & ", horas =" & TotHorHHMM & _
                    " WHERE thnro = " & hd_thdestino & " AND ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha)
            objConn.Execute StrSql, , adExecuteNoRecords
            
            'Borro las hs de origen
            StrSql = "DELETE FROM gti_acumdiario WHERE " & _
            " ternro = " & p_ternro & " AND adfecha = " & ConvFecha(p_fecha) & " AND thnro = " & hd_thorigen
            objConn.Execute StrSql, , adExecuteNoRecords
