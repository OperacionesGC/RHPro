...
' Descripcion: 13/01/2015 - Gonzalez Nicolás Se cambio Empleado.Ternro por Tercero
' ---------------------------------------------------------------------------------------------
...
'    StrSql = StrSql & ")"
    ...
    Else
        ...
        Encontro_Jus = False
        Do While Not rs.EOF And Not Encontro_Jus
            If rs!jussigla = "NOV" Then
                ...
                If Firma_Novedades Then
                    ...
                    StrSql = StrSql & " AND cysfircodext = '" & rs!juscodext & "' "
            ...
            If Nov_Firmada Then
                ...
                If Not IsNull(rs!turnro) Then
                    ...
                    If Not rs_Turno.EOF Then
                        ...
                        Turno.Numero = rs_Turno!turnro
                        Turno.Nombre = Trim(rs_Turno!turdesabr)
                        Turno.Tipo = rs_Turno!TipoTurno
...
'    End If
    ...
    Do While Not rs.EOF
        ...
        If Firmado Then
            ...
            StrSql = StrSql & " INNER JOIN estructura ON his_estructura.estrnro = estructura.estrnro "
            'StrSql = StrSql & " WHERE (tanro = " & lngAlcanGrupo & ") AND (ternro = " & Empleado.Ternro & ") AND "
            StrSql = StrSql & " WHERE (tanro = " & lngAlcanGrupo & ") AND (ternro = " & Tercero & ") AND " 'NG
            StrSql = StrSql & " (htetdesde <= " & ConvFecha(Fecha) & ") AND "
            StrSql = StrSql & " ((" & ConvFecha(Fecha) & " <= htethasta) or (htethasta is null))"
            StrSql = StrSql & " ORDER BY alcance_testr.alteorden DESC, his_estructura.htetdesde Desc "
            If rs.State = adStateOpen Then rs.Close
...
Dim rs_FT As New ADODB.Recordset
Dim BD_Inicio As Long
Dim BD_Fin As Long
Dim BD_Termine As Boolean
Dim BD_Subturno As Long
...
'    End If
    ...
    'If (num_dia = 0) Then num_dia = rs!turtamanio  'es el primer dia del turno
     ...
     'Calculo en que subturno cae el día
     StrSql = "SELECT subturnro,subturtamanio,subturiteracion FROM gti_subturno "
     StrSql = StrSql & " WHERE turnro = " & Nro_Turno & " ORDER BY subturorden"
     OpenRecordset StrSql, objRs
     Do While Not (BD_Termine)
         objRs.MoveFirst
         Do While Not objRs.EOF And Not BD_Termine
           BD_Subturno = objRs!subturnro
           BD_Fin = BD_Inicio + (objRs!subturtamanio * objRs!subturiteracion) - 1
           If (BD_Inicio <= dif_dias) And (dif_dias <= BD_Fin) Then
              BD_Termine = True
           Else
              BD_Inicio = BD_Fin + 1
           End If
           If BD_Termine And BD_Inicio = 1 And dif_dias > objRs!subturtamanio Then
             BD_Inicio = objRs!subturtamanio + 1
           End If
           objRs.MoveNext
         Loop
      Loop
    
    'Calculo en que dia del subturno cae
    BD_Termine = False
    StrSql = "SELECT gti_dias.*,gti_subturno.subturdesabr, gti_subturno.subtgen FROM gti_subturno "
    StrSql = StrSql & " INNER JOIN gti_dias ON gti_subturno.subturnro = gti_dias.subturnro "
    StrSql = StrSql & " WHERE (gti_subturno.subturnro = " & BD_Subturno & " AND turnro = " & Nro_Turno & ") "
    ...
    Do While Not BD_Termine
       objRs.MoveFirst
        Do While Not (objRs.EOF) And Not (BD_Termine)
           ...
           BD_Fin = BD_Inicio + objRs!diaiteracion - 1
           If (BD_Inicio <= dif_dias) And (dif_dias <= BD_Fin) Then
                BD_Termine = True
                
                Dia.Trabaja = True
                Dia.Orden_Dia = objRs!diaorden
                Dia.Nro_Dia = objRs!diaNro
                Dia.Nro_Subturno = objRs!subturnro
                Dia.NombreSubTurno = objRs!subturdesabr
                Dia.Dia_Libre = objRs!Dialibre
                'FGZ - 31/07/2009 - le agregue ese campo
                Dia.Genera = IIf(EsNulo(objRs!subtgen), 0, objRs!subtgen) 'objRs!subtgen
                'FGZ - 31/07/2009 - le agregue ese campo
           Else
                BD_Inicio = BD_Fin + 1
           End If
...
' ---------------------------------------------------------------------------------------------
Dim Fecha As Date
Dim Movi_F As Boolean
    
    Fecha = p_fecha
    
    'StrSql = "SELECT diahoradesde1,diahoradesde2, diahoradesde3,diahorahasta1,diahorahasta2,diahorahasta3 FROM gti_dias WHERE dianro = " & Nro_Dia
    StrSql = "SELECT diahoradesde1,diahoradesde2, diahoradesde3,diahorahasta1,diahorahasta2,diahorahasta3, diasiguiente FROM gti_dias WHERE dianro = " & Nro_Dia
    OpenRecordset StrSql, objRs
    If Not objRs.EOF Then
        ...
        If (objRs!diahoradesde1 <> "0000" Or objRs!diahorahasta1 <> "0000") Then
            ...
            If Not Movi_F Then
                If Not EsNulo(objRs!diasiguiente) Then
                    If objRs!diasiguiente = 1 Then
                        FS1 = DateAdd("d", 1, Fecha)
                    End If
                End If
            End If
            'FGZ - 28/11/2014 --------------------------
        ...
        If (objRs!diahoradesde2 <> "0000" Or objRs!diahorahasta2 <> "0000") Then
            ...
            'FGZ - 28/11/2014 --------------------------
            If Not EsNulo(objRs!diasiguiente) Then
                If objRs!diasiguiente = 2 Then
                    FS2 = DateAdd("d", 1, Fecha)
                End If
            End If
            'FGZ - 28/11/2014 --------------------------
        ...
        If (objRs!diahoradesde3 <> "0000" Or objRs!diahorahasta3 <> "0000") Then
            ...
            'FGZ - 28/11/2014 --------------------------
            If Not EsNulo(objRs!diasiguiente) Then
                If objRs!diasiguiente = 3 Then
                    FS3 = DateAdd("d", 1, Fecha)
                End If
            End If
            'FGZ - 28/11/2014 --------------------------
    ...
    End If
        ...
        'FGZ - 03/02/2015 -----------------------------------------------------
    If P_Asignacion Then
        StrSql = "SELECT ttemphdesde1,ttemphhasta1,ttemphdesde2,ttemphhasta2,ttemphdesde3,ttemphhasta3,diaanterior FROM gti_detturtemp WHERE ternro= " & Empleado.Ternro & _
        " AND gttempdesde <= " & ConvFecha(p_fecha) & " AND gttemphasta >= " & ConvFecha(p_fecha)
        OpenRecordset StrSql, objRs
        If Not objRs.EOF Then
            If Not EsNulo(objRs!ttemphdesde1) And Not EsNulo(objRs!ttemphhasta1) Then
                E1 = objRs!ttemphdesde1
                S1 = objRs!ttemphhasta1
            Else
                If depurar Then
            ...
            If Not EsNulo(objRs!ttemphdesde2) And Not EsNulo(objRs!ttemphhasta2) Then
                E2 = objRs!ttemphdesde2
                S2 = objRs!ttemphhasta2
            Else
                If depurar Then
                    Flog.writeline Espacios(Tabulador * 3) & "Parte con hora nula. Se descarta."
                End If
            ...
            If Not EsNulo(objRs!ttemphdesde3) And Not EsNulo(objRs!ttemphhasta3) Then
                E3 = objRs!ttemphdesde3
                S3 = objRs!ttemphhasta3
            Else
                If depurar Then
                    Flog.writeline Espacios(Tabulador * 3) & "Parte con hora nula. Se descarta."
                End If
