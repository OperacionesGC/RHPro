Attribute VB_Name = "MdlRHproRepAusentismo"
Option Explicit

Global Const Version = "1.00" 'Sebastian Stremel
Global Const FechaModificacion = "23/06/2014"
'Global Const UltimaModificacion = " CAS-24478 - PLA - Reporte de Ausentismo "
...
On Error GoTo ce
    ...
    Archivo = PathFLog & "RHPro_RepAusentismo" & "-" & NroProc & ".log"
    ...
    StrSql = "INSERT INTO rep_ausentismo_cab "
    ...
    'llamar al reporte
    Call reporteAusentismo(legdesde, leghasta, estado, Mes, anio, orden)
...
Private Sub reporteAusentismo(legdesde, leghasta, estado, Mes, anio, orden)
...
On Error GoTo pepe
    ...
    StrSql = StrSql & " WHERE repnro=440 "
    ...
    If Not rs_consultas.EOF Then
        Do While Not rs_consultas.EOF
            Select Case rs_consultas!confnrocol
                ...
                Case 4:
                    codHsTeoricas = IIf(EsNulo(rs_consultas!confval), 0, rs_consultas!confval)
                Case 5:
                    codhs(1) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 6:
                    codhs(2) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 7:
                    codhs(3) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 8:
                    codhs(4) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 9:
                    codhs(5) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 10:
                    codhs(6) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 11:
                    codhs(7) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 12:
                    codhs(8) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 13:
                    codhs(9) = IIf(EsNulo(rs_consultas!confval2), "0", rs_consultas!confval2)
                Case 14:
        ...
        For k = 0 To UBound(negocios)
            ...
            If Not rs_consultas.EOF Then
                Do While Not rs_consultas.EOF
                    ...
                    'busco las hs teoricas
                    lngAlcanGrupo = 0
                    HsTeoricas = (CDbl(calcularHsTeoricas(Ternro, fechadesde, fechahasta, codHsTeoricas))) * CDbl(cantdias2)
                    'hasta aca
                    
                    For o = 1 To 9
                        'Busco las hs de enfermedad dudosa
                        Fecha = fechadesde
                        For i = 1 To cantdias2
                            If tieneEstructura(Ternro, Fecha, area, negocio) Then
                                hs = Split(codhs(o), ",")
                                For l = 0 To UBound(hs)
                                    HsReporte(o) = HsReporte(o) + (CDbl(buscarHsAusencia(Ternro, Fecha, hs(l))))
                                Next
                            Else
                                HsReporte(o) = HsReporte(o) + CDbl(0)
                            End If
                            Fecha = DateAdd("d", 1, Fecha)
                        Next
                        'hasta aca
                    ...
                    StrSql = "INSERT INTO rep_ausentismo_det "
                    StrSql = StrSql & " ("
                    ...
                    For i = 1 To cantdias2
                        StrSql = StrSql & ",dia" & i
                    Next
                    StrSql = StrSql & ", HsTeoricas "
                    StrSql = StrSql & ",IndAusentismo "
                    StrSql = StrSql & ",Enfermedad "
                    StrSql = StrSql & ",EnfDudosa "
                    StrSql = StrSql & ",accART "
                    StrSql = StrSql & ",Injustificada "
                    StrSql = StrSql & ",LicGremial "
                    StrSql = StrSql & ",LicNacimiento "
                    StrSql = StrSql & ",LicFallecimiento "
                    StrSql = StrSql & ",EnfFamiliar "
                    StrSql = StrSql & ",Otros "
                    StrSql = StrSql & " ) "
                    StrSql = StrSql & " VALUES ( "
                    StrSql = StrSql & NroProc
                    ...
                    Next
                    StrSql = StrSql & "," & HsTeoricas
                    StrSql = StrSql & ",0 " 'indice de ausentismo
                    For i = 1 To 9
                        StrSql = StrSql & "," & HsReporte(i)
                    Next
                    StrSql = StrSql & ")"
                    objConnRep.Execute StrSql, , adExecuteNoRecords
                    'hasta aca
