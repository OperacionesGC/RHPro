Attribute VB_Name = "repInformeAusentismo2"
Option Explicit

Global Const Version = "1.00"
Global Const FechaModificacion = "02/12/2014"
Global Const UltimaModificacion = "Inicial - Sebastian Stremel - CAS-27374 - FARMOGRAFICA - CUSTOM FILTRO DE REPORTE DE AUSENTISMO II"
...
usaParametros = False
    ...
    Nombre_Arch = PathFLog & "ReporteInfAusentismo2" & "-" & NroProceso & ".log"
...
On Error GoTo MError
    ...
    '-------------------------------------------------------------------------
    StrSql = "SELECT * FROM confrep WHERE repnro = 466"
    OpenRecordset StrSql, rsConsult
    ...
    If Not rsConsult.EOF Then
        StrSql = "INSERT INTO rep_inf_ausentismo2 (bpronro,fecdesde,fechasta,tenro,tedabr,"
    ...
    StrSql = StrSql & " INNER JOIN confrep ON estructura.estrnro = confrep.confval AND confrep.repnro = 466 AND conftipo = 'EST' "
    ...
    Do While Not rsConsult.EOF
        ...
        If fecdesde_empl <= fechasta_empl Then
            'If (estrnro_ant <> rsConsult!estrnro) Or (ternro_ant <> rsConsult!Ternro) Then
            ...
            StrSql = " SELECT sum(adcanthoras) canthoras, gti_acumdiario.thnro, thdesc "
            StrSql = StrSql & " FROM gti_acumdiario "
            StrSql = StrSql & " INNER JOIN tiphora t on t.thnro = gti_acumdiario.thnro "
            ...
            StrSql = StrSql & " GROUP BY gti_acumdiario.thnro, thdesc "
            ...
            '    End If
                'inserto en la tabla
                StrSql = " INSERT INTO rep_inf_ausentismo_det "
                StrSql = StrSql & " ("
                StrSql = StrSql & " bpronro, ternro,estrnro "
                If CStr(tenro3) <> "" And CStr(tenro3) <> "0" Then
                ...
                Else
                    ...
                    End If
                End If
                StrSql = StrSql & " ,thnro, thdesc, threm, canths)"
                StrSql = StrSql & " VALUES"
                StrSql = StrSql & "("
                StrSql = StrSql & NroProceso & ","
                StrSql = StrSql & rsConsult!Ternro & ","
                StrSql = StrSql & rsConsult!estrnro
                If CStr(tenro3) <> "" And CStr(tenro3) <> "0" Then
                    StrSql = StrSql & "," & rsConsult!tenro1 & "," & rsConsult!estrnro1 & "," & rsConsult!tenro2 & "," & rsConsult!estrnro2 & "," & rsConsult!tenro3 & "," & rsConsult!estrnro3
                Else
                    If CStr(tenro2) <> "" And CStr(tenro2) <> "0" Then
                        StrSql = StrSql & "," & rsConsult!tenro1 & "," & rsConsult!estrnro1 & "," & rsConsult!tenro2 & "," & rsConsult!estrnro2
                    Else
                        If CStr(tenro1) <> "" And CStr(tenro1) <> "0" Then
                            StrSql = StrSql & "," & rsConsult!tenro1
                            StrSql = StrSql & "," & rsConsult!estrnro1
                        End If
                    End If
                End If
                StrSql = StrSql & "," & rsacumdiario!thnro & ","
                StrSql = StrSql & "'" & rsacumdiario!thdesc & "',"
                StrSql = StrSql & "-1,"
                StrSql = StrSql & rsacumdiario!canthoras
                StrSql = StrSql & ")"
                objConn.Execute StrSql, , adExecuteNoRecords
                'hasta aca
...
'                StrSql = "INSERT INTO rep_inf_aus_det (bpronro,estrnro,estrdabr,thnro,thdesc,threm,toths,tothsestr,cantempl,orden"
'                If CStr(tenro3) <> "" And CStr(tenro3) <> "0" Then
'                    StrSql = StrSql & ", tenro1, estrnro1, tenro2, estrnro2, tenro3, estrnro3  "
'                Else
'                    If CStr(tenro2) <> "" And CStr(tenro2) <> "0" Then
