Attribute VB_Name = "MdlSemestral"
Option Explicit
...
Global InicioSemestre As Date
Global FinSemestre As Date
Global Separador As String
Global Archivo As String

Global HorasSemestre As Single
Global THSemestre As Integer ' Tipo de Hora del semestre
Global HSSemestre As Integer ' Cantidad de Horas del semestre, Fijas configuradas en el conrep
Global THSTrimestre As Integer
Global THSControl As Integer
Global TotalHS As Integer
Global THSExcedente As Integer
Global THSExc50 As Integer
Global THSExc100 As Integer
Global thsdeltrim As Integer
...
'    End If
    ...
    Set Flog = fs.CreateTextFile(PathFLog & "\semestral" & "-" & NroProceso & ".log", True)
    ...
    If Not objRs.EOF Then
        ...
        InicioSemestre = fechaDesde
        FinSemestre = fechaHasta
        'para el reporte Semestral
        Call ObtenerConfiguracionSemestral
    End If
    ...
    Do While Not objRs.EOF
       'MyBeginTrans
        ...
        Call SumarHoras(objRs!cgtinro, fechaDesde, fechaHasta, objRs!gtprocnro, objRs!gpanro)
...
Dim ExtrasTrimestre As Single
Dim HSControl As Single
Dim HorasExc50 As Single
Dim HorasExc100 As Single
Dim HorasExcedente As Single
Dim PrimeraVez As Boolean
Dim PrimeraAct As Boolean
    ...
    ' punto 3. Horas del Semestre
       
    DiasHabiles = ContarDiasHabiles(InicioSemestre, FinSemestre)
    HorasSemestre = (HSSemestre) * (DiasHabiles - ContarDiasProporcionalesDelSemestre(Ternro, InicioSemestre, FinSemestre)) / DiasHabiles
    Call InsertarEnGtiDet(Nro_Cab, THSemestre, HorasSemestre)
        
    ' punto 4, Minimo de Horas extras
    ExtrasTrimestre = BuscarExtrasTrimestre(Ternro, InicioSemestre, FinSemestre)
    Call InsertarEnGtiDet(Nro_Cab, THSTrimestre, ExtrasTrimestre)
        
    ' punto 5, Horas de Control
    HSControl = HorasSemestre + ExtrasTrimestre
    Call InsertarEnGtiDet(Nro_Cab, THSControl, HSControl)
        
        
    'Totalizo las horas trabajadas de interés en el semestre que suman
    
    StrSql = "select turnro from gti_config_ctacte"
    OpenRecordset StrSql, objRs
    PrimerTurno = objRs!turnro
       
    StrSql = "SELECT ccsigno, gti_acumdiario.adfecha, gti_acumdiario.thnro, gti_acumdiario.ternro,gti_cab.cgtinro, SUM(adcanthoras) as Sumahoras FROM gti_cab "
    ...
    StrSql = StrSql & " GROUP BY  gti_acumdiario.thnro,ccsigno,gti_acumdiario.ternro, gti_cab.cgtinro,gti_acumdiario.adfecha "
    ...
    Do While Not objRs.EOF
        ...
        If objRs!ccsigno Then
            ...
            If HorasSuma > HSControl Then
                If PrimeraVez Then
                    PrimeraVez = False
                    PrimeraAct = True
                    HorasExcedente = HorasSuma - HSControl
                    HorasSuma = HSControl
                Else
                    HorasExcedente = HorasExcedente + objRs!Sumahoras
                End If
                ' si es de Lun a Vie ==> sumo en EXC100
                If Weekday(objRs!adfecha) = 1 Or Weekday(objRs!adfecha) = 7 Then
                    If objRs!thnro = THSExc50 Then
                        If PrimeraAct Then
                            HorasExc50 = HorasExc50 + HorasExcedente
                            PrimeraAct = False
                        Else
                            HorasExc50 = HorasExc50 + objRs!Sumahoras
                        End If
                    Else
                        If PrimeraAct Then
                            HorasExc100 = HorasExc100 + HorasExcedente
                            PrimeraAct = False
                        Else
                            HorasExc100 = HorasExc100 + objRs!Sumahoras
                        End If
                    End If
                Else 'sumo en EXC50
                    If PrimeraAct Then
    ...
    ' Punto 6, Total de Horas Es la suma de horas.
    If HorasSuma <= HorasSemestre Then
        acu = HorasSuma
    Else
        acu = HorasSuma - HSControl
    End If
    
    Call InsertarEnGtiDet(Nro_Cab, TotalHS, HorasSuma)
    
    ' punto 7, Horas Excedentes
    Call InsertarEnGtiDet(Nro_Cab, THSExcedente, HorasExcedente)
    
    ' punto 8, Horas Excedentes 50
    Call InsertarEnGtiDet(Nro_Cab, THSExc50, HorasExc50)
       
    ' punto 9, Horas Excedentes 100
    Call InsertarEnGtiDet(Nro_Cab, THSExc100, HorasExc100)
              
End Sub


Private Function BuscarExtrasTrimestre(NroDeTercero As Long, FecInicioSem As Date, FecFinSem As Date) As Integer
'Busco en la gti_det del trimesral anterior para esa persona, si existe registro que coincida
'con el tipo de hora configurado en el confrep columna 4 se completa con este valor,
'de lo contrario se completa con 0
Dim rsGTI_det As New ADODB.Recordset
Dim hs As Integer
Dim Cab As Long

' Seteo el valor por default
hs = 0

StrSql = " SELECT gti_procacum.gtprocnro,gti_procacum.gpadesde, gti_cab.cgtinro,gti_procacum.gtprocnro" & _
         " FROM  gti_procacum " & _
         " INNER JOIN gti_cab ON  gti_cab.gpanro = gti_procacum.gpanro " & _
         " WHERE gti_cab.ternro = " & NroDeTercero & " AND gpadesde = " & ConvFecha(FecInicioSem) & _
         " AND gti_procacum.gtprocnro = 5"
...
Do While Not objRs.EOF
    ...
    StrSql = "SELECT  * FROM gti_det WHERE cgtinro =" & Cab & " AND thnro = " & THSTrimestre
    OpenRecordset StrSql, rsGTI_det
    If Not rsGTI_det.EOF Then
        hs = rsGTI_det!dgticant
    End If
    
    objRs.MoveNext
Loop

BuscarExtrasTrimestre = hs
...
Private Sub ObtenerConfiguracionSemestral()

' Obtiene la configuracion para el reporte semestral
' del confrep

    StrSql = "SELECT confval FROM confrep WHERE repnro = 57 AND confnrocol = 1"
    OpenRecordset StrSql, objRs
    HSSemestre = objRs!confval

    StrSql = "SELECT confval FROM confrep WHERE repnro = 57 AND confnrocol = 3"
    OpenRecordset StrSql, objRs
    THSemestre = objRs!confval
    
    StrSql = "SELECT confval FROM confrep WHERE repnro = 57 AND confnrocol = 4"
    OpenRecordset StrSql, objRs
    THSTrimestre = objRs!confval
    
    StrSql = "SELECT confval FROM confrep WHERE repnro = 57 AND confnrocol = 5"
    OpenRecordset StrSql, objRs
    THSControl = objRs!confval
    
    StrSql = "SELECT confval FROM confrep WHERE repnro = 57 AND confnrocol = 6"
    OpenRecordset StrSql, objRs
    TotalHS = objRs!confval
    
    StrSql = "SELECT confval FROM confrep WHERE repnro = 57 AND confnrocol = 7"
    OpenRecordset StrSql, objRs
    THSExcedente = objRs!confval
    
    StrSql = "SELECT confval FROM confrep WHERE repnro = 57 AND confnrocol = 8"
    OpenRecordset StrSql, objRs
    THSExc50 = objRs!confval

    StrSql = "SELECT confval FROM confrep WHERE repnro = 57 AND confnrocol = 9"
    OpenRecordset StrSql, objRs
    THSExc100 = objRs!confval
