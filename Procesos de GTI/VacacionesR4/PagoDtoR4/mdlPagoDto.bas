...
'Const Version = "3.33"
'Const FechaVersion = "02/12/2014" 'EAM- Se adecuo para r4
                                   
'Const Version = "3.34"
'Const FechaVersion = "22/02/2016" 'MDZ - CAS-32780 - RH Pro (Producto) - Chile - Vacaciones [entrega 2]
'        Se regresa la version 22 de la politica 1500 a la logica que tenia hasta la version 3.29
'

Const Version = "3.35"
Const FechaVersion = "26/02/2016" 'LED - CAS-35547 - APEX AMERICA - GIV EL SALVADOR
'        Correcio en la version 22 de la politica 1500, para el calculo de dias a pagar y descontar
'
...
'VERSION NO LIBERADA **************************
'Const Version = "X.XX"
'Const FechaVersion = "22/01/2015" 'FGZ
...
'            End If
        ...
        Else
            ...
            StrSql = "SELECT (select min(vdetfdesde) FROM vacacion_detalle WHERE vacacion_detalle.vacnro=vacacion.vacnro and ternro=" & objReg!Ternro & " AND vdetfdesde <= " & ConvFecha(fecha_hasta) & ") vacfecdesde, "
            StrSql = StrSql & "(select max(vdetfhasta) FROM vacacion_detalle WHERE vacacion_detalle.vacnro=vacacion.vacnro and ternro=" & objReg!Ternro & " AND  vdetfhastae >= " & ConvFecha(fecha_desde) & ") vacfechasta, "
            StrSql = StrSql & "vacnro,vacdesc,vacsist,vacproxfecpago, vacanio, empnro, vacestado, alcannivel, ternro FROM vacacion "
            StrSql = StrSql & "WHERE (select min(vdetfdesde) FROM vacacion_detalle WHERE vacacion_detalle.vacnro=vacacion.vacnro and ternro=" & objReg!Ternro & ") is not null"
            StrSql = StrSql & " ORDER BY vacnro"
