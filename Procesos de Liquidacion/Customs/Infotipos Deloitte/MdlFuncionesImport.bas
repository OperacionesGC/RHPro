...
' ---------------------------------------------------------------------------------------------
    ...
    SubAccion = ""
    Contador_Familiar = 1
    PrimeraVez_Infotipo_0000 = True
    PrimeraVez_Infotipo_0001 = True
    PrimeraVez_Infotipo_0002 = True
    PrimeraVez_Infotipo_0006 = True
    PrimeraVez_Infotipo_0007 = True 'No se hace
    PrimeraVez_Infotipo_0008 = True
    PrimeraVez_Infotipo_0009 = True
    PrimeraVez_Infotipo_0014 = True
    PrimeraVez_Infotipo_0015 = True
    PrimeraVez_Infotipo_0016 = True 'No se hace
    PrimeraVez_Infotipo_0021 = True
    PrimeraVez_Infotipo_0023 = True 'No se hace
    PrimeraVez_Infotipo_0027 = True
    PrimeraVez_Infotipo_0041 = True
    PrimeraVez_Infotipo_0050 = True 'No se hace
    PrimeraVez_Infotipo_0057 = True
    PrimeraVez_Infotipo_0105 = True 'No se hace
    PrimeraVez_Infotipo_0185 = True
    PrimeraVez_Infotipo_0267 = True 'No se hace
    PrimeraVez_Infotipo_0389 = True
    PrimeraVez_Infotipo_0390 = True
    PrimeraVez_Infotipo_0391 = True
    PrimeraVez_Infotipo_0392 = True
    PrimeraVez_Infotipo_0393 = True
    PrimeraVez_Infotipo_0394 = True
    PrimeraVez_Infotipo_0416 = True 'No se hace
    PrimeraVez_Infotipo_2001 = True
    PrimeraVez_Infotipo_2006 = True 'No se hace
    PrimeraVez_Infotipo_2010 = True
    PrimeraVez_Infotipo_2013 = True 'No se hace
    PrimeraVez_Infotipo_9004 = True 'Benefits Brasil ???????????
    
    'Son para el infotipo 0390
    Cant_Acumulada = 0
    Monto_Acumulado = 0
...
Public Sub Insertar_Novedad(ByVal Nomina As String, ByVal Monto As Single, Cantidad As Single, ByVal Fecha_Desde As String, ByVal Fecha_Hasta As String, ByVal Texto As String)
' ---------------------------------------------------------------------------------------------
...
Public Sub Insertar_Prestamo(ByVal ternro As Long, ByVal Nro_Prestamo As String, ByVal Tipo_Prestamo As String, ByVal Poliza As String, ByVal Monto_Prestamo As Double, ByVal Modalidad_Prestamo As String, ByVal Cuotas_Prestamo As Integer, ByVal Concepto1 As String, ByVal Monto1 As Double, ByVal Cuota1 As Integer, ByVal Concepto2 As String, ByVal Monto2 As Double, ByVal Cuota2 As Integer, ByVal Moneda As Long, ByVal Inicio_Validez As String, ByVal Fin_Validez As String)
' ---------------------------------------------------------------------------------------------
' Descripcion: Procedimiento que Inserta, el prestamo/cuota de prestamo
' Autor      : FGZ
' Fecha      : 31/07/2007
' Ultima Mod.:
' Descripcion:
...
'Pasos
'1_ verifico si el prestamo ya esta creado
'2_ si no existe ==> lo creo
'3_ Verifico si la/s cuotas ya estan generadas
'4_ si no estan creadas ==> las genero
'5_ Verifico si es la ultima cuota

'la descripcion del prestamo va a ser la numeracion que le asigna SAP al mismo

If Tipo_Prestamo = "1" Or Tipo_Prestamo = "01" Then
    Texto = " 01 - Automotor"
    LNprenro = 1
Else
    Texto = " 02 - Personal"
    LNprenro = 2
End If
Descripcion = Nro_Prestamo & Texto


    'Busco la sucursal del empleado
    StrSql = " SELECT * FROM his_estructura "
    StrSql = StrSql & " WHERE ternro = " & ternro
    StrSql = StrSql & " AND tenro = 1"
    StrSql = StrSql & " AND (htetdesde <= " & ConvFecha(Inicio_Validez) & ") "
    StrSql = StrSql & " AND ((" & ConvFecha(Inicio_Validez) & " <= htethasta) or (htethasta is null))"
    StrSql = StrSql & " ORDER BY htetdesde "
    OpenRecordset StrSql, rs_Est
    If Not rs_Est.EOF Then
        Sucursal = rs_Est!Estrnro
    Else
        Sucursal = 0
    ...
    StrSql = StrSql & " WHERE prestamo.predesc = '" & UCase(Descripcion) & "'"
    StrSql = StrSql & " AND ternro = " & ternro
    OpenRecordset StrSql, rs_Prestamo
    If rs_Prestamo.EOF Then
        'No hay prestamo ==> lo creo
        
        StrSql = "INSERT INTO prestamo "
        StrSql = StrSql & "(predesc, preimp, preanio, precantcuo, sucursal, estnro, ternro, premes, "
        StrSql = StrSql & " prequin, monnro, lnprenro, prefecotor, precompr, iduser, quincenal, preiva, pretna, preotrosgas, prediavto )"
        StrSql = StrSql & " VALUES ( '" & Descripcion & "'"
        StrSql = StrSql & "," & Monto_Prestamo
        StrSql = StrSql & "," & Year(Inicio_Validez)
        StrSql = StrSql & "," & Cuotas_Prestamo
        StrSql = StrSql & "," & Sucursal
        StrSql = StrSql & ",3" 'Simepre aprobado '& l_estnro
        StrSql = StrSql & "," & ternro
        StrSql = StrSql & "," & Month(Inicio_Validez)
        StrSql = StrSql & " ,0"
        StrSql = StrSql & "," & Moneda
        StrSql = StrSql & "," & LNprenro
        StrSql = StrSql & "," & ConvFecha(Inicio_Validez)
        StrSql = StrSql & ",'" & Poliza & "'"
        StrSql = StrSql & ",'PU12'"
        StrSql = StrSql & ",0"
        StrSql = StrSql & ",0"
        StrSql = StrSql & ",0"
        StrSql = StrSql & ",0"
        'StrSql = StrSql & "," & ConvFecha(Fin_Validez)
        StrSql = StrSql & ",01" 'es la fecha de vencimiento de la cuota
        StrSql = StrSql & ")"
        objConn.Execute StrSql, , adExecuteNoRecords
        
        'Recupero el prenro que se le asignó
        prenro = getLastIdentity(objConn, "prestamo")
        
        'Genero las cuotas
        mes = Month(Inicio_Validez)
        Anio = Year(Inicio_Validez)
        If Cuota1 <> 0 And Monto1 <> 0 Then
            For i = 1 To Cuota1
                Fechavto = DateAdd("d", -1, DateAdd("m", 1, CDate("01/" & mes & "/" & Anio)))
                Saldo = Monto_Prestamo - (Monto1 * i)
                ...
                'StrSql = StrSql & ",05" 'es la fecha de vencimiento de la cuota
                StrSql = StrSql & ")"
                objConn.Execute StrSql, , adExecuteNoRecords
                
                If Modalidad_Prestamo = "1" Or Modalidad_Prestamo = "01" Then
                    'Modalidad de una cuota por mes
                    If mes = 12 Then
                        mes = 1
                        Anio = Anio + 1
                    Else
                    ...
                    End If
                Else
                    'Modalidad de cuota doble en Junio y Diciembre
                    If mes = 12 Then
                        If Ya_Genero Then
                            mes = 1
                            Anio = Anio + 1
                            Ya_Genero = False
                        Else
                            'no avanzo de mes porque es cuota doble
                            Ya_Genero = True
                        End If
                    Else
                        If mes = 6 Then
            ...
            Next i
        Else
            'No se creará la cuota .... cantidad de cuotas y/o monto = 0
        End If
        Monto_Prestamo = Saldo
        
        'Saldo
        If Cuota2 <> 0 And Monto2 <> 0 Then
            For i = 1 To Cuota2
                Fechavto = DateAdd("d", -1, DateAdd("m", 1, CDate("01/" & mes & "/" & Anio)))
                Saldo = Monto_Prestamo - (Monto2 * i)
            
                StrSql = "INSERT INTO pre_cuota "
                StrSql = StrSql & "(prenro,cuoimp,cuocancela, cuonrocuo,cuogastos,cuoiva,cuototal,cuocapital,cuointeres,cuosaldo,cuomes,cuoano,cuofecvto) "
                StrSql = StrSql & "VALUES ("
                StrSql = StrSql & prenro
                StrSql = StrSql & "," & Monto2
                StrSql = StrSql & "," & "0"
                StrSql = StrSql & "," & i + Cuota1
                StrSql = StrSql & ",0"
                StrSql = StrSql & ",0"
                StrSql = StrSql & "," & Monto2
                StrSql = StrSql & ",0"
    ...
    Else
        ...
        'Ya existe --> Verifico las cuotas
        StrSql = " SELECT * "
        StrSql = StrSql & " FROM  pre_cuota "
        StrSql = StrSql & " WHERE  pre_cuota.prenro = " & rs_Prestamo!prenro
        OpenRecordset StrSql, rs_Cuotas
        If Not rs_Cuotas.EOF Then
            'No hago nada pues ya tiene las cuotas generadas
        Else
            'Le creo las cuotas
            prenro = rs_Prestamo!prenro
