...
'        'FGZ - 27/10/2006
'        ' No actualizo la Situacion de revista dado que se carga a mano
'        '=========================================================================================
...
Case "01":  'Fecha de Ingreso del empleado
        'primero verificar que no haya fase abierta
        ...
        If Not rs_Fases.EOF Then
            ...
            If EsNulo(rs_Fases!bajfec) Then
                Flog.writeline
                Flog.writeline Espacios(Tabulador * 3) & "--- No se puede crear una nueva fase porque existe una abierta."
                Flog.writeline
                Exit Sub
            End If
        End If
...
'                    Flog.writeline Espacios(Tabulador * 3) & "Linea " & NroLinea & ": No se puede insertar la novedad porque ya existe una sin vigencia"
                    ...
                    'Buscar la fecha de alta reconocida del legajo
                    StrSql = "SELECT * FROM fases "
                    StrSql = StrSql & " WHERE empleado = " & Empleado.Tercero
                    ...
                    OpenRecordset StrSql, rs_Fases
                    If Not rs_Fases.EOF Then
                        Aux_Fecha = rs_Fases!altfec
                    Else
                        'Buscar la fecha de alta del legajo
                        StrSql = "SELECT * FROM fases "
                        StrSql = StrSql & " WHERE empleado = " & Empleado.Tercero
                        StrSql = StrSql & " ORDER BY altfec "
                        OpenRecordset StrSql, rs_Fases
                        ...
                        Else
                            Flog.writeline Espacios(Tabulador * 3) & "Error. Infotipo no actualizado"
                            Flog.writeline Espacios(Tabulador * 3) & "Linea " & NroLinea & ": No se puede insertar la novedad porque ya existe una sin vigencia y el legajo no tiene fecha de alta"
                            Exit Sub
                        End If
                    End If
                    StrSql = "UPDATE novemp SET nevigencia = -1"
                    StrSql = StrSql & " ,nehasta = " & ConvFecha(FechaDesde - 1)
                    StrSql = StrSql & " ,nedesde = " & ConvFecha(Aux_Fecha)
...
Dim rs_lic_Vacacion As New ADODB.Recordset
    ...
    Else
        Emp_LicNro = rs_existeLic!Emp_LicNro
    End If
    
    'reviso si estan creados los complementos
    ...
    Case 2:    'Licencia por Vacaciones
        ...
        If Not rs_Periodos_Vac.EOF Then
            StrSql = "SELECT * FROM lic_vacacion "
            StrSql = StrSql & " WHERE emp_licnro = " & Emp_LicNro
            OpenRecordset StrSql, rs_lic_Vacacion
            If rs_lic_Vacacion.EOF Then
            ...
            'reviso si tiene los dias correspondientes asociados
            'FGZ - 25/09/2006 - Se inserta el pedido de vacaciones
            StrSql = "SELECT * FROM vacdiascor WHERE ternro = " & ternro & " AND vacnro = " & rs_Periodos_Vac!vacnro
            ...
            If Not rs_Vacdiascor.EOF Then
                ...
                If Not rs_Tipovacac.EOF Then
                    ...
                    'Call DiasPedidos(nroTipvac, FechaDesde, FechaHasta, Ternro, Diaspend, Tothabiles, TotNohabiles, Totferiados)
                    CantDias = DateDiff("d", FechaDesde, FechaHasta) + 1
                    Totferiados = cFeriados
                    Tothabiles = cHabiles
                    TotNohabiles = CantDias - (cHabiles + Totferiados)
                    
                    'Borro por las dudas que ya exista
                    StrSql = "DELETE FROM vacdiasped WHERE ternro = " & ternro & " AND vacnro = " & rs_Periodos_Vac!vacnro
                    StrSql = StrSql & " AND vdiapeddesde = " & ConvFecha(FechaDesde)
                    objConn.Execute StrSql, , adExecuteNoRecords
                    ...
                    StrSql = StrSql & "," & ternro
                    StrSql = StrSql & "," & rs_Periodos_Vac!vacnro
                    StrSql = StrSql & "," & (Totferiados + Tothabiles + TotNohabiles)
                    StrSql = StrSql & "," & ConvFecha(FechaDesde)
                    StrSql = StrSql & ",-1"
                    StrSql = StrSql & " )"
                    objConn.Execute StrSql, , adExecuteNoRecords
                Else
                    Flog.writeline Espacios(Tabulador * 3) & "No se puede insertar los dias pedidos porque no hay tipos de vacaciones cargados"
                End If
            Else
                Flog.writeline Espacios(Tabulador * 3) & "No se puede insertar los dias pedidos porque no tiene dias correspondientes generados para el periodo de vacaciones " & rs_Periodos_Vac!vacnro
                Flog.writeline Espacios(Tabulador * 3) & "Genere los dias correspondientes y vuelva a correr el infotipo."
            End If
        Else
...
Fin:
    ...
    If rs_Tipovacac.State = adStateOpen Then rs_Tipovacac.Close
    Set rs_Tipovacac = Nothing
    If rs_existeLic.State = adStateOpen Then rs_existeLic.Close
    Set rs_existeLic = Nothing
    If rs_Periodos_Vac.State = adStateOpen Then rs_Periodos_Vac.Close
    Set rs_Periodos_Vac = Nothing
    If rs_lic_Vacacion.State = adStateOpen Then rs_lic_Vacacion.Close
    Set rs_lic_Vacacion = Nothing
    Exit Sub
...
Private Sub DiasPedidos(ByVal TipoVac As Long, ByVal FechaInicial As Date, ByRef Fecha As Date, ByVal ternro As Long, ByRef cant As Integer, ByRef cHabiles As Integer, ByRef cNoHabiles As Integer, ByRef cFeriados As Integer)
...
Dim rs As New ADODB.Recordset
    ...
    Else
        ...
        If Not rs.EOF Then
            BuscarLegajo = rs!empleg
        Else
            StrSql = "SELECT max(empleg) as legajo FROM empleado "
            If rs.State = adStateOpen Then rs.Close
            OpenRecordset StrSql, rs
            If Not rs.EOF Then
                BuscarLegajo = IIf(Not EsNulo(rs!Legajo), rs!Legajo + 1, 1)
            Else
