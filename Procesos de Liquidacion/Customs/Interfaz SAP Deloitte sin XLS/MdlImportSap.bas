...
Private Function Es_InfotipoValido(ByVal Linea As String) As Boolean
' ---------------------------------------------------------------------------------------------
...
Lista = Split("IT0007;IT0008;IT0009;IT0014;IT0015;IT0016;IT0021;IT0185;IT0392;IT0394;IT2001;IT2002", ";")
...
Next I
If ABM Then
    If Not Primer_Linea And Formato_IT = 2 Then
        ABM = False
    End If
End If
Es_ABM = ABM
...
'--------------------
    ...
    If UBound(Lista) = 69 Then
        Flog.writeline Espacios(Tabulador * 2) & "Procesando linea " & strlinea
    Else
        If UBound(Lista) > 69 Then
            Flog.writeline Espacios(Tabulador * 2) & "Formato de linea incorrecto. Linea abortado. " & strlinea
        Else
            Flog.writeline Espacios(Tabulador * 2) & "Fin."
            Exit Sub
        End If
    End If
    ...
    If Columna <= UBound(Lista) Then
        ...
        If Not OK Then
            ...
            Flog.writeline Espacios(Tabulador * 3) & "Linea " & NroLinea & ":" & Texto & " inválido " & Trim(Lista(Columna))
...
'    '-----------------------------------------------------------------------------------
'    'Start date y end date corresponden al IT0392
'    'Start Date
'    Obligatorio = True
'    Columna = 52
'    Texto = "Start Date"
...
'            If Obligatorio Then Exit Sub
'        End If
'        If EsNulo(Inicio_Validez) Then
'            Flog.Writeline Espacios(Tabulador * 3) & "La fecha de inicio de validez no puede ser nula."
'            If Obligatorio Then
'                Flog.Writeline Espacios(Tabulador * 4) & "Legajo Abortado."
'                Exit Sub
'            End If
'        End If
'    Else
...
'    '-----------------------------------------------------------------------------------
    ...
    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
    'FGZ - 08/09/2006 - Si el CUIL no trae los guiones ==> completarlos
    If InStr(1, Cuil, "-") <= 0 Then
        'Completo los guiones
    ...
    Case "BAJA":
        ...
        Causa = CLng(CalcularMapeoInv(Motivo, "CAUSAB", "0"))
        OK = False
        If Causa <> 0 Then
    ...
    Obligatorio = True
    Columna = 4
    'FGZ - 08/09/2006 - Todo en Mayusculas
    Apellido = UCase(Trim(Lista(Columna)))
    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
    ...
    Columna = 5
    'FGZ - 08/09/2006 - Todo en Mayusculas
    Apellido2 = UCase(Trim(Lista(Columna)))
    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
    ...
    Columna = 6
    'FGZ - 08/09/2006 - Todo en Mayusculas
    nombre = UCase(Trim(Lista(Columna)))
    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
...
'    Reg_Tercero.Ternom2 = EliminarCHInvalidos(Trim(Format_Str(nombre2, 25, True, " ")))
    ...
    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
    'Default = Masculino
    If Sexo = "MASCULINO" Or Sexo = "MASCULIN" Then
    ...
    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
    If Not OK Then
        Flog.writeline Espacios(Tabulador * 3) & "Valor en Dato incorrecto. Linea " & NroLinea & ":" & Texto & " inválido " & Trim(Lista(Columna))
        Flog.writeline
        If Obligatorio Then Exit Sub
    End If
    ...
    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
    Reg_Tercero.NacionalNro = CLng(CalcularMapeoInv(Nacionalidad, "NACION", "0"))
...
'    End If
    ...
    If Not EsNulo(Region) Then
        Nro_Provincia = CLng(CalcularMapeoInv(Region, "PROVIN", "0"))
    Else
        Nro_Provincia = 0
    ...
    Else
        Nro_Localidad = 0
        Flog.writeline Espacios(Tabulador * 3) & "Valor Nulo. Estado"
        Flog.writeline
    End If
    Nro_Partido = 0
    ...
    If Not ExisteLegajo Then
        ...
        If Not EsNulo(Cuil) Then
            Call Insertar_Documento(Cuil, 10)
        End If
        MyCommitTrans
        ...
        Else
            Flog.writeline
            Flog.writeline Espacios(Tabulador * 3) & "Domicilio no insertado."
            Flog.writeline
        End If
    ...
    If Not EsNulo(TipoFechaFase) Then
        If Not EsNulo(Inicio_Validez) Then
            Call Insertar_Fecha(TipoFechaFase, CDate(Inicio_Validez), Inicio_Validez, Fin_Validez)
        Else
            Flog.writeline
            Flog.writeline Espacios(Tabulador * 3) & "---- Inicio de Validez con Valor Nulo, no se insertará la fase."
            Flog.writeline
        End If
    End If
...
'        End If
    Else
        ...
        Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
    End If
'FGZ - 20/09/2006 - se reemplazó por la columna 22
...
'        Flog.writeline
'    End If
'    'FGZ - 08/09/2006 - Desactivé esto ... este dato no lo voy a utilizar
'    Estructuras(Tenro).Tenro = Tenro
...
'    Estructuras(Tenro).Hasta = Fin_Validez
    ...
    Case Else
        ...
        Grupo_Liq = 624
        Actividad = 655
        Flog.writeline Espacios(Tabulador * 3) & "No se encontró el Sector/Grupo de Liquidación " & Area_de_Personal & ". Se cargarán defaults."
        Flog.writeline
    End Select
    'Sector
    Tenro = 2
    ...
    'FGZ - 20/09/2006 - se agregó esto
    Tenro = 29
    Estructuras(Tenro).Tenro = Tenro
    Estructuras(Tenro).Estrnro = Actividad
    Estructuras(Tenro).Desde = Inicio_Validez
    Estructuras(Tenro).Hasta = Fin_Validez
    ...
    Case "ARRO":
        Sucursal = 2
    Case Else
        Flog.writeline Espacios(Tabulador * 3) & "No se encontró la Sucursal " & Area_Funcional & ". Se cargará la Sucursal por default"
        Flog.writeline
    End Select
    'Sucursal
    Tenro = 1
    ...
    'replico en Lugar de pago
    Tenro = 20
    Tablaref = "LPAGO"
    Estructuras(Tenro).Tenro = Tenro
    Estructuras(Tenro).Estrnro = CLng(CalcularMapeoInv(Area_Funcional, Tablaref, "-1"))
    Estructuras(Tenro).Desde = Inicio_Validez
    Estructuras(Tenro).Hasta = Fin_Validez
    ...
    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
    Texto = "Subdivision de Personal - [ RHPro(Gerencia)] " & SubDivision_de_Personal
    Tablaref = "GERENC"
    Tenro = 6
    Gerencia = CLng(CalcularMapeoInv(SubDivision_de_Personal, Tablaref, "-1"))
    If Gerencia = -1 Then
        'Descripcion
        Obligatorio = True
        Columna = 23
        ...
        If Not EsNulo(Trim(Descripcion_Subdivision_Personal)) Then
            Call ValidaEstructura(Tenro, Descripcion_Subdivision_Personal, Nro_Estr, Inserto_estr)
            If Inserto_estr Then
                'Call CreaTercero(Tenro, Descripcion, Nro_Tercero)
                'Call CreaComplemento(Tenro, Nro_Tercero, Nro_Estr, Descripcion)
            End If
            Call Mapear(Tablaref, Descripcion_Subdivision_Personal, CStr(Nro_Estr))
            Gerencia = Nro_Estr
        Else
            Flog.writeline Espacios(Tabulador * 4) & "**** Descripción nula, no se puede insertar"
            Flog.writeline
...
'FGZ - 20/09/2006 - Esto se reemplazo por lo de arriba
'    Obligatorio = True
'    Columna = 22
'    SubDivision_de_Personal = Trim(Lista(Columna))
'    Call Insertar_Valor_Excel(Hoja, Fila_Infotipo, Columna, Lista(Columna))
...
''hasta la revision del 15/05/2006 estos campos traian la sucursal pero se combino que lo trae el campo 16 (Business Area)
    ...
    If Nro_Area_de_Nomina = -1 Then
        Flog.writeline Espacios(Tabulador * 3) & "Inexistente - No se encuentra " & Texto & ". No hay Descripción, no se puede insertar"
        Flog.writeline
    End If
    Tenro = 22
    Estructuras(Tenro).Tenro = Tenro
    ...
    Case Else
        Convenio = 391
        Sindicato = 335
        Flog.writeline Espacios(Tabulador * 3) & "No se encontró el Convenio/Sindicato " & Relacion_Laboral & ". Se cargarán defaults 01"
        Flog.writeline
    End Select
    'Convenio
    Tenro = 19
...
'    '*******************
'    'El puesto tambien se suspende hasta nuevo aviso
'    'Funcion - STELL
'    Obligatorio = True
'    Columna = 31
...
'
    ...
    'FGZ - 06/09/2006
    If UCase(Obra_Social) = "400909" Then
        Obra_Social = Obra_Social & Descripcion_OSocial
    End If
...
'FGZ - 20/09/2006 - se dejó de usar, ahora se saca de la columna 15
'    Tablaref = "ASIJP"
'    Tenro = 29
...
'    '*************************************************************
    ...
    Tablaref = "BANCOS"
    Tenro = 41
    'FGZ - 06/09/2006 - Esto es un caso especial
    If UCase(Clave_de_Banco) = "7" Or UCase(Clave_de_Banco) = "17" Then
        Clave_de_Banco = Clave_de_Banco & Destino_para_Transferencias
    End If
    
    Nro_Banco = CLng(CalcularMapeoInv(Clave_de_Banco, Tablaref, "0"))
    If Nro_Banco = 0 Then
        'Flog.writeline Espacios(Tabulador * 3) & "Inexistente - No se encuentra el Banco"
        'Flog.writeline
        ...
        If Not EsNulo(Trim(Banco)) Then
            ...
            If Inserto_estr Then
                Call CreaTercero(13, Format_Str(Banco, 40, False, ""), Nro_Ter_Banco)
                Call CreaComplemento(Tenro, Nro_Ter_Banco, Nro_Estr, Banco)
            End If
    ...
    Else
        ...
        'buscar el tercero del banco
        StrSql = "select ternro from banco where estrnro = " & Nro_Banco
        OpenRecordset StrSql, rs_Banco
        If Not rs_Banco.EOF Then
            Nro_Ter_Banco = rs_Banco!ternro
        Else
            Nro_Ter_Banco = 0
        End If
    ...
    'FGZ - 08/09/2006 - Replico la estructura banco
    Estructuras(Tenro).Tenro = Tenro
    Estructuras(Tenro).Estrnro = Nro_Banco
    Estructuras(Tenro).Desde = Inicio_Validez
    Estructuras(Tenro).Hasta = Fin_Validez
    ...
    'FGZ - 08/09/2006 - La forma de pago sale de Via_de_pago + Clave_Control_Banco
    Via_de_Pago = Via_de_Pago & Clave_Control_Banco
    Nro_FormaPago = CLng(CalcularMapeoInv(Via_de_Pago, "FORMAP", "0"))
    If Nro_FormaPago = 0 Then
        Flog.writeline Espacios(Tabulador * 4) & "**** Descripción nula, no se puede insertar"
    End If
    'Debug.Print "control: " & Clave_Control_Banco & ", via: " & Via_de_Pago & ", Provincia: " & Region & ", ASIJP: " & Actividad_SIJP & ", CSIJP: " & Grupo_SIJP
...
If rs_caja.State = adStateOpen Then rs_caja.Close
Set rs_caja = Nothing
Exit Sub
...
Dim Hoja As Integer
    ...
    If Not Es_InfotipoValido(strlinea) Then
...
Dim Fecha_V_Desde As String
Dim Fecha_V_Hasta As String
...
Dim Nro_Identific As String
Dim Nro_Tipo_ID As String
Dim SalarioFam As Boolean
Dim Aux_SalarioFam As String

Dim rs_Tercero As New ADODB.Recordset

    
    Fecha_V_Desde = StrToDate(Arr_Datos(1).Valor, OK, FormatoFechaSap1, NuloFechaSap1)
    If Not OK Then
        Flog.writeline Espacios(Tabulador * 2) & "Error. Infotipo no actualizado"
        Flog.writeline Espacios(Tabulador * 3) & "FGBDT. Formato de fecha invalido"
    End If
    Fecha_V_Hasta = StrToDate(Arr_Datos(2).Valor, OK, FormatoFechaSap1, NuloFechaSap1)
    If Not OK Then
        Flog.writeline Espacios(Tabulador * 2) & "Error. Infotipo no actualizado"
        Flog.writeline Espacios(Tabulador * 3) & "FGBDT. Formato de fecha invalido"
    End If
    ...
    Identificacion_de_objeto = IIf(Not EsNulo(Trim(Arr_Datos(5).Valor)), Trim(Arr_Datos(5).Valor), 0)
    ...
    Aux_SalarioFam = IIf(Not EsNulo(Trim(Arr_Datos(12).Valor)), Trim(Arr_Datos(12).Valor), 0)
    If Aux_SalarioFam = "0" Then
        SalarioFam = False
    Else
        SalarioFam = True
    End If
    ...
    If rs_Tercero.EOF Then
        ...
        StrSql = "INSERT INTO familiar (empleado,ternro,parenro,famnrocorr,famest,famtrab,famestudia,famcernac,faminc,famsalario"
        If SalarioFam And Not EsNulo(Fecha_V_Hasta) Then
        ...
        StrSql = StrSql & "," & CInt(SalarioFam)
        If SalarioFam And Not EsNulo(Fecha_V_Hasta) Then
            StrSql = StrSql & "," & ConvFecha(Fecha_V_Hasta)
        End If
        StrSql = StrSql & " )"
        objConn.Execute StrSql, , adExecuteNoRecords
        ...
        If Not EsNulo(Arr_Datos(10).Valor) Then
            'Nro_Tipo_ID = Trim(Arr_Datos(10).Valor)
            Nro_Tipo_ID = CLng(CalcularMapeoInv(Trim(Arr_Datos(10).Valor), "TIPDOC", "0"))
            If Not EsNulo(Arr_Datos(11).Valor) Then
                Nro_Identific = Trim(Arr_Datos(11).Valor)
    ...
    Else
        ...
        StrSql = StrSql & " ,famsalario = " & CInt(SalarioFam)
        If SalarioFam And Not EsNulo(Fecha_V_Hasta) Then
            StrSql = StrSql & " ,famfecvto = " & ConvFecha(Fecha_V_Hasta)
        End If
        ...
        If Not EsNulo(Arr_Datos(10).Valor) Then
            ...
            If Not EsNulo(Arr_Datos(11).Valor) Then
                Nro_Identific = Trim(Arr_Datos(11).Valor)
                Call Insertar_Documento_Familiar(rs_Tercero!ternro, Nro_Identific, Nro_Tipo_ID)
            Else
                Flog.writeline Espacios(Tabulador * 2) & "Infotipo " & UltimoInfotipo & " no insertado. Nro de Doc con valor nulo."
            End If
        Else
...
Public Sub Insertar_Horario()
' ---------------------------------------------------------------------------------------------
' Descripcion: Procedimiento que inserta el Regimen Horario del empleado
' Autor      : FGZ
' Fecha      : 22/03/2006
' Ultima Mod.:
' Descripcion:
...
Dim Nro_Tercero As Long
    ...
    If Not OK Then
        ...
        Flog.writeline Espacios(Tabulador * 3) & "FGBDT. Formato de fecha invalido"
    End If
    Plan_Horario_Trabajo = Trim(Arr_Datos(3).Valor)
    Descripcion_Horario_Trabajo = Trim(Arr_Datos(4).Valor)
    Porcentaje_Horario_Trabajo = Trim(Arr_Datos(5).Valor)

    Regimen_Horario = CLng(CalcularMapeoInv(Plan_Horario_Trabajo, "HORAT", "-1"))
    If Regimen_Horario = -1 Then
        Flog.writeline Espacios(Tabulador * 3) & "Regimen Horario Inexistente"
        Flog.writeline
        
        If Not EsNulo(Trim(Descripcion_Horario_Trabajo)) Then
            Call ValidaEstructura(21, Descripcion_Horario_Trabajo, Nro_Estr, Inserto_estr)
            If Inserto_estr Then
    '            Call CreaTercero(Tenro, Descripcion, Nro_Tercero)
    '            Call CreaComplemento(Tenro, Nro_Tercero, Nro_Estr, Descripcion)
            End If
            Call Mapear("HORAT", Plan_Horario_Trabajo, CStr(Nro_Estr))
            Regimen_Horario = Nro_Estr
        Else
            Flog.writeline Espacios(Tabulador * 4) & "**** Descripción nula, no se puede insertar"
        End If
    ...
    If Regimen_Horario <> 0 And Regimen_Horario <> -1 Then
        Call Insertar_His_Estructura(21, Regimen_Horario, Empleado.Tercero, Fecha_V_Desde, Fecha_V_Hasta)
    End If
    
End Sub

Public Sub Insertar_Contrato()
' ---------------------------------------------------------------------------------------------
' Descripcion: Procedimiento que inserta el Contrato del empleado
' Autor      : FGZ
' Fecha      : 22/03/2006
' Ultima Mod.:
' Descripcion:
...
Dim Nro_Tercero As Long
    ...
    Contrato = Trim(Arr_Datos(3).Valor)
    Descripcion_Contrato = Trim(Arr_Datos(4).Valor)
    'Fecha_Fin_Contrato = StrToDate(Trim(Arr_Datos(5).Valor), OK, FormatoFechaSap1, NuloFechaSap1)
    'If Not OK Then
    '    Flog.writeline Espacios(Tabulador * 2) & "Error. Infotipo no actualizado"
    '    Flog.writeline Espacios(Tabulador * 3) & "Fin de contrato. Formato de fecha invalido"
    'End If
    Periodo_Prueba = Trim(Arr_Datos(6).Valor)
    Periodo_Prueba_UN = Trim(Arr_Datos(7).Valor)
  
    Nro_Contrato = CLng(CalcularMapeoInv(Contrato, "CONTRA", "-1"))
    If Nro_Contrato = -1 Then
        Flog.writeline Espacios(Tabulador * 3) & "Contrato Inexistente"
        Flog.writeline
        
        If Not EsNulo(Trim(Descripcion_Contrato)) Then
            Call ValidaEstructura(18, Descripcion_Contrato, Nro_Estr, Inserto_estr)
            If Inserto_estr Then
    '            Call CreaTercero(Tenro, Descripcion, Nro_Tercero)
    '            Call CreaComplemento(Tenro, Nro_Tercero, Nro_Estr, Descripcion)
            End If
            Call Mapear("CONTRA", Contrato, CStr(Nro_Estr))
            Nro_Contrato = Nro_Estr
        Else
            Flog.writeline Espacios(Tabulador * 4) & "**** Descripción nula, no se puede insertar"
        End If
    ...
    If Nro_Contrato <> 0 And Nro_Contrato <> -1 Then
        'Call Insertar_His_Estructura(18, Nro_Contrato, Empleado.Tercero, Fecha_V_Desde, Fecha_Fin_Contrato)
        Call Insertar_His_Estructura(18, Nro_Contrato, Empleado.Tercero, Fecha_V_Desde, Fecha_V_Hasta)
    End If
...
Public Sub Insertar_CtaBancaria()
' ---------------------------------------------------------------------------------------------
' Descripcion: Procedimiento que inserta la Cta Bancaria del empleado
' Autor      : FGZ
' Fecha      : 22/03/2006
' Ultima Mod.:
' Descripcion:
...
Dim Banco As String
Dim rs_Cta As New ADODB.Recordset
Dim rs_Cta2 As New ADODB.Recordset
Dim rs_Banco As New ADODB.Recordset
    ...
    Clave_de_Banco = Trim(Arr_Datos(5).Valor)
    Banco = Trim(Arr_Datos(6).Valor)
    Cuenta_Bancaria = Trim(Arr_Datos(7).Valor)
    Clave_Control_Banco = Trim(Arr_Datos(8).Valor)
    Via_de_Pago = Trim(Arr_Datos(9).Valor)
    Banco_Sucursal = Trim(Left(Arr_Datos(10).Valor, 40))
    
    'FGZ - 06/09/2006
    'Esto es un caso especial
    If UCase(Clave_de_Banco) = "7" Or UCase(Clave_de_Banco) = "17" Then
        Clave_de_Banco = Clave_de_Banco & Banco_Sucursal
    End If
    
    Nro_Banco = CLng(CalcularMapeoInv(Clave_de_Banco, "BANCOS", "0"))
    If Nro_Banco = 0 Then
        Flog.writeline Espacios(Tabulador * 3) & "Clave de Banco Inexistente " & Texto
        Flog.writeline
        
        If Not EsNulo(Trim(Banco)) Then
            Call ValidaEstructura(41, Banco, Nro_Estr, Inserto_estr)
            If Inserto_estr Then
                Call CreaTercero(13, Format_Str(Banco, 40, False, ""), Nro_Ter_Banco)
                Call CreaComplemento(41, Nro_Ter_Banco, Nro_Estr, Banco)
            End If
            Call Mapear("BANCOS", Clave_de_Banco, CStr(Nro_Estr))
            Nro_Banco = Nro_Estr
        Else
            Flog.writeline Espacios(Tabulador * 4) & "**** Descripción de Banco nula, no se puede insertar"
        End If
    Else
        'buscar el tercero del banco
        StrSql = "select ternro from banco where estrnro = " & Nro_Banco
        ...
        Else
            If rs_Cta!ctabnro = Cuenta_Bancaria Then
                ...
                If Not EsNulo(CBU) Then
                    ...
                    End If
                End If
                If Nro_Ter_Banco <> 0 Then
                    StrSql = StrSql & " ,banco = " & Nro_Ter_Banco
                End If
                StrSql = StrSql & " WHERE cbnro = " & rs_Cta!cbnro
                objConn.Execute StrSql, , adExecuteNoRecords
            Else
                'Si esa cuenta existe y esta inactiva se descarta
                StrSql = "SELECT * FROM ctabancaria "
                StrSql = StrSql & " WHERE ternro = " & Empleado.Tercero
                StrSql = StrSql & " AND ctabnro = " & Cuenta_Bancaria
                StrSql = StrSql & " AND ctabestado = 0 "
                OpenRecordset StrSql, rs_Cta2
                If Not rs_Cta2.EOF Then
                    'la cuenta existe,
                    'Descarto porque se supone que esta es una cuenta vieja
                Else
                    'Desactivo la anterior
                    StrSql = " UPDATE ctabancaria SET "
                    StrSql = StrSql & " ctabestado = 0 "
    ...
    'FGZ - 08/09/2006 - Replico en la estructura banco
    If Nro_Banco <> 0 And Nro_Banco <> -1 Then
        Call Insertar_His_Estructura(41, Nro_Banco, Empleado.Tercero, Fecha_V_Desde, Fecha_V_Hasta)
    End If


If rs_Cta.State = adStateOpen Then rs_Cta.Close
If rs_Cta2.State = adStateOpen Then rs_Cta2.Close
If rs_Banco.State = adStateOpen Then rs_Banco.Close
...
Select Case Infotipo
    ...
    Case "IT2002":
        CantidadLineas = 5
    Case Else
        Flog.writeline Espacios(Tabulador * 2) & "Infotipo " & UltimoInfotipo & " incorrecto. Se descarta."
        CantidadLineas = 1
End Select
