Attribute VB_Name = "ExpRenatreSt"
Option Explicit

'Global Const Version = "1.01"
'Global Const FechaModificacion = "27/05/2010" 'Elizabeth Gisela Oviedo
'Global Const UltimaModificacion = "" 'Version Inicial
                                     
'Global Const Version = "1.02"
'Global Const FechaModificacion = "02/06/2010" 'Elizabeth Gisela Oviedo
'Global Const UltimaModificacion = "" 'Se eliminan los filtros de empleados activos en fases
                                     
'Global Const Version = "1.03"
'Global Const FechaModificacion = "17/06/2010" 'FGZ
'Global Const UltimaModificacion = "" 'Se agregaron detalles de log
                                     
'Global Const Version = "1.04"
'Global Const FechaModificacion = "17/06/2010" 'EGO - Elizabeth Gisela Oviedo
'Global Const UltimaModificacion = "" 'Se genera el archivo con el nombre y numero de proceso
                                     
'Global Const Version = "1.05"
'Global Const FechaModificacion = "28/09/2010" 'JPB - Brzozowski Juan Pablo
'Global Const UltimaModificacion = "" 'Se modifico la Fecha de Ingreso de la exportacion.
                                     
Global Const Version = "1.06"
Global Const FechaModificacion = "07/01/2011" 'Matias Dallegro
Global Const UltimaModificacion = "" 'Se modifico la Fecha limite para que tome la fecha del ultimo periodo a considerar
...
Dim nroModelo As Integer
    ...
    StrSql = StrSql & " AND btprcnro = 268"
...
Dim rs2 As New ADODB.Recordset
    ...
    Nombre_Arch = Directorio & "\renatre-" & NroProceso & ".csv"
    ...
    'Estructura fuera de convenio
    StrSql = "SELECT confval FROM confrep "
    StrSql = StrSql & " WHERE repnro = " & NroReporte
    StrSql = StrSql & " AND conftipo= 'FCO'"
    If rs.State = adStateOpen Then rs.Close
    OpenRecordset StrSql, rs
    If rs.EOF Then
        Flog.writeline "No se encontró la configuración fuera de convenio del Reporte"
        Exit Sub
    Else
    ...
    StrSql = "SELECT confval FROM confrep "
    StrSql = StrSql & " WHERE repnro = " & NroReporte
    StrSql = StrSql & " AND conftipo= 'CAT'"
    If rs.State = adStateOpen Then rs.Close
    OpenRecordset StrSql, rs
    If rs.EOF Then
        Flog.writeline "No se encontró la configuración de categoria del Reporte"
        Exit Sub
    Else
        conf_categoria = rs!confval
    End If
    ...
    OpenRecordset StrSql, rs
    If rs.EOF Then
        Flog.writeline "No se encontró la configuración de contrato del Reporte"
        Exit Sub
    Else
        conf_contrato = rs!confval
    End If
    
    'Acumulador Remuneracion Imponible
    conf_acum_rem = 0
    ...
    per3 = Format(per_mes_3, "00") & "/" & Format(per_anio_3, "0000")
         ...
         '--------------------------------------------------------------------------------
               'Busco La fecha del tercer periodo
          
        '---------------------------------------------------------------------------------
               
               StrSql = " SELECT  convert(varchar(100),pliqhasta,103) fechalimite "
               StrSql = StrSql & " FROM  periodo p "
               StrSql = StrSql & " WHERE p.pliqmes = " & per_mes_3
               StrSql = StrSql & " and p.plianio =" & per_anio_3
    ...
    'fecha_limite = DateAdd("d", 90, fecha_alta)
               ...
               If Not rs.EOF Then
                ...
                If Not IsNull(rs!fechalimite) Then
                 
                   fecha_limite = rs!fechalimite
                 
                 End If
               End If
        rs.Close
        Flog.writeline Espacios(Tabulador * 1) & " Fecha Limite tercer periodo " & fecha_limite
    '------------------------------------------------------------------
    'Busco los datos
    '------------------------------------------------------------------
         Flog.writeline Espacios(Tabulador * 1) & " Buscando empleado del proceso entre " & fecha_alta & " al " & fecha_limite
    ...
    'StrSql = StrSql & " OR empresa.htethasta IS NULL) AND empresa.estrnro = " & Empnro
     StrSql = StrSql & " AND empresa.tenro = 10 AND empresa.htetdesde <= " & ConvFecha(fecha_limite)
     StrSql = StrSql & " AND (empresa.htethasta >= " & ConvFecha(fecha_limite)
     StrSql = StrSql & " OR empresa.htethasta IS NULL) AND empresa.estrnro = " & Empnro
    If tipoestr <> 0 Or estructura <> 0 Then
       ...
       ' StrSql = StrSql & " AND his_estructura.htetdesde <= " & ConvFecha(fecha_alta)
        ...
        StrSql = StrSql & " AND his_estructura.htetdesde <= " & ConvFecha(fecha_limite)
        StrSql = StrSql & " AND (his_estructura.htethasta >= " & ConvFecha(fecha_limite)
        StrSql = StrSql & "      OR his_estructura.htethasta IS NULL) "
    ...
    Do Until rs.EOF
       ...
       ' StrSql = StrSql & " AND (    ( altfec <= " & ConvFecha(fecha_limite) & " AND  bajfec IS NULL   )"
       ' StrSql = StrSql & "       OR ( bajfec IS NOT NULL AND altfec <= " & ConvFecha(fecha_limite) & " AND  bajfec >= " & ConvFecha(fecha_limite) & " )  "
       ' StrSql = StrSql & "       OR ( bajfec IS NOT NULL AND altfec <= " & ConvFecha(fecha_limite) & " AND  bajfec < " & ConvFecha(fecha_limite) & " AND  bajfec >= " & ConvFecha(fecha_alta) & "  ) "
        'StrSql = StrSql & "      )"
          
        StrSql = StrSql & " AND (    ( altfec <= " & ConvFecha(fecha_limite) & " AND  bajfec IS NULL   )"
        StrSql = StrSql & "       OR ( bajfec IS NOT NULL AND altfec <= " & ConvFecha(fecha_limite) & " AND  bajfec >= " & ConvFecha(fecha_limite) & " )  "
        StrSql = StrSql & "       OR ( bajfec IS NOT NULL AND altfec <= " & ConvFecha(fecha_limite) & " AND  bajfec < " & ConvFecha(fecha_limite) & " AND  bajfec >= " & ConvFecha(fecha_limite) & "  ) "
        StrSql = StrSql & "      )"
        ...
        StrSql = StrSql & " WHERE his_estructura.ternro = " & rs!Ternro & " AND his_estructura.tenro = " & conf_categoria
       ' StrSql = StrSql & " AND his_estructura.htetdesde <= " & ConvFecha(fecha_alta)
       ' StrSql = StrSql & " AND (his_estructura.htethasta >= " & ConvFecha(fecha_alta) & " OR his_estructura.htethasta IS NULL)"
        StrSql = StrSql & " AND his_estructura.htetdesde <= " & ConvFecha(fecha_limite)
       StrSql = StrSql & " AND (his_estructura.htethasta >= " & ConvFecha(fecha_limite) & " OR his_estructura.htethasta IS NULL)"
        ...
        StrSql = StrSql & " FROM his_estructura "
        StrSql = StrSql & " LEFT JOIN estr_cod ON his_estructura.estrnro = estr_cod.estrnro AND estr_cod.tcodnro = " & tipCodRenatre
        StrSql = StrSql & " WHERE his_estructura.ternro = " & rs!Ternro & " AND his_estructura.tenro = " & conf_contrato
        'StrSql = StrSql & " AND his_estructura.htetdesde <= " & ConvFecha(fecha_alta)
        'StrSql = StrSql & " AND (his_estructura.htethasta >= " & ConvFecha(fecha_alta) & " OR his_estructura.htethasta IS NULL)"
        StrSql = StrSql & " AND his_estructura.htetdesde <= " & ConvFecha(fecha_limite)
        StrSql = StrSql & " AND (his_estructura.htethasta >= " & ConvFecha(fecha_limite) & " OR his_estructura.htethasta IS NULL)"
...
'        per3 = Format(per_mes_3, "00") & "/" & Format(per_anio_3, "0000")
        ...
        If Not rs2.EOF Then
            ...
            Else
                Flog.writeline Espacios(Tabulador * 1) & "Acumulador de Dias " & conf_acum_dias & " con valor nulo."
            End If
        Else
            Flog.writeline Espacios(Tabulador * 1) & "No se encuentra Acumulador de Dias " & conf_acum_dias & " para el periodo."
