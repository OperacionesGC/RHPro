...
Do While Not rs_Empleados.EOF
    ...
    StrSql = "UPDATE batch_proceso SET bprcprogreso = " & Replace(CStr(Progreso), ",", ".") & " WHERE bpronro = " & NroProcesoBatch
...
If Valido Then
    MyBeginTrans
        ...
        'Estr.   : PSPAR
        'Campo   : PERNR
        'Descrip.: Numero de Personal - [ Heith (Legajo del empleado)]
        ...
        'Estr.   : PSPAR
        'Campo   : PERSG
        'Descrip.: Grupo de Personal - [ Heith (Tipo de Contrato)]
        '          Mapeo tabla T501P
        Salida = Salida & Separador & busq_GrupoDePersonal(Tercero)
        
        'Estr.   : PSPAR
        'Campo   : PERSK
        'Descrip.: Area de Personal - [ Heith (Convenio)]
        '          Mapeo tabla T503K
        ...
        '          Mapeo Tabla T005
        Salida = Salida & Separador & calcularMapeo(rs_Tercero!paisnro, "T005", "AR")

        'Estr.   : Q0002
        'Campo   : FATXT
        'Descrip.: Estado Civil
        '          Mapeo Tabla T502T
        Salida = Salida & Separador & calcularMapeo(rs_Tercero!estcivnro, "T502T", "0")
...
Function calcularMapeoInv(ByVal Parametro, ByVal Tabla, ByVal Default)
...
Function busq_ZonaGeograficaAFIP(ByVal Tercero)
    ...
    If Sucursal <> "" Then
        ...
        If Not rs_Consult.EOF Then
            Salida = calcularMapeo(rs_Consult!zonacod, "T500P", "E401")
        Else
...
If Valido Then
    MyBeginTrans
        ...
        Do Until rs_Familiares.EOF
            ...
            If IsNull(rs_Familiares!terfecnac) Then
...
If Valido Then
    MyBeginTrans
        ...
        'Busco el tipo de convenio y Mapeo
        StrSql = " SELECT tipocont.tcanios,his_estructura.estrnro,tipocont.tcind FROM his_estructura " & _
        " INNER JOIN replica_estr ON his_estructura.estrnro = replica_estr.estrnro" & _
        " INNER JOIN tipocont ON replica_estr.origen = tipocont.tcnro " & _
        " WHERE his_estructura.tenro = " & tipoEstructura & _
        " AND his_estructura.ternro = " & Tercero & _
        " AND his_estructura.htetdesde <=" & ConvFecha(Fecha_Hasta) & _
        " AND (his_estructura.htethasta >= " & ConvFecha(Fecha_Desde) & _
...
Dim rs_Consult3 As New ADODB.Recordset
...
If Valido Then
    MyBeginTrans
        ...
        'Controlo si esta liquidado el concepto A Cta. fut. Aument., concepto 1110
        ...
        StrSql = StrSql & " Where detliq.concnro = 38 "
        
        OpenRecordset StrSql, rs_Consult3
        
        If Not rs_Consult3.EOF Then
        
            'Busco el valor del concepto Sueldo Basico, concepto 1100
            StrSql = " SELECT sum(detliq.dlimonto) AS monto, sum(detliq.dlicant) AS Cantidad "
            StrSql = StrSql & " From cabliq"
            StrSql = StrSql & " INNER JOIN detliq ON cabliq.cliqnro = detliq.cliqnro "
            StrSql = StrSql & " AND cabliq.empleado = " & Tercero & " AND cabliq.pronro IN (" & Procesos & ")"
            StrSql = StrSql & " Where detliq.concnro = 12 "
            ...
            If Not rs_Consult2.EOF Then
               If Not IsNull(rs_Consult2!Monto) And Not IsNull(rs_Consult2!Cantidad) Then
                  ANSAL = ANSAL + CDbl(rs_Consult2!Monto)
               End If
            End If
            
            'Busco el valor del concepto A Cta. fut. Aument., concepto 1110
            StrSql = " SELECT sum(detliq.dlimonto) AS monto, sum(detliq.dlicant) AS Cantidad "
            StrSql = StrSql & " From cabliq"
            StrSql = StrSql & " INNER JOIN detliq ON cabliq.cliqnro = detliq.cliqnro "
            StrSql = StrSql & " AND cabliq.empleado = " & Tercero & " AND cabliq.pronro IN (" & Procesos & ")"
        ...
        StrSql = " SELECT * FROM infotipos_mapeo WHERE tablaref='T539A' ORDER BY codexterno "
        
        OpenRecordset StrSql, rs_Conceptos
        
        If Not rs_Conceptos.EOF Then
           Anterior = rs_Conceptos!codexterno
        End If
        ...
        Do Until rs_Conceptos.EOF
            ...
            If Not rs_Consult.EOF Then
                ...
                If Not rs_Consult2.EOF Then
                   If Not IsNull(rs_Consult2!Monto) And Not IsNull(rs_Consult2!Cantidad) Then
                      Total = Total + CDbl(rs_Consult2!Monto)
                      Cantidad = Cantidad + CDbl(rs_Consult2!Cantidad)
                   End If
                End If
            ...
            If rs_Conceptos.EOF Then
               Mostrar = True
            Else
               Mostrar = rs_Conceptos!codexterno <> Anterior
            End If
            
            If Mostrar Then
            
                Mostrar = False
                TipoEmp = busq_Estructura(Tercero, 19, "T503K", "PM")
                
                Select Case TipoEmp
                
                   Case "FN"
                      Mostrar = (Anterior = "1F00")
                   
                   Case "NS"
                      Mostrar = (Anterior = "1F10") Or (Anterior = "1F15")
                   
                   Case "PM"
                      Mostrar = (Anterior = "1F30")
                ...
                If Mostrar Then
                     ...
                     'Campo   : LGART
                     'Descrip.: CC-Nomina
                     Salida = Salida & Separador & Anterior
                     
                     'Estr.   : Q0008
                     'Campo   : BETRG
                     'Descrip.: Importe
                     Salida = Salida & Separador & Total
                 
                     'Estr.   : Q0008
                     'Campo   : ANZHL
                     'Descrip.: Numero
                     Salida = Salida & Separador & Cantidad
...
If Valido Then
    MyBeginTrans
        ...
        '-----------------------------------------------------------------------
        'Genero la salida para los dias pendientes de goce
        ...
        '-----------------------------------------------------------------------
        'Genero la salida para los dias pendientes de pago
        ...
        'Busco el valor del concepto
        StrSql = " SELECT sum(dlicant) AS total "
        StrSql = StrSql & " FROM cabliq"
        StrSql = StrSql & " INNER JOIN detliq ON detliq.cliqnro = cabliq.cliqnro"
        StrSql = StrSql & " WHERE detliq.concnro = 325 "
        ...
        If Not rs_Consult.EOF Then
           If Not IsNull(rs_Consult!Total) Then
              diasPagados = CDbl(rs_Consult!Total)
           End If
        End If
        
        rs_Consult.Close

        Salida = Salida & Separador & (diasCorCant - diasPagados)
        
        'Estr.   : P2006
        'Campo   : DESTA
        'Descrip.: Fecha de inicio de la liquidacion
        'Busco el valor del concepto
        StrSql = " SELECT periodo.* "
        StrSql = StrSql & " FROM cabliq"
        ...
        StrSql = StrSql & " WHERE detliq.concnro = 325 "
        StrSql = StrSql & "         AND empleado = " & Tercero
        StrSql = StrSql & " ORDER BY periodo.pliqdesde ASC "
        
        OpenRecordset StrSql, rs_Consult
                
        If Not rs_Consult.EOF Then
           Salida = Salida & Separador & Format_Fecha(rs_Consult!pliqdesde, 1)
        Else
           Salida = Salida & Separador & Space(8)
        End If
        ...
        StrSql = StrSql & " WHERE detliq.concnro = 325 "
        StrSql = StrSql & "         AND empleado = " & Tercero
        StrSql = StrSql & " ORDER BY periodo.pliqdesde DESC "
        
        OpenRecordset StrSql, rs_Consult
                
        If Not rs_Consult.EOF Then
           Salida = Salida & Separador & Format_Fecha(rs_Consult!pliqhasta, 1)
        Else
           Salida = Salida & Separador & Space(8)
        End If
    ...
    MyBeginTrans
        ...
        Do Until rs_Items.EOF
            ...
            If Not rs_Consult.EOF Then
                ...
                If CInt(rs_Items!codinterno) = 10 Or CInt(rs_Items!codinterno) = 11 Or CInt(rs_Items!codinterno) = 12 Then
...
If Valido Then
    MyBeginTrans
        ...
        If CInt(busq_GrupoDePersonal(Tercero)) <> 9 Then
            ...
            tipoEstructura = 17
            StrSql = "SELECT estructura.* FROM his_estructura " & _
            " INNER JOIN estructura ON his_estructura.estrnro = estructura.estrnro " & _
            " WHERE his_estructura.tenro = " & tipoEstructura & _
            " AND his_estructura.ternro = " & Tercero & _
            " AND his_estructura.htetdesde <=" & ConvFecha(Fecha_Hasta) & _
            ...
            ObraSocialEleg = 0
            If Not rs_Consult.EOF Then
                ObraSocialEleg = CInt(rs_Consult!estrnro)
            End If
            ...
            If Not rs_Consult.EOF Then
                If (ObraSocialEleg = 573) Or (ObraSocialEleg = 567) Or (ObraSocialEleg = 570) Then
                   PlanOS = Mid(rs_Consult!estrdabr, 1, 10)
                End If
            End If
            
            rs_Consult.Close
            
            Salida = Salida & Separador & PlanOS
            ...
            For II = 1 To 12
                ...
                If Not rs_Consult.EOF Then
                    ...
                    If (II = 1) Or (II = 3) Or (II = 5) Or (II = 7) Or (II = 8) Or (II = 10) Or (II = 12) Then
                      Fecha1 = Format_Fecha("31/" & II & "/2004", 1)
                    Else
                      If (II = 4) Or (II = 6) Or (II = 9) Or (II = 11) Then
                         Fecha1 = Format_Fecha("30/" & II & "/2004", 1)
                      Else
                         Fecha1 = Format_Fecha("29/" & II & "/2004", 1)
                      End If
                    End If
                    Salida = Salida & Separador & Fecha1
                    Salida = Salida & Separador & Fecha1
