...
Dim v_Fecha_Antiguedad As Date
Dim v_Ingreso_Grupo As Date
Dim v_Fecha_Promo As Date
Dim v_Fecha_Baja As Date
                    ...
                    Else
                        ...
                        If Not objrS1.EOF Then
                            ...
                            If sin_error Then
                                ...
                                v_fech_nac = ConvFecha(objrS1!terfecnac)
                            ...
                            If Not IsNull(objrS1!empfaltagr) And objrS1!empfaltagr <> "" Then
                                v_fecha_ingreso = ConvFecha(objrS1!empfaltagr)
                            ...
                            End If
                            If Not EsNulo(objrS1!nivnro) And objrS1!nivnro <> "0" Then
                            ...
                            If Not objrS1.EOF Then
                                ...
                                Select Case CInt(objrS1!tenro)
                                    ...
                                    Case C_horas_mesh:
                                        ...
                                        If sin_error Then
                                            Call InsertarItemLegajoLiquidar("HORAS_MESH", v_legajo, "", Empty, Empty, Empty, "", "", "", "", "", "", "", v_horas_mesh, 0, "", "", "", "", "", "", Empty, "", "", "", "", "", "", Empty, "", sin_error, objAuditoria!audnro, objAuditoria!aud_ternro)
                                        End If
                                    Case C_horas_mesd:
                        ...
                        If Not objrS1.EOF Then
                            ...
                            If sin_error Then
                                ...
                                If sin_error Then
                                    ...
                                    If sin_error Then
                                        v_fech_ini_vinc = ConvFecha(objrS1!famfec)
                    ...
                    Else
                        ...
                        If Not objrS1.EOF Then
                            ...
                            If CBool(objrS1!fasrecofec) Then
                                v_Fecha_Antiguedad = ConvFecha(objrS1!altfec)
                            End If
                            If CBool(objrS1!Real) Then
                                v_Ingreso_Grupo = ConvFecha(objrS1!altfec)
                            End If
                            If CBool(objrS1!vacaciones) Then
                                v_Fecha_Promo = ConvFecha(objrS1!altfec)
                            End If
                            Call InsertarItemLegajoLiquidar("", v_legajo, "", Empty, v_Fecha_Antiguedad, v_Ingreso_Grupo, "", "", "", "", "", "", "", 0, 0, "", "", "", "", "", "", Empty, "", "", "", "", "", "", v_Fecha_Promo, "", sin_error, objAuditoria!audnro, objAuditoria!aud_ternro)
                            If Not EsNulo(objrS1!bajfec) Then
                                ...
                                If Not objrS1.EOF Then
                                   If Not EsNulo(objrS1!bajfec) Then
                                        v_Fecha_Baja = ConvFecha(objrS1!bajfec)
                            ...
                            Else
                                ...
                                If Not objrS1.EOF Then
                                    ...
                                    If Not EsNulo(objrS1!bajfec) Then
                                        v_Fecha_Baja = ConvFecha(objrS1!bajfec)
                                        v_Motivo_Baja = Mapeo(v_legajo, "LEGAJO_LIQUIDAR", "MOTIVO_BAJA", objAuditoria!audnro, "[MOT_BAJA]", objrS1!caunro, sin_error) ' Hacer mapeo
                                        Call InsertarItemLegajoLiquidar("FECHA_BAJA", v_legajo, "", Empty, Empty, Empty, "", "", "", "", "", "", "", 0, 0, "", "", "", "", "", "", v_Fecha_Baja, v_Motivo_Baja, "", "", "", "", "", Empty, "", sin_error, objAuditoria!audnro, objAuditoria!aud_ternro)
                                    End If
                                Else
                                    Call InsertarError("0", "GENERAL", "GENERAL", "Error. No se puede procesar. Ver archivo log InterfaceLobruno-" & NroProceso & ".log", objAuditoria!audnro, True, StrSql)
                                End If
...
MyBeginTrans
    ...
    StrSql = "SELECT * FROM legajo WHERE LEGAJO = '" & legajo & "'"
    
    OpenRecordset StrSql, objInsert
    
    If objInsert.EOF Then
        StrSql = "INSERT INTO legajo (LEGAJO,APELLIDO_NOMBRE,DOCUMENTO_EMISOR,SEXO,ESTADO_CIVIL,APELLIDO_CASADA,FECHA_NACIMIENTO,NACION_NACIMIENTO,LUGAR_NACIMIENTO,NACIONALIDAD,DOCUMENTO_TIPO,DOCUMENTO_NUMERO,EMPRESA)"
        ...
        StrSql = StrSql & ConvFecha(fech_nac) & ",'"
    ...
    Else
        ...
        Select Case tipo
            Case "ALTA_EMP":
                ...
                StrSql = StrSql & "FECHA_NACIMIENTO = " & ConvFecha(fech_nac) & ","
...
MyBeginTrans
    ...
    StrSql = "SELECT * FROM legajo_familiares WHERE LEGAJO = '" & legajo & "'"
    OpenRecordset StrSql, objInsert
    
    If objInsert.EOF Then
        StrSql = "INSERT INTO legajo_familiares (LEGAJO,FAMILIAR_NUMERO,PARENTESCO,APELLIDO_NOMBRE,FECHA_NACIMIENTO,NACIONALIDAD,NACION_NACIMIENTO,LUGAR_NACIMIENTO,DOCUMENTO_TIPO,DOCUMENTO_NUMERO,SEXO,ESTADO_CIVIL,CUIL_FAM,FECHA_INICIO_VINCULO) "
        StrSql = StrSql & " VALUES ('" & legajo & "','"
        ...
        StrSql = StrSql & estcivil & "','"
        StrSql = StrSql & Cuil & "',"
        StrSql = StrSql & ConvFecha(famfec) & ")"
    Else
        StrSql = "UPDATE legajo_familiares SET "
        Select Case tipo
            Case "ALTA_FAM":
                ...
                StrSql = StrSql & "SEXO = '" & sexo & "',"
                StrSql = StrSql & "ESTADO_CIVIL = '" & estcivil & "',"
                StrSql = StrSql & "FECHA_INICIO_VINCULO = " & ConvFecha(famfec)
            Case "ALTA_DOC_F":
                StrSql = StrSql & "DOCUMENTO_TIPO = '" & doc_tipo & "',"
                StrSql = StrSql & "DOCUMENTO_NUMERO = '" & doc_nro & "'"
            Case "PARENT_F":
...
MyBeginTrans
    ...
    StrSql = "SELECT * FROM legajo_posicion WHERE LEGAJO = '" & legajo & "' AND FECHA = " & ConvFecha(Fecha)
    OpenRecordset StrSql, objInsert
    
    If objInsert.EOF Then
        StrSql = "INSERT INTO legajo_posicion(legajo,fecha,puesto,puesto_cambio,grado,grado_cambio,empresa,"
    ...
    Else
        StrSql = "UPDATE legajo_posicion SET "
        Select Case tipo
            Case "PUESTO":
...
MyBeginTrans
    ...
    If tipo <> "EMPRESA" Or tipo <> "FECHA_INGRESO" Then
        'Buscar tipo estructura EMPRESA y FECHA_INGRESO
        StrSql = "SELECT empleado.empfaltagr, empleado.empleg, his_estructura.estrnro "
        StrSql = StrSql & " FROM empleado INNER JOIN his_estructura ON his_estructura.ternro=empleado.ternro "
        StrSql = StrSql & " WHERE empleado.ternro = " & ternro & " AND his_estructura.tenro= " & C_empresa
        StrSql = StrSql & " ORDER BY htetdesde DESC"
        OpenRecordset StrSql, objInsert
        If objInsert.EOF Then
            Call InsertarError("0", "GENERAL", "GENERAL", "Error. No se puede procesar. Ver archivo log InterfaceLobruno-" & NroProceso & ".log", audnro, True, StrSql)
        Else
            If tipo <> "EMPRESA" Then
                ...
                End If
            End If
            If tipo <> "FECHA_INGRESO" Then
                aux_fecha_ingreso = CDate(objInsert!empfaltagr)
            End If
        End If
    End If
    
    If Not (EsNulo(legajo) Or EsNulo(aux_empresa) Or EsNulo(aux_fecha_ingreso)) Then
        StrSql = "SELECT * FROM legajo_liquidar WHERE LEGAJO = '" & legajo & "' AND EMPRESA = '" & aux_empresa & "' AND FECHA_INGRESO = " & ConvFecha(aux_fecha_ingreso)
        OpenRecordset StrSql, objInsert
        
        If objInsert.EOF Then
            StrSql = "INSERT INTO legajo_liquidar(legajo,EMPRESA,FECHA_INGRESO,FECHA_ANTIGUEDAD,INGRESO_GRUPO,"
            ...
            StrSql = StrSql & ConvFecha(Fecha_Antiguedad) & ","
            StrSql = StrSql & ConvFecha(Ingreso_Grupo) & ",'"
            ...
            StrSql = StrSql & horas_mesh & ","
            StrSql = StrSql & horas_mesd & ",'"
            ...
            StrSql = StrSql & ConvFecha(Fecha_baja) & ",'"
            ...
            StrSql = StrSql & ConvFecha(Fecha_Promo) & ",'"
            StrSql = StrSql & CBU & "')"
        Else
            StrSql = "UPDATE legajo_liquidar SET "
            Select Case tipo
                ...
                Case "HORAS_MESH":
                    StrSql = StrSql & "HORAS_MESH = " & horas_mesh
                Case "HORAS_MESD":
                    StrSql = StrSql & "HORAS_MESD = " & horas_mesd
                Case "INSTRUM_PAGO":
                ...
                Case "FECHA_BAJA":
                    StrSql = StrSql & "FECHA_BAJA = " & ConvFecha(Fecha_baja)
...
'-- LEGAJO -----------------
    ...
    Do Until objRs.EOF
        StrSql = "select * from legajo where legajo='" & objRs!legajo & "'"
...
'-- LEGAJO FAMILIARES -----------------
    ...
    Do Until objRs.EOF
        StrSql = "select * from legajo_familiares where legajo='" & objRs!legajo & "' and FAMILIAR_NUMERO='" & objRs!FAMILIAR_NUMERO & "'"
...
'-- LEGAJO POSICION -----------------
    ...
    Do Until objRs.EOF
        StrSql = "SELECT * FROM legajo_posicion WHERE LEGAJO = '" & objRs!legajo & "' AND FECHA = '" & Format(objRs!Fecha, formatofecha) & "'"
...
'-- LEGAJO LIQUIDAR -----------------
    ...
    Do Until objRs.EOF
        StrSql = "SELECT * FROM legajo_liquidar WHERE LEGAJO = '" & objRs!legajo & "' AND EMPRESA = '" & objRs!empresa & "' AND FECHA_INGRESO = '" & Format(objRs!Fecha_Ingreso, formatofecha) & "'"
