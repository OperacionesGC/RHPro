...
For I = LI_WF_Tpa To LS_WF_Tpa
    ...
    Case Par_Base_Sac:
        Base_Sac = Arr_WF_TPA(I).Valor
        Par1 = True
    Case Par_Mes:
        Mes_Semestre = Arr_WF_TPA(I).Valor
        Par2 = True
    Case Else
    End Select
Next I
...
For I = LI_WF_Tpa To LS_WF_Tpa
    Select Case Arr_WF_TPA(I).tipoparam
    Case Par_Base_Vac:
        Base_Vac = Arr_WF_TPA(I).Valor
        Par1 = True
    Case Par_Mes:
        Mes_Anio = Arr_WF_TPA(I).Valor
        Par2 = True
    Case Par_Dias:
        Dias_Vac = Arr_WF_TPA(I).Valor
        Par3 = True
    Case Par_Prv:
        Prom_Vac = Arr_WF_TPA(I).Valor
        Par4 = True
    Case Par_Div1:
        Div_Vac1 = Arr_WF_TPA(I).Valor
        Par5 = True
    Case Par_Div2:
        Div_Vac2 = Arr_WF_TPA(I).Valor
        Par6 = True
    Case Par_Mul:
        Mul_Vac = Arr_WF_TPA(I).Valor
        Par7 = True
    Case Else
    End Select
Next I
...
For I = LI_WF_Tpa To LS_WF_Tpa
    Select Case Arr_WF_TPA(I).tipoparam
    Case Par_Prov_Sac:
        Ya_Provisionado = Arr_WF_TPA(I).Valor
    Case Par_Mes:
        Mes_Semestre = Arr_WF_TPA(I).Valor
        Case Else
    End Select
...
If rs_Concepto.EOF Then
    ...
    Exit Function
Else
    StrSql = "SELECT detliq.concnro FROM periodo "
    StrSql = StrSql & " INNER JOIN proceso ON proceso.pliqnro = periodo.pliqnro "
    StrSql = StrSql & " INNER JOIN cabliq ON proceso.pronro = cabliq.pronro "
    StrSql = StrSql & " INNER JOIN detliq ON detliq.cliqnro = cabliq.cliqnro "
