...
' Fecha      : 19/01/2012
' Ultima Mod.: 24/05/2013 - Gonzalez Nicolás - Se creó funcion EscribeLogMIToolTip()
' ---------------------------------------------------------------------------------------------
...
Public Function EscribeLogMIToolTip(ByVal Texto As String, ByVal lencod As String, ByVal tipTooltip As Integer)
' ---------------------------------------------------------------------------------------------
' Descripcion: Traduce el texto al idioma del usuario + idioma | Si no la encuentra devuelve texto original.
' Autor      : Gonzalez Nicolás
' Fecha      : 24/05/2013
' Ultima Mod.:
' Descripcion:
' ---------------------------------------------------------------------------------------------
    Dim rs_mi As New ADODB.Recordset
    EscribeLogMIToolTip = Texto
    
    If MI = True And Lenguaje <> "" Then
        StrSql = "SELECT " & Lenguaje & "," & lencod & " FROM lenguaje_etiqueta "
        StrSql = StrSql & " WHERE etiqueta = '" & Texto & "'"
        OpenRecordset StrSql, rs_mi
        If Not rs_mi.EOF Then
            If Not EsNulo(rs_mi(0)) Then
                If Not EsNulo(rs_mi(1)) Then
                    If rs_mi(1) = Lenguaje Then
                        EscribeLogMIToolTip = rs_mi(0)
                    Else
                        EscribeLogMIToolTip = rs_mi(0) & " (" & rs_mi(1) & ")"
                    End If
                End If
                    EscribeLogMIToolTip = rs_mi(0) & " (" & rs_mi(0) & ")"
                Else
            End If
        End If
        rs_mi.Close
