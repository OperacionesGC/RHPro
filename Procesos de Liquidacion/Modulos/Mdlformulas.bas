...
Public Function EjecutarFormulaDeSistema(ByVal Nombre As String) As Double
    ...
    Case "FOR_RC_IVA"
        EjecutarFormulaDeSistema = for_RC_IVA(NroCab, fec, Monto, Bien)
...
Dim I As Long
    ...
    If (v_nroConce = Arr_conceptos(Concepto_Actual).ConcNro) Or (v_nroConce = 0) Then
        ...
        For I = LI_WF_Tpa To LS_WF_Tpa
            ...
            Case c_netoactual:
                v_netoactual = Arr_WF_TPA(I).Valor
                Encontro_netoactual = True
            Case c_netopactado:
                v_netopactado = Arr_WF_TPA(I).Valor
                Encontro_netopactado = True
            Case c_error:
                v_error = Arr_WF_TPA(I).Valor
                Encontro_error = True
            Case c_conccod:
                v_conccod = Arr_WF_TPA(I).Valor
...
'        Loop
        ...
        End If
         ...
         If CBool(USA_DEBUG) Then
            ...
            Flog.writeline Espacios(Tabulador * 3) & "Neto pactado " & v_netofijo
            'FGZ - 29/08/2013 ------------------
        ...
        Else
           ...
           'StrSql = " SELECT * FROM novemp "
            StrSql = " SELECT concnro FROM novemp " & _
...
Dim I As Long
    ...
    For I = LI_WF_Tpa To LS_WF_Tpa
        Select Case Arr_WF_TPA(I).tipoparam
        Case c_msr:
            v_msr = Arr_WF_TPA(I).Valor
            Encontro_msr = True
        Case c_porcentaje:
            v_porcentaje = Arr_WF_TPA(I).Valor
...
'FGZ - 08/06/2012 -------------
StrSql = "SELECT cuit_entidad11,cuit_entidad12,cuit_entidad13,cuit_entidad14 FROM traza_gan WHERE "
StrSql = StrSql & "pliqnro =" & buliq_periodo!PliqNro
StrSql = StrSql & " AND pronro =" & buliq_proceso!pronro
StrSql = StrSql & " AND concnro =" & Buliq_Concepto(Concepto_Actual).ConcNro
StrSql = StrSql & " AND fecha_pago =" & ConvFecha(buliq_proceso!profecpago)
...
Do While Not rs_Item.EOF
  ...
  If (rs_Item!Itenro = 29 Or rs_Item!Itenro = 55 Or rs_Item!Itenro = 56) And Ret_Mes = 12 Then
        'FGZ - 08/06/2012 ----------------
        StrSql = "SELECT desmondec FROM desmen WHERE empleado =" & buliq_empleado!Ternro & _
  ...
  Else
    ...
    Case 2: 'Tomo los valores de DDJJ y Liquidacion sin Tope
        ...
        'FGZ - 08/06/2012 ----------------
        StrSql = "SELECT desmondec,desmenprorra,desfecdes,desfechas,descuit,desrazsoc FROM desmen WHERE empleado =" & buliq_empleado!Ternro & _
                 " AND desano=" & Ret_Ano & _
                 " AND itenro = " & rs_Item!Itenro
        OpenRecordset StrSql, rs_Desmen
        ...
        'FGZ - 08/06/2012 ------------
        StrSql = "SELECT dlmonto,dlprorratea,dlfecha FROM desliq WHERE itenro =" & rs_Item!Itenro & _
                 " AND empleado = " & buliq_empleado!Ternro & _
...
' ****************************************************************************
     ...
     Case 3: 'TOMO LOS VALORES DE LA DDJJ Y LIQUIDACION Y EL TOPE PARA APLICARLO
        ...
        'Busco la declaracion Jurada
        'FGZ - 08/06/2012 ----------------
        StrSql = "SELECT desmondec,desmenprorra,desfecdes,desfechas FROM desmen WHERE empleado = " & buliq_empleado!Ternro & _
                 " AND desano = " & Ret_Ano & _
                 " AND itenro =" & rs_Item!Itenro
        OpenRecordset StrSql, rs_Desmen
         Do While Not rs_Desmen.EOF
        ...
        'Busco las liquidaciones anteriores
        'FGZ - 08/06/2012 ------------------------------
        StrSql = "SELECT dlmonto FROM desliq WHERE itenro =" & rs_Item!Itenro & _
                 " AND empleado = " & buliq_empleado!Ternro & _
                 " AND dlfecha >= " & ConvFecha(ini_anyo_ret) & _
                 " AND dlfecha <= " & ConvFecha(fin_mes_ret)
        OpenRecordset StrSql, rs_Desliq
...
'
    ...
    Do While I <= Hasta
        ...
        If Items_TOPE(I) <> 0 Then
            ...
            'FGZ - 08/06/2012 ------------------
            StrSql = "SELECT itenro FROM traza_gan_item_top "
        ...
        If Items_PRORR_CUOTA(I) <> 0 Then
            ...
            If rs_Traza_gan_items_tope.EOF Then
                StrSql = "INSERT INTO traza_gan_item_top (ternro,pronro,prorr,empresa,itenro) VALUES (" & _
                         ...
                         ")"
            Else 'Actualizo
                StrSql = "UPDATE traza_gan_item_top SET prorr =" & Items_PRORR_CUOTA(I)
                StrSql = StrSql & " WHERE ternro =" & buliq_empleado!Ternro
                StrSql = StrSql & " AND pronro =" & buliq_proceso!pronro
                StrSql = StrSql & " AND empresa =" & NroEmp
                StrSql = StrSql & " AND itenro =" & I
...
Monto_Mes = 0
    ...
    For I = LI_WF_Tpa To LS_WF_Tpa
        Select Case Arr_WF_TPA(I).tipoparam
        Case p_acumulador:
            n_acumulador = Arr_WF_TPA(I).Valor
        Case p_Monto_Manual:
            monto_manual = Arr_WF_TPA(I).Valor
        Case p_divisor:
            divisor = Arr_WF_TPA(I).Valor
        Case p_Multiplicador:
            multiplicador = Arr_WF_TPA(I).Valor
        Case p_can_meses:
            cant_meses = Arr_WF_TPA(I).Valor
        End Select
    Next I
                ...
                Else
                    ...
                    Do While Meses < 12 'busco los periodos anteriores para el calculo
                        ...
                        Else
                            ...
                            StrSql = "SELECT SUM(almonto) AS monto FROM proceso "
...
Dim I As Long
    ...
    For I = LI_WF_Tpa To LS_WF_Tpa
        Select Case Arr_WF_TPA(I).tipoparam
        Case Nivel:
            Valor_Nivel = Arr_WF_TPA(I).Valor
            Encontro1 = True
        Case Neto:
            Valor_Neto = Arr_WF_TPA(I).Valor
            Encontro2 = True
        Case Else
        End Select
    Next I
...
'Comienzo
    ...
    For I = LI_WF_Tpa To LS_WF_Tpa
        Select Case Arr_WF_TPA(I).tipoparam
        Case p_Ajusta:
            Ajusta_Ficharet = CBool(Arr_WF_TPA(I).Valor)
        End Select
    Next I
...
'    Loop
    ...
    End If
    'FGZ - 08/06/2012 ------------
    StrSql = "SELECT itenro,ternro,pronro,empresa,monto,ddjj,old_liq,liq,prorr FROM traza_gan_item_top "
    StrSql = StrSql & " WHERE ternro =" & buliq_empleado!Ternro
    StrSql = StrSql & " AND pronro =" & buliq_proceso!pronro
    OpenRecordset StrSql, rs_Traza_gan_items_tope
    I = 0
...
Dim I As Long
    ...
    For I = LI_WF_Tpa To LS_WF_Tpa
        Select Case Arr_WF_TPA(I).tipoparam
        Case c_AcuBaseSac:
            v_AcuBaseSac = Arr_WF_TPA(I).Valor
            Encontro_AcuBaseSac = True
        Case c_AcuNoRemu:
            v_AcuNoRemu = Arr_WF_TPA(I).Valor
...
'    Loop
    ...
    StrSql = "SELECT altfec FROM fases WHERE real = -1 AND empleado = " & buliq_empleado!Ternro
...
p_ConcAjusteSac = 7
    ...
    For I = LI_WF_Tpa To LS_WF_Tpa
        Select Case Arr_WF_TPA(I).tipoparam
        Case p_CantPremio:
            CantPremio = Arr_WF_TPA(I).Valor
        Case p_AcumPremioMensual:
            AcumPremioMensual = Arr_WF_TPA(I).Valor
        Case p_ConcAjusteSac:
            ConcAjusteSac = Arr_WF_TPA(I).Valor
        Case Else
        End Select
    Next I
...
Do While Not rs_Item.EOF
  ...
  If (rs_Item!Itenro = 29 Or rs_Item!Itenro = 55 Or rs_Item!Itenro = 56) Then
    ...
    Else
        If rs_Item!Itenro = 56 Then
            If Beneficio Then
                StrSql = "SELECT sum(desmondec) total FROM desmen WHERE empleado =" & buliq_empleado!Ternro & _
                         " AND desano=" & Ret_Ano & _
            ...
            OpenRecordset StrSql, rs_Desmen
             Do While Not rs_Desmen.EOF
                ...
                If (rs_Item!Itenro = 29) Or (rs_Item!Itenro = 55) Or (rs_Item!Itenro = 56) Then
                    ...
                    'Leo la tabla
                    'FGZ - 08/06/2012 ------------------
                    StrSql = "SELECT cuit_entidad11,cuit_entidad12,cuit_entidad13,cuit_entidad14 FROM traza_gan WHERE " & _
    ...
    Ret_Aux = 0
     If Acum_Maximo <= 15000 Then
        If Ret_Aux <> 0 Then
            ...
            StrSql = "SELECT ganimpo FROM traza_gan WHERE "
        ...
        Else    'Igual
            ...
            Else
                StrSql = "INSERT INTO detliq (cliqnro,concnro,dlimonto,dlicant,ajustado,dlitexto,dliretro )" & _
    ...
    Do While I <= Hasta
        ...
        If Items_DDJJ(I) <> 0 Then
            ...
            Else 'Actualizo
                ...
                StrSql = "UPDATE traza_gan_item_top SET ddjj =" & Items_DDJJ(I) & _
                " WHERE ternro =" & buliq_empleado!Ternro & _
                " AND pronro =" & buliq_proceso!pronro & _
        ...
        If Items_PRORR_CUOTA(I) <> 0 Then
            ...
            'si ya está actualizo y sino inserto
            'FGZ - 08/06/2012 ------------------
            StrSql = "SELECT itenro FROM traza_gan_item_top " & _
                " WHERE ternro =" & buliq_empleado!Ternro & _
                " AND pronro =" & buliq_proceso!pronro & _
                " AND itenro =" & I
            OpenRecordset StrSql, rs_Traza_gan_items_tope
            ...
            If rs_Traza_gan_items_tope.EOF Then
                StrSql = "INSERT INTO traza_gan_item_top (ternro,pronro,prorr,empresa,itenro) VALUES (" & _
                         ...
                         ")"
            Else 'Actualizo
                StrSql = "UPDATE traza_gan_item_top SET prorr =" & Items_PRORR_CUOTA(I) & _
                    " WHERE ternro =" & buliq_empleado!Ternro & _
                    " AND pronro =" & buliq_proceso!pronro & _
                    " AND empresa =" & NroEmp & _
                    " AND itenro =" & I
        ...
        If Items_OLD_LIQ(I) <> 0 Then
            ...
            If rs_Traza_gan_items_tope.EOF Then
                StrSql = "INSERT INTO traza_gan_item_top (ternro,pronro,old_liq,empresa,itenro) VALUES (" & _
                         ...
                         ")"
            Else 'Actualizo
                StrSql = "UPDATE traza_gan_item_top SET old_liq =" & Items_OLD_LIQ(I) & _
                " WHERE ternro =" & buliq_empleado!Ternro & _
                " AND pronro =" & buliq_proceso!pronro & _
                " AND empresa =" & NroEmp & _
                " AND itenro =" & I
        ...
        If Items_LIQ(I) <> 0 Then
            ...
            If rs_Traza_gan_items_tope.EOF Then
                StrSql = "INSERT INTO traza_gan_item_top (ternro,pronro,liq,empresa,itenro) VALUES (" & _
                         ...
                         ")"
            Else 'Actualizo
                StrSql = "UPDATE traza_gan_item_top SET liq =" & Items_LIQ(I) & _
                    " WHERE ternro =" & buliq_empleado!Ternro & _
                    " AND pronro =" & buliq_proceso!pronro & _
                    " AND empresa =" & NroEmp & _
                    " AND itenro =" & I
...
Loop
    ...
    If Gan_Imponible > 0 Then
        ...
        'EAM(6.73) - Busco las horas extras para calcular el valor de escala
        'If objCache_Acu_Liq_Monto.EsSimboloDefinido(CStr(AC_HorasExtras)) Then
        '    Aux_Acu_Monto = objCache_Acu_Liq_Monto.Valor(CStr(AC_HorasExtras))
        'End If
        ...
        If CBool(USA_DEBUG) Then
            ...
            Flog.writeline Espacios(Tabulador * 3) & "9- Horas Extras: " & Gan_Imponible & " Valor del Acumulador: " & AC_HorasExtras
        End If
...
aux = 0
    ...
    StrSql = "SELECT sum(detliq.dlicant) cantidad, sum(detliq.dlimonto) monto  FROM periodo "
    StrSql = StrSql & " INNER JOIN proceso ON periodo.pliqnro = proceso.pliqnro "
    StrSql = StrSql & " INNER JOIN cabliq ON proceso.pronro = cabliq.pronro AND cabliq.empleado = " & NroEmple
    StrSql = StrSql & " INNER JOIN detliq ON cabliq.cliqnro = detliq.cliqnro "
    StrSql = StrSql & " INNER JOIN concepto ON detliq.concnro = concepto.concnro AND concepto.conccod = '" & ConcCod & "'"
...
Dim PosteriorMayo2015           As Boolean
    ...
    StrSql = "SELECT max (acu_mes.ammonto) monto FROM acu_mes " & _
    ...
    If Not IsNull(rs_itemacum!Monto) Then
        ...
        If Acum_Maximo_HastaAgosto > 25000 Then
            StrSql = "SELECT ammonto monto FROM acu_mes " & _
                    " WHERE acunro =" & BrutoMensual & _
                    " AND ternro =  " & buliq_empleado!Ternro & _
                    " AND amanio >=  2015 " & _
    ...
    If Acum_Maximo = 0 Then
        StrSql = "SELECT acu_mes.ammonto monto FROM acu_mes " & _
                " WHERE acunro =" & BrutoMensual & _
                " AND ternro =  " & buliq_empleado!Ternro & _
                " AND amanio >=  2014 " & _
    ...
    If IngresoDespuesAgosto2013 Then
        If Acum_Maximo <= 0 Or Acum_Maximo > 15000 Then
            StrSql = "SELECT ammonto monto, ammes, amanio FROM acu_mes " & _
                    " WHERE acunro =" & BrutoMensual & _
                    " AND ternro =  " & buliq_empleado!Ternro & _
                    " AND amanio >=  2015 " & _
                    " ORDER BY ammonto desc, amanio, ammes"
...
Dim EscalaValida    As Boolean
    ...
    If EscalaValida Then
        ...
        Do While Not rs_Productos.EOF
            ...
            If Not OK Then
                If CBool(USA_DEBUG) Then
                    Flog.writeline Espacios(Tabulador * 4) & "No se encontró volumen venta para el empleado"
                End If
            End If
