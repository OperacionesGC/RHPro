Attribute VB_Name = "repCompEmpl"
Option Explicit

'Const Version = 1.01  'Version Inicial
'Const FechaVersion = "30/10/2006"

Const Version = 1.02
Const FechaVersion = "21/10/2009"
Const UltimaModificacion = "Encriptacion de string connection"
Const UltimaModificacion1 = "Manuel Lopez"
...
Dim concnro As Integer
    ...
    Nombre_Arch = PathFLog & "ReporteCompEmpleados" & "-" & NroProceso & ".log"
    ...
    If Not objRs.EOF Then
       ...
       StrSql = "INSERT INTO rep_comp_empl (bpronro,pliqnro1,pliqdesc1,pliqmesanio1," & _
...
Dim emplmonto1 As Double
Dim emplcant1 As Double
Dim emplmonto2 As Double
Dim emplcant2 As Double
Dim difmontoempl As Double
Dim porcmontoempl As Double
Dim difcantempl As Double
Dim porccantempl As Double
Dim concnro As Long
...
'------------------------------------------------------------------
If lista = "0" Then
    unSoloConcepto = False
Else
    unSoloConcepto = InStr(lista, ",") = 0
End If

StrSql = "SELECT proceso.pliqnro, detliq.concnro, detliq.dlimonto, detliq.dlicant, "
StrSql = StrSql & "conccod, concabr, ternro, empleg, terape, terape2, ternom, ternom2,con_acum.acunro "
...
End If
'StrSql = StrSql & "ORDER BY detliq.concnro, ternro"
'StrSql = StrSql & "ORDER BY con_acum.acunro,conccod, ternro"
StrSql = StrSql & "ORDER BY ternro, con_acum.acunro,conccod "
...
'    concnro = rsConsult!concnro
    ...
    If Not IsNull(rsConsult!ternom) Then
...
Do Until rsConsult.EOF
    ...
    If rsConsult!concnro <> concnro Or rsConsult!ternro <> ternro Then
        Orden = Orden + 1
        ' Genero el procesamiento del Concepto
'        Flog.writeline Espacios(Tabulador * 1) & "Insertando en la tabla los datos del concepto: " & Conccod & " y empleado: " & empleg & " - " & terape & ", " & ternom
        ...
        '------------------------------------------------------------------
        difmontoempl = CDbl(emplmonto1) - CDbl(emplmonto2)
        If CDbl(emplmonto2) <> 0 Then
            porcmontoempl = (CDbl(difmontoempl) * CDbl(100)) / CDbl(emplmonto2)
        Else
            porcmontoempl = IIf(CDbl(emplmonto1) <> 0, CDbl(100), CDbl(0))
        End If
        difcantempl = CDbl(emplcant1) - CDbl(emplcant2)
        If CDbl(emplcant2) <> 0 Then
            porccantempl = (CDbl(difcantempl) * CDbl(100)) / CDbl(emplcant2)
        Else
            porccantempl = IIf(CDbl(emplcant1) <> 0, CDbl(100), CDbl(0))
        End If

        '-------------------------------------------------------------------------------
        'Inserto los datos en la BD
        '-------------------------------------------------------------------------------
        StrSql = "INSERT INTO rep_comp_empl_det (bpronro,tconcepto,concnro,conccod,concabr," & _
                 "emplmonto1,emplcant1,emplmonto2,emplcant2,difmontoempl,porcmontoempl," & _
                 "difcantempl,porccantempl,empleg,terape,ternom,terape2,ternom2,ternro,orden) " & _
                 "VALUES (" & NroProceso & "," & TConcepto & "," & _
                 concnro & ",'" & Conccod & "','" & concabr & "'," & emplmonto1 & "," & _
                 emplcant1 & "," & emplmonto2 & "," & emplcant2 & "," & difmontoempl & "," & _
                 porcmontoempl & "," & difcantempl & "," & porccantempl & "," & empleg & ",'" & _
                 terape & "','" & ternom & "','" & terape2 & "','" & ternom2 & "'," & ternro & "," & Orden & ")"
        objConn.Execute StrSql, , adExecuteNoRecords
    ...
    If rsConsult!pliqnro = pliqnro1 Then
        If Not IsNull(rsConsult!dlimonto) Then
            emplmonto1 = CDbl(rsConsult!dlimonto)
        End If
        If Not IsNull(rsConsult!dlicant) Then
            emplcant1 = CDbl(rsConsult!dlicant)
        End If
    End If

    If rsConsult!pliqnro = pliqnro2 Then
        If Not IsNull(rsConsult!dlimonto) Then
            emplmonto2 = CDbl(rsConsult!dlimonto)
        End If
        If Not IsNull(rsConsult!dlicant) Then
            emplcant2 = CDbl(rsConsult!dlicant)
        End If
    End If
    ...
    If rsConsult!concnro <> concnro Or unSoloConcepto Then
        TiempoAcumulado = GetTickCount
