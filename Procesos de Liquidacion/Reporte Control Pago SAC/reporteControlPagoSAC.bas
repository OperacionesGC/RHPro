Attribute VB_Name = "repControlPagoSAC"
Option Explicit
...
Dim ArrParametros
    ...
    Nombre_Arch = PathFLog & "ReporteControlPagoSAC" & "-" & NroProceso & ".log"
    
    Set fs = CreateObject("Scripting.FileSystemObject")
    Set Flog = fs.CreateTextFile(Nombre_Arch, True)
    
    Flog.Writeline "Inicio Proceso de Control Pago SAC : " & Now
    ...
    If Not objRs.EOF Then
       ...
       StrSql = "INSERT INTO rep_pago_sac (bpronro,pliqnro,pliqdesc,pliqmesanio," & _
...
StrSql = StrSql & " WHERE repnro = 143 "
...
Do Until rsConsult.EOF
    ...
    '------------------------------------------------------------------
    ' Busco el valor de la 2 a la 8 columna
    '------------------------------------------------------------------
    For i = 2 To 8
        If col(i, 0) Then
...
'            StrSql = StrSql & " INNER JOIN detliq   ON cabliq.cliqnro = detliq.cliqnro  AND cabliq.empleado = " & rsConsult!Empleado & " AND detliq.concnro = " & col(i, 1)
    ...
    If guardarDatos Then
        ...
        '-------------------------------------------------------------------------------
        StrSql = "INSERT INTO rep_pago_sac_det (bpronro,ternro,empleg,terape,ternom,terape2," & _
