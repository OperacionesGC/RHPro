Attribute VB_Name = "repControlPrevicionSAC"
Option Explicit
...
Dim ArrParametros
    ...
    Nombre_Arch = PathFLog & "ReporteControlPrevisionSAC" & "-" & NroProceso & ".log"
    
    Set fs = CreateObject("Scripting.FileSystemObject")
    Set Flog = fs.CreateTextFile(Nombre_Arch, True)
    
    Flog.Writeline "Inicio Proceso de Control Prevision SAC : " & Now
    ...
    If Not objRs.EOF Then
       ...
       StrSql = "INSERT INTO rep_cont_sac (bpronro,pliqnro,pliqdesc,pliqmesanio," & _
...
StrSql = StrSql & " WHERE repnro = 141 "
...
Do Until rsConsult.EOF
    ...
    If empleado_ant <> CLng(rsConsult!Empleado) Then
        ...
        '-------------------------------------------------------------------------------
        'Busco el mes con el mayor monto para la tercer columna (confnrocol = 3)
        '-------------------------------------------------------------------------------
        pliqmes_aux = pliqmes
        ...
        Do While Not terminar
            ...
            StrSql = StrSql & "WHERE ammes=" & pliqmes_aux & " AND amanio=" & pliqanio_aux & " AND ternro=" & rsConsult!Empleado & " AND acunro = " & col(3, 1)
            
            OpenRecordset StrSql, rs2
            If Not rs2.EOF Then
                If CDbl(rs2!ammonto) > CDbl(monto_aux) Then
                    monto_aux = CDbl(rs2!ammonto)
                    result_col(3) = pliqmes_aux
                End If
            End If
            rs2.Close
...
'            StrSql = StrSql & " INNER JOIN detliq   ON cabliq.cliqnro = detliq.cliqnro  AND cabliq.empleado = " & rsConsult!Empleado & " AND detliq.concnro = " & col(i, 1)
    ...
    If guardarDatos Then
        ...
        '-------------------------------------------------------------------------------
        StrSql = "INSERT INTO rep_cont_sac_det (bpronro,ternro,empleg,terape,ternom,terape2," & _
