Attribute VB_Name = "repControlVacaciones"
Option Explicit
...
Dim ArrParametros
    ...
    Nombre_Arch = PathFLog & "ReporteControlVacaciones" & "-" & NroProceso & ".log"
    
    Set fs = CreateObject("Scripting.FileSystemObject")
    Set Flog = fs.CreateTextFile(Nombre_Arch, True)
    
    Flog.Writeline "Inicio Proceso de Control Vacaciones: " & Now
    ...
    If Not objRs.EOF Then
       ...
       StrSql = "INSERT INTO rep_control_vaca (bpronro,pliqnro,pliqdesc,pliqmesanio," & _
       ...
       Call GenerarDatosProceso(pliqmes, pliqanio, pliqdesde, pliqhasta, pronro)
...
'--------------------------------------------------------------------
Sub GenerarDatosProceso(ByVal pliqmes As Integer, ByVal pliqanio As Integer, ByVal pliqdesde As Date, ByVal pliqhasta As Date, ByVal pronro As Integer)
...
Dim result_col(16) As Double
Dim col(16, 2) As Long
Dim tenro As String
...
StrSql = StrSql & " WHERE repnro = 145 "
...
Do Until rsConsult.EOF
    ...
    fechaegr = "null"
    If rs2.EOF Then
         Flog.Writeline "No encontró fase real para el empleado: " & rsConsult!Empleado
    Else
    ...
    '------------------------------------------------------------------
    ' Busco la estructura. Primer columna del confrep
    '------------------------------------------------------------------
...
'    rs2.Close
    ...
    '------------------------------------------------------------------
    ' Busco el valor de la 3 columna
    '------------------------------------------------------------------
    j = j + 1
    fecha_vac = "null"
    ...
    StrSql = "SELECT * FROM emp_lic " & _
             ...
             " AND elfechadesde < " & ConvFecha(pliqhasta) & _
             " AND elfechahasta > " & ConvFecha(pliqdesde) & _
             " ORDER BY elfechadesde DESC"
    OpenRecordset StrSql, rs2

    If Not rs2.EOF Then
        fecha_vac = ConvFecha(rs2!elfechadesde)
    End If
    
    rs2.Close
    
    '------------------------------------------------------------------
    ' Busco el valor de la  columna 3 hasta 13 del confrep
    '------------------------------------------------------------------
    result_col(2) = 0
    For i = 4 To 14
        j = j + 1
    ...
    '-------------------------------------------------------------------------------
    StrSql = "INSERT INTO rep_control_vaca_det (bpronro,ternro,empleg,terape,ternom,terape2," & _
             "ternom2,fechaing,col1,col2,col3,col4,col5,col6,col7,col8,col9,col10,col11," & _
             "col12,col13,col14,col15,col16,fechaegr,catdes,catcod)" & _
             " VALUES (" & NroProceso & "," & rsConsult!Empleado & "," & empleg & ",'" & _
             terape & "','" & ternom & "','" & terape2 & "','" & ternom2 & "'," & _
             ConvFecha(fechaing) & ",'" & estrdabr & "'," & numberForSQL(result_col(2)) & "," & fecha_vac & "," & _
             ...
             numberForSQL(result_col(15)) & "," & numberForSQL(result_col(16)) & "," & _
