Attribute VB_Name = "MdlHsLiq"
Option Explicit

'Const Version = "1.01"
'Const FechaVersion = "09/02/2009"
''Autor = Diego Nuñez
''   Version Inicial
...
Dim ArrParametros
    ...
    Nombre_Arch = PathFLog & "Reporte_HSLiq" & "-" & NroProcesoBatch & ".log"
...
Dim Detalle(1 To 20, 1 To 20) As Variant
...
'        Proc_Aprob = Mid(Parametros, pos1, pos2 - pos1 + 1)
        ...
        Flog.writeline "Parametro ordenado =" & Mid(Parametros, pos1, pos2 - pos1 + 1)
...
'Ahora busco el periodo anterior
RegDetalle = 0
StrSql = "SELECT pliqnro, pliqmes, pliqanio, pliqdesde, pliqhasta, pliqdesc  FROM periodo WHERE pliqnro = " & _
"(SELECT MAX(pliqnro) FROM periodo WHERE pliqmes < '" & MesPeriodo & "' AND pliqanio = '" & anio & "')"
OpenRecordset StrSql, rs_Periodo
...
Else 'era el primer periodo del año por lo que busco el maximo del año anterior
    rs_Periodo.Close
    RegDetalle = 0
    StrSql = "SELECT MAX(pliqanio) aa FROM periodo WHERE pliqanio < '" & anio & "'"
    OpenRecordset StrSql, rs_Periodo
    If Not rs_Periodo.EOF Then
        rs_Periodo.MoveFirst
        anioAnt = rs_Periodo!aa
        rs_Periodo.Close
        RegDetalle = 0
        StrSql = "SELECT pliqnro, pliqmes, pliqanio, pliqdesde, pliqhasta, pliqdesc  FROM periodo WHERE pliqnro = " & _
        "(SELECT MAX(pliqnro) FROM periodo WHERE pliqanio = '" & anioAnt & "')"
        OpenRecordset StrSql, rs_Periodo
        If Not rs_Periodo.EOF Then
            Do
                RegDetalle = RegDetalle + 1
        ...
        End If
        rs_Periodo.Close
    Else 'No existe el período
    End If
End If



'Ahora debería encontrar el primer periodo anterior al menor de mis periodos (Memopliq(1))
'StrSql = "SELECT pliqnro, pliqmes, pliqanio FROM periodo WHERE pliqnro < " & perdesde
'RegDetalle = 0
'StrSql = "SELECT pliqnro, pliqmes, pliqanio, pliqdesde, pliqhasta, pliqdesc  FROM periodo WHERE pliqnro = (SELECT MAX(pliqnro) FROM periodo WHERE pliqnro < " & perdesde & ")"
'OpenRecordset StrSql, rs_Periodo
'If Not rs_Periodo.EOF Then
'    Do
'        RegDetalle = RegDetalle + 1
...
MesPeriodo & "' AND pliqmes <= '" & MesPeriodo2 & "' AND pliqanio = '" & anio & "' ORDER BY pliqmes"
OpenRecordset StrSql, rs_Periodo
...
' Traigo todos los periodos de liquidación válidos para la selección del usuario
'StrSql = "SELECT pliqnro, pliqmes, pliqanio, pliqdesde, pliqhasta, pliqdesc FROM periodo WHERE pliqnro >= " & perdesde & " AND pliqnro <= " & perhasta
'OpenRecordset StrSql, rs_Periodo
'If rs_Periodo.EOF Then
'    Flog.writeline "No se encontró el Periodo"
'    Exit Sub
...
'StrSql = "SELECT confnrocol, confval, confetiq, confexp, conftipo FROM confrep where repnro = 250 and confcalc = 1 order by confnrocol"
'OpenRecordset StrSql, rs_Confrep
'IncPorc = rs_Confrep.RecordCount
'rs_Confrep.Close
...
StrSql = "SELECT confnrocol, confval, confetiq, conftipo FROM confrep where repnro = 250 and confcalc = 0 order by confnrocol"
OpenRecordset StrSql, rs_Confrep
...
If Not rs_Confrep.EOF Then
    ...
    Do
        If RegDetalle <> -1 Then
            If rs_Confrep!confnrocol <> RegDetalle Then k = k + 1
        End If
        RegDetalle = rs_Confrep!confnrocol
        rs_Confrep.MoveNext
...
'IncPorc = 98 / ColDetalle
RegDetalle = 0
If k > 20 Then k = 20
If Not rs_Confrep.EOF Then
    rs_Confrep.MoveFirst
    MemoConfrep = ""
    ColConfrep = rs_Confrep!confnrocol
    Do
        If rs_Confrep!confnrocol <> ColConfrep Or Salir Then
            'Hago la consulta anterior
            ColDetalle = 1
            RegDetalle = RegDetalle + 1
            If RegDetalle > 20 Then Exit Do
            For I = 1 To CantPeriodos
                IncPorc = 98 / (k * CantPeriodos)
                'Acá ciclo para cada uno de los periodos del reporte
                'me quedo con la primer columna del confrep (registro de mi detalle de reporte)
                
                'Tener en cuenta que hay que recalcular la lista de empleados para el período actual
                ...
                If tenro3 <> "" And tenro3 <> "0" Then ' esto ocurre solo cuando se seleccionan los tres niveles
                    ...
                    StrSql = StrSql & " AND (estact1.htetdesde<=" & ConvFecha(Memopliqhasta(I)) & " AND (estact1.htethasta is null or estact1.htethasta>=" & ConvFecha(Memopliqhasta(I)) & "))"
                    ...
                    StrSql = StrSql & " AND (estact2.htetdesde<=" & ConvFecha(Memopliqhasta(I)) & " AND (estact2.htethasta is null or estact2.htethasta>=" & ConvFecha(Memopliqhasta(I)) & "))"
                    ...
                    " AND (estact3.htetdesde<=" & ConvFecha(Memopliqhasta(I)) & " AND (estact3.htethasta is null or estact3.htethasta>=" & ConvFecha(Memopliqhasta(I)) & "))"
                ...
                Else
                    If tenro2 <> "" And tenro2 <> "0" Then ' ocurre cuando se selecciono hasta el segundo nivel
                        ...
                        " AND (estact2.htetdesde<=" & ConvFecha(Memopliqhasta(I)) & " AND (estact2.htethasta is null or estact2.htethasta>=" & ConvFecha(Memopliqhasta(I)) & "))"
                ...
                Flog.writeline "Consulta de empleados: " & StrSql
                Flog.writeline "Inicio Periodo: " & ConvFecha(Memopliqdesde(I))
                Flog.writeline "Final Periodo: " & ConvFecha(Memopliqhasta(I))
                Flog.writeline ""
                If rs_EmplFiltro.EOF Then
                    Flog.writeline "No se encontraron Empleados"
                    Exit Sub
                End If
                'RegDetalle = 0
                
                MemoDotacion(I) = rs_EmplFiltro.RecordCount
                ...
                Do
                    'RegDetalle = RegDetalle + 1
                    'ReDim Preserve MemoEmpleados(RegDetalle)
                    'MemoEmpleados(RegDetalle) = rs_EmplFiltro!ternro
                    If MemoEmpleados = "" Then
                        MemoEmpleados = MemoEmpleados & Mid(Str(rs_EmplFiltro!ternro), 2, Len(Str(rs_EmplFiltro!ternro)))
                    Else
                        MemoEmpleados = MemoEmpleados & "," & Str(rs_EmplFiltro!ternro)
                    End If
                    rs_EmplFiltro.MoveNext
                Loop Until rs_EmplFiltro.EOF
                rs_EmplFiltro.Close
                
                'Calculo la dotación para el período actual
'                StrSql = "SELECT DISTINCT empleado FROM fases WHERE empleado IN (" & MemoEmpleados & ") AND altfec <=" & _
'                ConvFecha(Memopliqhasta(I)) & " AND (bajfec is NULL OR bajfec >" & ConvFecha(Memopliqdesde(I)) & ")"
'                OpenRecordset StrSql, rs_EmplFiltro
'                MemoDotacion(I) = rs_EmplFiltro.RecordCount
'                rs_EmplFiltro.Close
                ...
                StrSql = "SELECT SUM(acu_mes.ammonto) SUMA from acu_mes "
                StrSql = StrSql & "WHERE (acu_mes.ternro IN (" & StrEmpleado & ") AND acu_mes.acunro IN ("
                StrSql = StrSql & MemoConfrep & ") AND acu_mes.amanio = " & Memopliqanio(I)
                StrSql = StrSql & " AND acu_mes.ammes = " & Memopliqmes(I) & ")"
                'MsgBox StrSql
                
                
                'StrSql = "SELECT SUM(acu_liq.almonto) SUMA from periodo "
                ...
                'StrSql = StrSql & "WHERE cabliq.empleado IN (" & MemoEmpleados & ") AND "
                'StrSql = StrSql & "acu_liq.acunro IN (" & MemoConfrep & ") AND periodo.pliqnro = " & I
                OpenRecordset StrSql, rs_Detalle
                Detalle(RegDetalle, ColDetalle) = rs_Detalle!SUMA
                If IsNull(Detalle(RegDetalle, ColDetalle)) Then Detalle(RegDetalle, ColDetalle) = 0
            ...
            ColConfrep = rs_Confrep!confnrocol
            MemoConfrep = ""
            If Salir Then Exit Do
        Else
            If MemoConfrep = "" Then
                MemoConfrep = MemoConfrep & Mid(Str(rs_Confrep!confval), 2, Len(Str(rs_Confrep!confval)))
                j = j + 1
                ReDim Preserve LabelsRegistrosDetalle(j)
                LabelsRegistrosDetalle(j) = rs_Confrep!confetiq
            Else
                MemoConfrep = MemoConfrep & "," & Str(rs_Confrep!confval)
            End If
            StrEncontrado = rs_Confrep!confetiq
            rs_Confrep.MoveNext
        End If
        ...
        End If
    Loop Until rs_Confrep.EOF
    'Detalle(RegDetalle, ColDetalle) = rs_Confrep!confetiq
End If
'Progreso = Progreso + IncPorc
'UpdateProgreso (Progreso)
If RegDetalle > 20 Then
    RegDetalle = 20
End If
NroFilasDetalle = RegDetalle

Flog.writeline "Calculando Promedios, mínimos y máximos del sector de detalles"

'calculo el Promedio, Mínimo y Máximo de la tabla (últimas 3 columnas)
For RegDetalle = 1 To 20
    If Detalle(RegDetalle, 1) = "" Then Exit For
    I = 0
    Sumatoria = 0
    Min = Detalle(RegDetalle, 1)
    Max = Detalle(RegDetalle, 1)
    For ColDetalle = 1 To CantPeriodos
        If Detalle(RegDetalle, ColDetalle) <> "" Then I = I + 1
        Sumatoria = Sumatoria + Detalle(RegDetalle, ColDetalle)
        If Detalle(RegDetalle, ColDetalle) < Min And Detalle(RegDetalle, ColDetalle) <> "" Then Min = Detalle(RegDetalle, ColDetalle)
        If Detalle(RegDetalle, ColDetalle) > Max And Detalle(RegDetalle, ColDetalle) <> "" Then Max = Detalle(RegDetalle, ColDetalle)
    Next
    Prom = Round(Sumatoria / I, 2)
    Detalle(RegDetalle, 18) = Prom
    Detalle(RegDetalle, 19) = Min
    Detalle(RegDetalle, 20) = Max
Next
rs_Confrep.Close

'Comienzo a analizar los valores que son calculados por los indicadores y me quedo con el número de registro a operar en
'cada caso (registro de la tabla Detalle
RegDetalle = 0: j = 0: NoInd = False: NoEv = False
StrSql = "SELECT confnrocol, confval, confetiq, confexp, conftipo FROM confrep where repnro = 250 and confcalc = 1 order by confnrocol"
OpenRecordset StrSql, rs_Confrep
If Not rs_Confrep.EOF Then IncPorc = 1 / rs_Confrep.RecordCount
If Not rs_Confrep.EOF Then
    'Analizo el string encontrado entre [], determino a q columna está asociado
    rs_Confrep.MoveFirst
    Do
        TipoEncontrado = rs_Confrep!conftipo
        MemoConfrep = rs_Confrep!confexp
        FormulaOriginal = MemoConfrep
        If Mid(FormulaOriginal, 1, 4) <> "sql:" Then
            Flog.writeline "Obteniendo parámetros de indicadores con fórmulas:"
            Do
                pos1 = 0: pos2 = 0
                pos1 = InStr(1, MemoConfrep, "[") + 1
                pos2 = InStr(1, MemoConfrep, "]") - 1
                If pos1 = 1 And pos2 = -1 Then Exit Do
                StrEncontrado = Mid(MemoConfrep, pos1, pos2 - pos1 + 1)
                'Ahora comparo este string con la memoria que generé en el paso anterior
                Pase = False
                For I = 1 To UBound(ColumnasConfrep)
                    If StrEncontrado = ColumnasConfrep(I) Then
                        MemoConfrep = Replace(MemoConfrep, "[" & StrEncontrado & "]", "@" & I & "@")
                        Pase = True
                    End If
                Next
                
                If Not Pase Then
                        'Si no encuentro nada en el paso anterior, esto significa que se trata de un indicador y no un detalle
                        StrSql = "SELECT confnrocol, confval, confetiq, confexp, conftipo FROM confrep where repnro = 250 and confcalc = 1 order by confnrocol"
                        OpenRecordset StrSql, rs_ConfrepInd
                        If Not rs_ConfrepInd.EOF Then
                            rs_ConfrepInd.MoveFirst
                            Do
                                If rs_ConfrepInd!confetiq = StrEncontrado Then
                                    'Si el indicador encontrado es una consulta, entonces evalúo la consulta y en vez de expandir
                                    'el indicador por sus componentes, reemplazo el mismo por el resultado de la evaluación
                                    If Mid(rs_ConfrepInd!confexp, 1, 4) = "sql:" Then 'es una consulta
                                        MemoConfrep = FormulaOriginal 'restauro MemoConfrep a su estado original de la consulta
                                        For ColDetalle = 1 To CantPeriodos
                                            StrSql = Replace(Mid(rs_ConfrepInd!confexp, 5, Len(rs_ConfrepInd!confexp)), "[Empleados]", StrEmpleado)
                                            StrSql = Replace(StrSql, "[Limite Inf periodo]", ConvFecha(Memopliqdesde(ColDetalle)))
                                            StrSql = Replace(StrSql, "[Limite Sup periodo]", ConvFecha(Memopliqhasta(ColDetalle)))
                                            
                                            On Error Resume Next
                                            OpenRecordset StrSql, rs_Consulta
                                            Salir = False
                                            If Err.Number <> 0 Then
                                                Flog.writeline " Error en el indicador " & rs_Confrep!confetiq & ", periodo " & _
                                                NombrePeriodos(ColDetalle) & ",al evaluar la expresión: " & FormulaOriginal & " (" & _
                                                StrSql & ")"
                                                Salir = True
                                                NoEv = True
                                                    FormulaOriginal = Replace(FormulaOriginal, "[" & StrEncontrado & "]", "(1.01)")
                                                    FormulaOriginal = Replace(FormulaOriginal, "[" & StrEncontrado & "]", "(1.01)")
                                                    MemoConfrep = FormulaOriginal
                                                    Exit Do
                                                'Evaluaciones(RegDetalle, ColDetalle) = -1.010101010101
                                            End If
                                            On Error GoTo CE
                                            ...
                                            If Not Salir Then
                                                If Not rs_Consulta.EOF Then
                                                    ...
                                                    'ahora reemplazo el indicador encontrado junto con los corchetes por el resultado de la consulta
                                                    FormulaOriginal = Replace(FormulaOriginal, "[" & StrEncontrado & "]", "(" & rs_Consulta(0) & ")")
                                                    FormulaOriginal = Replace(FormulaOriginal, "[" & StrEncontrado & "]", "(" & rs_Consulta.Fields(0) & ")")
                                                    MemoConfrep = FormulaOriginal
                                                    Exit Do
                                                    
'                                                    If IsNull(Evaluaciones(RegDetalle, ColDetalle)) Then
'                                                        Evaluaciones(RegDetalle, ColDetalle) = 0
'                                                    End If
'                                                    If Not IsNumeric(Evaluaciones(RegDetalle, ColDetalle)) And Not IsNull(Evaluaciones(RegDetalle, ColDetalle)) Then
'                                                        Evaluaciones(RegDetalle, ColDetalle) = -1.010101010101
'                                                        Flog.writeline " Error en el indicador " & rs_Confrep!confetiq & ", periodo " & _
'                                                        NombrePeriodos(ColDetalle) & ",al evaluar la expresión: " & FormulaOriginal & " (" & _
'                                                        StrSql & ")"
'                                                    End If
                                    ...
                                    Else
                                        MemoConfrep = FormulaOriginal 'restauro MemoConfrep a su estado original de la consulta
                                        'ahora reemplazo el indicador encontrado junto con los corchetes por la expresión que representa
                                        FormulaOriginal = Replace(FormulaOriginal, "[" & StrEncontrado & "]", "(" & rs_ConfrepInd!confexp & ")")
                                        MemoConfrep = FormulaOriginal
                                        Exit Do
                                    End If
                                End If
                                rs_ConfrepInd.MoveNext
                                
                            Loop Until rs_ConfrepInd.EOF
                        End If
                End If
                Pase = False
            ...
            Loop
            'En memoconfrep tengo la expresion a evaluar, por lo q la guardo en la memoria de evaluaciones
            RegDetalle = RegDetalle + 1
            If RegDetalle > 20 Then Exit Do
            For ColDetalle = 1 To CantPeriodos
                'Evalúo MemoConfrep y cambio los índices por los valores presentes en Detalle
                StrEncontrado = Replace(MemoConfrep, "@1@", Detalle(1, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@2@", Detalle(2, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@3@", Detalle(3, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@4@", Detalle(4, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@5@", Detalle(5, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@6@", Detalle(6, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@7@", Detalle(7, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@8@", Detalle(8, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@9@", Detalle(9, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@10@", Detalle(10, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@11@", Detalle(11, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@12@", Detalle(12, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@13@", Detalle(13, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@14@", Detalle(14, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@15@", Detalle(15, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@16@", Detalle(16, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@17@", Detalle(17, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@18@", Detalle(18, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@19@", Detalle(19, ColDetalle))
                StrEncontrado = Replace(StrEncontrado, "@20@", Detalle(20, ColDetalle))
                'Evaluaciones(RegDetalle, ColDetalle) = StrEncontrado
                If NoEv Then
                    exito = False
                Else
                    Evaluaciones(RegDetalle, ColDetalle) = Round(eval.Evaluate(StrEncontrado, exito, True), 2)
                End If
                If Not exito Then 'hubo un problema en la evaluacion de la fórmula
                    If TipoEncontrado <> "BLA" And TipoEncontrado <> "TIT" Then
                        Evaluaciones(RegDetalle, ColDetalle) = -1.010101010101
                        Flog.writeline " Error en el indicador " & rs_Confrep!confetiq & ", periodo " & _
                        NombrePeriodos(ColDetalle) & ",al evaluar la expresión: " & FormulaOriginal & " (" & _
                        StrEncontrado & ")"
                    ...
                    'NoEv = False
                End If
                Evaluaciones(RegDetalle, 17) = TipoEncontrado
            Next
        Else
            Flog.writeline "Obteniendo parámetros de indicadores con consultas:"
            RegDetalle = RegDetalle + 1
            If RegDetalle > 20 Then Exit Do
            For ColDetalle = 1 To CantPeriodos
                StrSql = Replace(Mid(FormulaOriginal, 5, Len(FormulaOriginal)), "[Empleados]", StrEmpleado)
                StrSql = Replace(StrSql, "[Limite Inf periodo]", ConvFecha(Memopliqdesde(ColDetalle)))
                StrSql = Replace(StrSql, "[Limite Sup periodo]", ConvFecha(Memopliqhasta(ColDetalle)))
                ...
                If Not Salir Then
                    If Not rs_Consulta.EOF Then
                        rs_Consulta.MoveFirst
                        Evaluaciones(RegDetalle, ColDetalle) = rs_Consulta(0)
                        Evaluaciones(RegDetalle, ColDetalle) = rs_Consulta.Fields(0)
                        If IsNull(Evaluaciones(RegDetalle, ColDetalle)) Then
                            Evaluaciones(RegDetalle, ColDetalle) = 0
                        End If
                        If Not IsNumeric(Evaluaciones(RegDetalle, ColDetalle)) And Not IsNull(Evaluaciones(RegDetalle, ColDetalle)) Then
        ...
        j = j + 1
        ReDim Preserve LabelsRegistrosIndicadores(j)
        LabelsRegistrosIndicadores(j) = rs_Confrep!confetiq
        rs_Confrep.MoveNext
        NoEv = False
        Progreso = Round(Progreso + IncPorc, 2)
        UpdateProgreso (Progreso)
    ...
    Loop Until rs_Confrep.EOF
Else
    Flog.writeline "No se encontraron Items configurados para el sector de Indicadores del reporte."
    NoInd = True
    'Exit Sub
End If
rs_Confrep.Close

' Ahora debo llenar la memoria de Indicadores
For RegDetalle = 1 To 20
    For ColDetalle = 1 To 17
        'Indicadores(RegDetalle, ColDetalle) = Evaluacion(Evaluaciones(RegDetalle, ColDetalle))
    Next
Next
Flog.writeline "Obteniendo promedios, mínimos y máximos del sector de indicadores:"

'calculo el Promedio, Mínimo y Máximo de la tabla de indidicadores(últimas 3 columnas)
For RegDetalle = 1 To 20
    If Evaluaciones(RegDetalle, 1) = "" Then Exit For
    I = 0
    Sumatoria = 0
    If Evaluaciones(RegDetalle, ColDetalle) <> -1.010101010101 Then Min = Evaluaciones(RegDetalle, 1)
    If Evaluaciones(RegDetalle, ColDetalle) <> -1.010101010101 Then Max = Evaluaciones(RegDetalle, 1)
    For ColDetalle = 1 To CantPeriodos
        If Evaluaciones(RegDetalle, ColDetalle) <> -1.010101010101 Then
            If Evaluaciones(RegDetalle, ColDetalle) <> "" Then I = I + 1
            Sumatoria = Sumatoria + Evaluaciones(RegDetalle, ColDetalle)
            If Evaluaciones(RegDetalle, ColDetalle) < Min And Evaluaciones(RegDetalle, ColDetalle) <> "" Then Min = Evaluaciones(RegDetalle, ColDetalle)
            If Evaluaciones(RegDetalle, ColDetalle) > Max And Evaluaciones(RegDetalle, ColDetalle) <> "" Then Max = Evaluaciones(RegDetalle, ColDetalle)
        End If
    Next
    If I <> 0 Then
        Prom = Round(Sumatoria / I, 2)
        Evaluaciones(RegDetalle, 18) = Prom
        Evaluaciones(RegDetalle, 19) = Min
        Evaluaciones(RegDetalle, 20) = Max
    Else
        Evaluaciones(RegDetalle, 18) = -1.010101010101
        Evaluaciones(RegDetalle, 19) = -1.010101010101
        Evaluaciones(RegDetalle, 20) = -1.010101010101
    End If
Next


'calculo el Promedio, Mínimo y Máximo de la tabla de LA DOTACION(últimas 3 columnas)
Flog.writeline "Obteniendo promedios, mínimos y máximos de la dotación:"
I = 0
Sumatoria = 0
RegDetalle = 1
Min = MemoDotacion(RegDetalle)
Max = MemoDotacion(RegDetalle)
For RegDetalle = 1 To CantPeriodos
    I = I + 1
    Sumatoria = Sumatoria + MemoDotacion(RegDetalle)
    If MemoDotacion(RegDetalle) < Min Then Min = MemoDotacion(RegDetalle)
    If MemoDotacion(RegDetalle) > Max Then Max = MemoDotacion(RegDetalle)
    Prom = Round(Sumatoria / I, 2)
Next
MemoDotacion(18) = Prom
MemoDotacion(19) = Min
MemoDotacion(20) = Max
...
If LabelsRegistrosDetalle(j) <> "" Then
    StrSql = "INSERT INTO rep_hsliq (bpronro,fecha,hora,iduser,filanro,valor,valor1"
    StrSql = StrSql & ",valor2,valor3,valor4,valor5,valor6,valor7,valor8,valor9,valor10"
    StrSql = StrSql & ",valor11,valor12,valor13,valor14,valor15,valor16,valor17,valor18,valor19,valor20"
    ...
    StrSql = StrSql & j & ","
    StrSql = StrSql & NroFilasDetalle & ","
    For ColDetalle = 1 To CantPeriodos
        StrSql = StrSql & "'" & NombrePeriodos(ColDetalle) & "',"
    Next
    For ColDetalle = CantPeriodos + 1 To 17
        StrSql = StrSql & "NULL,"
    Next
    StrSql = StrSql & "'Promedio',"
    StrSql = StrSql & "'Mínimo',"
    StrSql = StrSql & "'Máximo'"
    StrSql = StrSql & ")"
    objConn.Execute StrSql, , adExecuteNoRecords
    'Después inserto la dotacón de cada periodo
    j = 2
    StrSql = "INSERT INTO rep_hsliq (bpronro,fecha,hora,iduser,filanro,valor,valor1"
    StrSql = StrSql & ",valor2,valor3,valor4,valor5,valor6,valor7,valor8,valor9,valor10"
    StrSql = StrSql & ",valor11,valor12,valor13,valor14,valor15,valor16,valor17,valor18,valor19,valor20"
    ...
    StrSql = StrSql & "'" & iduser & "',"
    StrSql = StrSql & j & ","
    StrSql = StrSql & "'DOTACION',"
    For ColDetalle = 1 To CantPeriodos
        StrSql = StrSql & "'" & MemoDotacion(ColDetalle) & "',"
    Next
    For ColDetalle = CantPeriodos + 1 To 17
        StrSql = StrSql & "NULL,"
    Next
    StrSql = StrSql & MemoDotacion(18) & ","
    StrSql = StrSql & MemoDotacion(19) & ","
    StrSql = StrSql & MemoDotacion(20)
    StrSql = StrSql & ")"
    objConn.Execute StrSql, , adExecuteNoRecords
End If
'IncPorc = 1 / UBound(LabelsRegistrosDetalle)
'inserto en rep_hsliq
If UBound(LabelsRegistrosDetalle) > 20 Then
    ReDim Preserve LabelsRegistrosDetalle(20)
End If
For j = 1 To UBound(LabelsRegistrosDetalle)
    RegDetalle = RegDetalle + 1
    If LabelsRegistrosDetalle(j) <> "" Then
        StrSql = "INSERT INTO rep_hsliq (bpronro,fecha,hora,iduser,filanro,valor,valor1"
        StrSql = StrSql & ",valor2,valor3,valor4,valor5,valor6,valor7,valor8,valor9,valor10"
        ...
        StrSql = StrSql & "'" & iduser & "',"
        StrSql = StrSql & j + 2 & ","
        StrSql = StrSql & "'" & LabelsRegistrosDetalle(j) & "',"
        For ColDetalle = 1 To CantPeriodos
            StrSql = StrSql & "'" & Detalle(RegDetalle, ColDetalle) & "',"
        Next
        For ColDetalle = CantPeriodos + 1 To 17
            StrSql = StrSql & "NULL,"
        Next
        For ColDetalle = 18 To 19
            StrSql = StrSql & "'" & Detalle(RegDetalle, ColDetalle) & "',"
        Next
        StrSql = StrSql & "'" & Detalle(RegDetalle, 20) & "'"
        StrSql = StrSql & ")"
        objConn.Execute StrSql, , adExecuteNoRecords
    End If
'    Progreso = Progreso + IncPorc
...
'IncPorc = 1 / UBound(LabelsRegistrosIndicadores)
RegDetalle = 0
'inserto en rep_hsliq
If Not NoInd Then
    If UBound(LabelsRegistrosIndicadores) > 20 Then
        ReDim Preserve LabelsRegistrosIndicadores(20)
    End If
    For I = 1 To UBound(LabelsRegistrosIndicadores)
        RegDetalle = RegDetalle + 1
        If LabelsRegistrosIndicadores(I) <> "" Then
            StrSql = "INSERT INTO rep_hsliq (bpronro,fecha,hora,iduser,filanro,valor,valor1"
            StrSql = StrSql & ",valor2,valor3,valor4,valor5,valor6,valor7,valor8,valor9,valor10"
            StrSql = StrSql & ",valor11,valor12,valor13,valor14,valor15,valor16,valor17,valor18,valor19,valor20"
            StrSql = StrSql & ") VALUES ("
            ...
            StrSql = StrSql & j + 2 & ","
            j = j + 1
            StrSql = StrSql & "'" & LabelsRegistrosIndicadores(I) & "',"
            For ColDetalle = 1 To CantPeriodos
                StrSql = StrSql & "'" & Evaluaciones(RegDetalle, ColDetalle) & "',"
            Next
            For ColDetalle = CantPeriodos + 1 To 16
                StrSql = StrSql & "NULL,"
            Next
            StrSql = StrSql & "'" & Evaluaciones(RegDetalle, 17) & "',"
            For ColDetalle = 18 To 19
                StrSql = StrSql & "'" & Evaluaciones(RegDetalle, ColDetalle) & "',"
            Next
            StrSql = StrSql & "'" & Evaluaciones(RegDetalle, 20) & "'"
            StrSql = StrSql & ")"
            objConn.Execute StrSql, , adExecuteNoRecords
        End If
    '    Progreso = Progreso + IncPorc
