Attribute VB_Name = "repControlSACyBoni"
Option Explicit
...
Dim ArrParametros
    ...
    Nombre_Arch = PathFLog & "ReporteControlSACyBoni" & "-" & NroProceso & ".log"
    
    Set fs = CreateObject("Scripting.FileSystemObject")
    Set Flog = fs.CreateTextFile(Nombre_Arch, True)
    
    Flog.Writeline "Inicio Proceso de FN Control SAC y BONI: " & Now
    ...
    If Not objRs.EOF Then
       ...
       StrSql = "INSERT INTO rep_sacybonif (bpronro,pliqnro,pliqdesc,pliqmesanio," & _
       ...
       Call GenerarDatosProceso(tprocnro, pliqnro, pliqmes, pliqanio, pliqhasta, pronro)
...
'--------------------------------------------------------------------
Sub GenerarDatosProceso(ByVal tprocnro As Integer, ByVal pliqnro As Integer, ByVal pliqmes As Integer, ByVal pliqanio As Integer, ByVal pliqhasta As Date, ByVal pronro As Integer)
...
StrSql = StrSql & " WHERE repnro = 144 "
...
Do Until rs2.EOF
     ...
     If rs2!conftipo = "CO" Then
         ...
         If rs3.EOF Then
             col(rs2!confnrocol, 1) = 0
         Else
             col(rs2!confnrocol, 1) = rs3!concnro
             lista_conc = lista_conc & "," & CStr(rs3!concnro)
         End If
         col(rs2!confnrocol, 0) = False
         
         rs3.Close
     End If
     
     If rs2!conftipo = "AC" Then
         col(rs2!confnrocol, 1) = rs2!confval
         col(rs2!confnrocol, 0) = True
...
Do Until rsConsult.EOF
    ...
    '------------------------------------------------------------------
    ' Busco el valor de la 2 a la 4 columna
    '------------------------------------------------------------------
    For i = 2 To 4
        If col(i, 0) Then
...
'            StrSql = StrSql & " INNER JOIN detliq   ON cabliq.cliqnro = detliq.cliqnro  AND cabliq.empleado = " & rsConsult!Empleado & " AND detliq.concnro = " & col(i, 1)
    ...
    '------------------------------------------------------------------
    ' Calculo el valor de la 5 columna
    '------------------------------------------------------------------
    If CDbl(result_col(2)) = CDbl(0) Then
        result_col(5) = 100
    Else
        result_col(5) = result_col(3) / result_col(2)
    End If
    
    '------------------------------------------------------------------
    ' Busco el valor de la 6
    '------------------------------------------------------------------
    If col(6, 0) Then
        StrSql = " SELECT almonto,acunro "
        StrSql = StrSql & " FROM acu_liq"
        StrSql = StrSql & " WHERE acunro = " & col(6, 1)
        StrSql = StrSql & " AND cliqnro =  " & rsConsult!cliqnro
    Else
        StrSql = " SELECT detliq.dlimonto AS almonto "
        StrSql = StrSql & " FROM cabliq "
        StrSql = StrSql & " INNER JOIN detliq ON cabliq.cliqnro = detliq.cliqnro AND cabliq.cliqnro = " & rsConsult!cliqnro & " AND detliq.concnro = " & col(6, 1)
'        StrSql = " SELECT detliq.dlimonto AS almonto "
'        StrSql = StrSql & " FROM cabliq "
'        StrSql = StrSql & " INNER JOIN proceso  ON proceso.pronro = cabliq.pronro AND cabliq.pronro = " & pronro
'        StrSql = StrSql & " INNER JOIN periodo  ON proceso.pliqnro = periodo.pliqnro "
'        StrSql = StrSql & " INNER JOIN detliq   ON cabliq.cliqnro = detliq.cliqnro  AND cabliq.empleado = " & rsConsult!Empleado & " AND detliq.concnro = " & col(6, 1)
    ...
    OpenRecordset StrSql, rs2
    If Not rs2.EOF Then
        result_col(6) = result_col(6) + rs2!almonto
    End If
    rs2.Close
    
    '------------------------------------------------------------------
    ' Calculo el valor de la 7 columna
    '------------------------------------------------------------------
    If CDbl(result_col(2)) = CDbl(0) Then
        result_col(7) = 100
    Else
        result_col(7) = result_col(6) / result_col(2)
    End If
    ...
    If guardarDatos Then
        ...
        '-------------------------------------------------------------------------------
        StrSql = "INSERT INTO rep_sacybonif_det (bpronro,ternro,empleg,terape,ternom,terape2," & _
