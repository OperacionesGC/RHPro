Attribute VB_Name = "MdlValesImp"
Option Explicit

Const Version = 1.01 'Martin Ferraro - Version Inicial
Const FechaVersion = "27/02/2007"


Public Sub Main()
' ---------------------------------------------------------------------------------------------
' Descripcion: Procedimiento inicial Vales.
...
Dim ArrParametros
    ...
    Nombre_Arch = PathFLog & "Importacion Vales" & "-" & NroProcesoBatch & ".log"
    ...
    StrSql = "SELECT * FROM batch_proceso WHERE btprcnro = 161 AND bpronro =" & NroProcesoBatch
    ...
    If Not rs_batch_proceso.EOF Then
        ...
        Call Generar_Vales(NroProcesoBatch, bprcparam)
    End If
...
Public Sub Generar_Vales(ByVal bpronro As Long, ByVal Parametros As String)
' --------------------------------------------------------------------------------------------
...
'l_tvalenro & "@" & l_pliqnro & "@" & l_todospro & "@" & l_pronro & "@" & l_todosemp & "@" & l_proaprob & "@" & l_usuario
...
If Not IsNull(Parametros) Then
    ...
    If UBound(arrParam) = 6 Then
        ...
        Flog.writeline Espacios(Tabulador * 1) & "Tipo de Vale = " & TvaleNro
...
'--------------------------------------------------------------------------------------
Flog.writeline Espacios(Tabulador * 0) & "Buscando los Conceptos del Confrep 190"
...
StrSql = StrSql & " WHERE repnro = 190 AND conftipo = 'CO' "
OpenRecordset StrSql, rs_Consult

If rs_Consult.EOF Then
    Flog.writeline Espacios(Tabulador * 1) & "No se encuentran los conceptos del confrep 190."
    GoTo Fin
Else
...
If Not rs_Consult.EOF Then
    ...
    Do While Not rs_Consult.EOF
        ...
        If Monto <> 0 Then
            Flog.writeline
            'Creo Cantidad de vales con monto = monto/cantidad
            Flog.writeline Espacios(Tabulador * 2) & "Creando " & Fix(Cantidad) & " Vales Por " & Round(Monto / Cantidad, 2)
            For i = 1 To Fix(Cantidad)
                
                'Inserto en Vales
                StrSql = "INSERT INTO vales "
                StrSql = StrSql & "(empleado,ppagnro,monnro,valmonto, "
                StrSql = StrSql & " valfecped,valfecprev,pliqnro,valdesc, "
                StrSql = StrSql & " pliqdto,pronro,tvalenro,valrevis,valusuario) "
                StrSql = StrSql & " VALUES ("
                StrSql = StrSql & rs_Consult!ternro & ","
                StrSql = StrSql & "NULL" & ","
                StrSql = StrSql & Moneda & ","
                StrSql = StrSql & Round(Monto / Cantidad, 2) & ","
