...
Loop
  ...
  If Not rs_WF_EscalaUTM.EOF Then
   ...
   End If
    ...
    StrSql = "SELECT * FROM sim_ficharet " & NOLOCK & _
             " INNER JOIN sim_proceso ON sim_proceso.pronro = sim_ficharet.pronro " & _
             " AND sim_proceso.pliqnro = " & buliq_periodo!PliqNro & _
...
Public Function for_RecalcConcepto_old(ByVal NroCab As Long, ByVal AFecha As Date, Monto As Double, Bien As Boolean) As Double
' ---------------------------------------------------------------------------------------------
...
Dim rs_trabajo As New ADODB.Recordset
    ...
    Do While Not rs_Periodos.EOF
        ...
        StrSql = StrSql & " WHERE sim_fases.Empleado = " & buliq_empleado!Ternro
        StrSql = StrSql & " AND sim_fases.altfec <= " & ConvFecha(rs_Periodos!pliqhasta)
        StrSql = StrSql & " AND (" & ConvFecha(rs_Periodos!pliqdesde) & " <= sim_fases.bajfec OR sim_fases.bajfec is null)"
        StrSql = StrSql & " AND  sim_fases.real = -1 "
...
SgtPer: rs_Periodos.MoveNext
    ...
    Bien = True
    exito = Bien
    for_RecalcConcepto_old = Abs(TotalDifUFHistPorc) * -1
...
Dim CorrespLiquidarConcp As Double          'EAM (6.67) Determina si se reliquida el conc. para el periodo de recalc.
    ...
    Do While Not rs_Periodos.EOF
        ...
        StrSql = "SELECT empleado FROM sim_fases"
        StrSql = StrSql & " WHERE sim_fases.Empleado = " & buliq_empleado!Ternro
        StrSql = StrSql & " AND sim_fases.altfec <= " & ConvFecha(rs_Periodos!pliqhasta)
        StrSql = StrSql & " AND (" & ConvFecha(rs_Periodos!pliqdesde) & " <= sim_fases.bajfec OR sim_fases.bajfec is null)"
        StrSql = StrSql & " AND  sim_fases.real = -1 "
       ...
       'EAM (6.67) - Si esta configurado el parámtro busco el monto del concepto para ver si hay que recalcularlo
        If EncLicMedica = True Then
            StrSql = "SELECT SUM(ammonto) Monto" & _
                    " FROM sim_acu_mes" & _
                    " WHERE sim_acu_mes.ternro = " & buliq_empleado!Ternro & _
                    " AND sim_acu_mes.acunro = " & ACLicMedica & _
                    " AND sim_acu_mes.amanio = " & rs_Periodos!pliqanio & _
                    " AND sim_acu_mes.ammes = " & rs_Periodos!pliqmes
        ...
        If CorrespLiquidarConcp = 0 Then
            ...
            If ImpoMesHistReliq < (Tope * UFHist) Then
                ...
                StrSql = "SELECT SUM(ammonto) Monto"
                StrSql = StrSql & " FROM sim_acu_mes"
                StrSql = StrSql & " WHERE sim_acu_mes.ternro = " & buliq_empleado!Ternro
                StrSql = StrSql & " AND sim_acu_mes.acunro = " & ConcCodRec
                StrSql = StrSql & " AND sim_acu_mes.amanio = " & rs_Periodos!pliqanio
                StrSql = StrSql & " AND sim_acu_mes.ammes = " & rs_Periodos!pliqmes
                OpenRecordset StrSql, rs_consult
                If Not rs_consult.EOF Then
                    If Not EsNulo(rs_consult!Monto) Then
                        MontoConcRecalc = rs_consult!Monto
                ...
                StrSql = StrSql & " FROM sim_acu_mes"
                StrSql = StrSql & " WHERE sim_acu_mes.ternro = " & buliq_empleado!Ternro
                StrSql = StrSql & " AND sim_acu_mes.acunro = " & AdicSalud
                StrSql = StrSql & " AND sim_acu_mes.amanio = " & rs_Periodos!pliqanio
                StrSql = StrSql & " AND sim_acu_mes.ammes = " & rs_Periodos!pliqmes
                OpenRecordset StrSql, rs_consult
...
Public Function for_RecalcImpuestoUnico_old(ByVal NroCab As Long, ByVal AFecha As Date, Monto As Double, Bien As Boolean) As Double
' ---------------------------------------------------------------------------------------------
...
sgt_periodo:
    ...
    for_RecalcImpuestoUnico_old = Abs(ImpRecalculadoAcum * buliq_periodo!pliqutm) * -1
    If CBool(USA_DEBUG) Then
        Flog.writeline Espacios(Tabulador * 3) & "_______________________________________________________________________________________________"
...
'    Loop
    ...
    'Busco la cantidad de periodos de recalculo del proceso
        ...
        StrSql = StrSql & " AND altfec <= " & ConvFecha(rs_Periodos!pliqhasta)
        StrSql = StrSql & " AND (" & ConvFecha(rs_Periodos!pliqdesde) & " <= bajfec OR bajfec is null)"
