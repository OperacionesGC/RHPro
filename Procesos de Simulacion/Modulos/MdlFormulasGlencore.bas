...
Else
    ...
    StrSql = "SELECT sum(sim_detliq.dlimonto) as Total FROM periodo "
    ...
    StrSql = StrSql & " INNER JOIN sim_detliq ON sim_detliq.cliqnro = sim_cabliq.cliqnro "
    ...
    StrSql = StrSql & " AND sim_detliq.concnro = " & rs_Concepto!concnro
...
StrSql = " SELECT estructura.estrnro, estructura.estrcodext  FROM sim_his_estructura"
StrSql = StrSql & " INNER JOIN estructura ON sim_his_estructura.estrnro = estructura.estrnro "
StrSql = StrSql & " WHERE ternro = " & buliq_empleado!Ternro & " AND "
StrSql = StrSql & " sim_his_estructura.tenro = 22 AND "
