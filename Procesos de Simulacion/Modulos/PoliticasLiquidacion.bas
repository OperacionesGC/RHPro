...
Dim Detalle As Long
    ...
    If rs_Politica.EOF Then
        ...
        StrSql = StrSql & " INNER JOIN sim_his_estructura ON gti_alcanpolitica.alcpolorigen = sim_his_estructura.estrnro "
        StrSql = StrSql & " INNER JOIN alcance_tEstr ON sim_his_estructura.tenro = alcance_tEstr.tenro "
        StrSql = StrSql & " INNER JOIN sim_empleado ON sim_empleado.ternro = sim_his_estructura.ternro"
        ...
        StrSql = StrSql & " And sim_empleado.ternro = " & buliq_empleado!ternro
        StrSql = StrSql & " And (sim_his_estructura.htetdesde <= " & ConvFecha(P_Fecha) & ")"
        StrSql = StrSql & " And ((" & ConvFecha(P_Fecha) & " <= sim_his_estructura.htethasta) or (sim_his_estructura.htethasta is null))"
