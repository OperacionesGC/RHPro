Attribute VB_Name = "MdlSimulador"
Option Explicit
...
Global Arr_Tablas_SIM(25) As String
...
Dim ArrParametros
    ...
    If Err.Number <> 0 Or Error_Encrypt Then
        Nombre_Arch = PathFLog & "Simulacion" & "-" & NroProcesoBatch & ".log"
    ...
    StrSql = "SELECT * FROM batch_proceso WHERE btprcnro = 223 AND bpronro =" & NroProcesoBatch
    ...
    Nombre_Arch = PathFLog & rs_batch_proceso!iduser & "\" & "Simulacion" & "-" & NroProcesoBatch & ".log"
    Set Flog = fs.CreateTextFile(Nombre_Arch, True)
   ...
   Flog.writeline "PID = " & PID
    ...
    Version_Valida = ValidarV(Version, 223, TipoBD)
...
'Cargar el Buffer de los Imponibles Generales
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Cargar el Buffer de los Imponibles Generales"
End If
...
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "TOPES AMPO GLOBALES PARA EL PROCESO DE LIQUIDACIÓN"
End If
Call SetearAmpo
...
OpenRecordset StrSql, rs_Empleados
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Levanto de batch_empleado"
End If
...
End If
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Por cada empleado,  si no esta en cabliq entonces lo inserto"
End If
...
Do While Not rs_Empleados.EOF And Not Todos
    ...
    If rs_cabliq.EOF Then
        StrSql = "INSERT INTO sim_cabliq (pronro, empleado) VALUES (" & _
        NroProc & _
...
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Cargo el arreglo de conceptos"
End If
...
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Cargo las cabezeras de liquidacion en el arreglo global"
End If
...
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Cargo todas las tablas comunes durante toda la liquidacion del proceso"
    Flog.writeline Espacios(Tabulador * 1) & "Cargar_Acumuladores"
End If
Call Cargar_Acumuladores

If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Call CargarBusquedas"
End If
...
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Call Cargar_For_Tpa"
End If
Call Cargar_For_Tpa

If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Call Cargar_Con_For_Tpa"
End If
Call Cargar_Con_For_Tpa

If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Call Cargar_Cge_Segun"
End If
Call Cargar_Cge_Segun

If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Call Cargar_Cft_Segun"
End If
Call Cargar_Cft_Segun

If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Call Cargar_Con_Acum(buliq_proceso!tprocnro, fecha_inicio, fecha_fin)"
End If
Call Cargar_Con_Acum(buliq_proceso!tprocnro, Fecha_Inicio, Fecha_Fin)

'cargo nombre de wf
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "cargo nombre de wf"
End If
Call CargarNombresTablasTemporales

'creo los WF
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "Creo los wf"
End If
...
Do While Empleado_Actual <= Cantidad_de_Empleados 'Not rs_Empleados.EOF
    If CBool(USA_DEBUG) Then
        Flog.writeline Espacios(Tabulador * 1) & "Comienzo la transaccion"
    End If
    ...
    Termino_gross = True
    If CBool(USA_DEBUG) Then
        Flog.writeline Espacios(Tabulador * 1) & "Call Establecer_Empleado"
    End If
    ...
    ' FGZ - 27/04/2004
    If CBool(USA_DEBUG) Then
        Flog.writeline Espacios(Tabulador * 1) & "Call Establecer_Empresa(fecha_inicio, fecha_fin)"
    End If
    ...
    If Mantener_Liq Then 'no se puede borrar
        StrSql = "SELECT * FROM sim_detliq WHERE cliqnro = " & buliq_cabliq!cliqnro
        ...
        If Not rs_HayAlgo.EOF Then
            ...
            End If
        Else
            StrSql = "SELECT * FROM sim_acu_liq WHERE cliqnro = " & buliq_cabliq!cliqnro
            If rs_HayAlgo.State = adStateOpen Then rs_HayAlgo.Close
            OpenRecordset StrSql, rs_HayAlgo
            If Not rs_HayAlgo.EOF Then
                'Empleado liquidado, no se hace nada
    ...
    If Not SoloLimpieza And ((Mantener_Liq And Not EmpleadoLiquidado) Or Not Mantener_Liq) Then
        ...
        Call CalcularDiferencias(buliq_cabliq!cliqnro, buliq_cabliq!Empleado, buliq_cabliq!pronro, buliq_proceso!PliqNro, False)
    ...
    If SoloLimpieza Then
        StrSql = "DELETE FROM sim_cabliq " & _
                 " WHERE pronro =" & NroProc & _
...
'Cheque si hubo algun error con el empleado
    If EmpleadoSinError Then
        ...
        If CBool(USA_DEBUG) Then
            Flog.writeline Espacios(Tabulador * 1) & "Fin de la transaccion"
        End If
...
If HayAcuNoNeg Then
    StrSql = "UPDATE sim_proceso SET proaprob = 1, proestdesc = 'Acumuladores Negativos No permitidos' WHERE pronro = " & CStr(buliq_proceso!pronro)
Else
    StrSql = "UPDATE sim_proceso SET proaprob = 1, proestdesc = null WHERE pronro = " & CStr(buliq_proceso!pronro)
End If
objConn.Execute StrSql, , adExecuteNoRecords

If borrarProc Then
    StrSql = "DELETE sim_proceso WHERE pronro = " & CStr(buliq_proceso!pronro)
    objConn.Execute StrSql, , adExecuteNoRecords
...
'    ' Mes por Mes
'    For Mes_Aux = 1 To 12
        StrSql = "SELECT * FROM sim_acu_mes " & _
        ...
        If Not rs_Acu_Mes.EOF Then
            StrSql = "UPDATE sim_acu_mes SET ammonto = ammonto - " & rs_Hisretroactivo!dlimonto & _
                     " WHERE ternro = " & Nro_empleado & _
                     " AND acunro = " & rs_Hisretroactivo!acuNro & _
                     " AND amanio = " & rs_Hisretroactivo!amanio & _
...
Principal:
    ...
    StrSql = "SELECT cliqnro FROM sim_detliq WHERE cliqnro = " & Cabecera
    ...
    If rs_Procesos.EOF And Usa_grossing = False Then
        ...
        StrSql = "DELETE FROM sim_ficharet " & _
    ...
    StrSql = "SELECT sim_cabliq.cliqnro FROM sim_cabliq "
    StrSql = StrSql & " INNER JOIN sim_proceso ON sim_proceso.pronro = sim_cabliq.pronro "
    StrSql = StrSql & " INNER JOIN periodo ON sim_proceso.pliqnro = periodo.pliqnro "
    StrSql = StrSql & " WHERE periodo.pliqnro = " & Nro_Periodo
    StrSql = StrSql & "   AND sim_cabliq.empleado = " & Nro_Ter
    StrSql = StrSql & "   AND sim_cabliq.pronro  <> " & Nro_Proceso
    OpenRecordset StrSql, rs_Procesos
    ...
    If Not rs_Periodo.EOF Then
        '****************No se usa en la simulacion*****************************
        ''FGZ - 20/05/2011 -----------------------------------------------
        ''select *
        ''StrSql = "SELECT * FROM detliq WHERE cliqnro =" & Cabecera
        'StrSql = "SELECT dliretro,ajustado,concnro FROM sim_detliq WHERE cliqnro =" & Cabecera
        ...
        'Elimino los detliq
        StrSql = "DELETE FROM sim_detliq WHERE cliqnro = " & Cabecera
        objConn.Execute StrSql, , adExecuteNoRecords
        ...
        StrSql = "SELECT acumulador.acumes, acumulador.acunro, acumulador.acuimponible,acumulador.acuimpcont " & _
                 " ,sim_acu_liq.almonto,sim_acu_liq.alcant,sim_acu_liq.almontoreal " & _
                 " FROM sim_acu_liq " & _
                 " INNER JOIN acumulador ON sim_acu_liq.acunro = acumulador.acunro " & _
        ...
        Do While Not rs_Acu_Liq.EOF
            If CBool(rs_Acu_Liq!acumes) Then
                ...
                StrSql = "SELECT acunro FROM sim_acu_mes " & _
...
'                                 " AND ammes =" & rs_Periodo!pliqmes & _
'                                 " AND acunro =" & rs_Acu_Liq!acunro
                        StrSql = "UPDATE sim_acu_mes SET ammontoreal = 0, ammonto = 0 " & _
                                 " ,amcant = 0 " & _
                                 " WHERE amanio =" & rs_Periodo!pliqanio & _
                                 " AND ternro =" & Nro_Ter & _
                                 " AND ammes =" & rs_Periodo!pliqmes & _
                                 " AND acunro =" & rs_Acu_Liq!acuNro
                    Else
                        StrSql = "UPDATE sim_acu_mes SET ammontoreal = ammontoreal - " & rs_Acu_Liq!almontoreal & ", ammonto = ammonto - " & rs_Acu_Liq!almonto & _
                                 " ,amcant = amcant - " & IIf(Not EsNulo(rs_Acu_Liq!alcant), rs_Acu_Liq!alcant, 0) & _
                                 " WHERE amanio =" & rs_Periodo!pliqanio & _
                                 " AND ternro =" & Nro_Ter & _
                                 " AND ammes =" & rs_Periodo!pliqmes & _
            ...
            If CBool(rs_Acu_Liq!acuimponible) Or CBool(rs_Acu_Liq!acuimpcont) Then
                ...
                StrSql = "SELECT tconnro, ipamonto,ipacant FROM sim_impproarg " & _
                ...
                Do While Not rs_ImpPro.EOF  'por cada imponible
                    ...
                    'select *
                    'StrSql = "SELECT * FROM impmesarg "
                    StrSql = "SELECT acunro FROM sim_impmesarg " & _
                    ...
                    If Not rs_ImpMes.EOF Then
                        'Actualizo
                        If ultimoProceso Then
                            StrSql = "UPDATE sim_impmesarg SET imamonto = 0 " & _
                                     " ,imacant = 0 " & _
                                 " WHERE imaanio =" & rs_Periodo!pliqanio & _
                                 " AND ternro =" & Nro_Ter & _
                                 " AND imames =" & rs_Periodo!pliqmes & _
                        ...
                        Else
                            StrSql = "UPDATE sim_impmesarg SET imamonto = imamonto - " & rs_ImpPro!ipamonto & _
                        ...
                        StrSql = "UPDATE sim_impproarg SET ipamonto = 0 , ipacant = 0 " & _
                             " WHERE cliqnro =" & Cabecera & _
                             " AND tconnro =" & rs_ImpPro!tconnro & _
                             " AND acunro =" & rs_Acu_Liq!acuNro
                        objConn.Execute StrSql, , adExecuteNoRecords
                        ...
                        If Excluye_Liq Then
                            ...
                            StrSql = "DELETE FROM sim_impmesarg " & _
                                 " WHERE imaanio =" & rs_Periodo!pliqanio & _
                                 " AND ternro =" & Nro_Ter & _
                                 " AND imames =" & rs_Periodo!pliqmes & _
                                 " AND tconnro =" & rs_ImpPro!tconnro & _
        ...
        'Borro el acu_liq
        StrSql = "DELETE FROM sim_acu_liq " & _
                 " WHERE cliqnro =" & Cabecera
        objConn.Execute StrSql, , adExecuteNoRecords
        
        If CBool(USA_DEBUG) Then
            Flog.writeline Espacios(Tabulador * 2) & "Borro el sim_acu_liq"
        End If
        ...
        'Esta tabla no existe en Simulacion ---------------------------
        ...
        StrSql = "DELETE FROM sim_desliq " & _
                 " WHERE pronro =" & Nro_Proceso & _
                 " AND empleado =" & Nro_Ter
        objConn.Execute StrSql, , adExecuteNoRecords
...
'        Loop
        ...
        StrSql = "UPDATE sim_vales SET pronro = null " & _
                 " WHERE pronro = " & NroProc & _
                 " AND sim_vales.empleado = " & Nro_Ter
        objConn.Execute StrSql, , adExecuteNoRecords
...
'FGZ - 18/04/2006 - REEMPLACE LAS LINEAS DE ARRIBA POR LO QUE SIGUE ABAJO
        ...
        StrSql = "UPDATE sim_vacpagdesc SET pronro = null "
        StrSql = StrSql & " WHERE pronro = " & Nro_Proceso
        StrSql = StrSql & " AND ternro = " & Nro_Ter
        objConn.Execute StrSql, , adExecuteNoRecords
        ...
        End If
        'Desmarco las licencias Marcadas por el Proceso
        StrSql = "UPDATE sim_emp_lic SET pronro = null "
        StrSql = StrSql & " WHERE pronro = " & Nro_Proceso
...
'FGZ - 18/04/2006
        ...
        If guarda_nov Then
            StrSql = "DELETE FROM sim_hisnovemp " & _
                     " WHERE empleado = " & Nro_Ter & _
        ...
        If Not Usa_grossing Then
            StrSql = "DELETE FROM sim_novemp " & _
                     " WHERE empleado = " & Nro_Ter & _
                     " AND pronro = " & Nro_Proceso
            objConn.Execute StrSql, , adExecuteNoRecords
        End If
        ...
        ''***********************************************************************************************
        ''***********************************************************************************************
        ''Diego Rosso - 29-08-2010
        ''Borro las Novedades retroactivas generadas por el simulador.
        ''Diego Rosso - 16-01-2011
        '' viejo: El campo pronro de novretro nos dice en que proceso se va a pagar
        '' viejo: la novedad por lo que si ese proceso esta liquidado no debo borrar la novedad
        ''Nuevo 09/02/2011: Cuando una novedad esta liquidada en real tiene asociado el numero de proceso
        ''       que la liquido en el campo pronropago(q deberia ser el mismo que pronro)
        'StrSql = " DELETE FROM novretro" & _
        '         " WHERE simpronro = " & Nro_Proceso & _
        '         " AND ternro =" & Nro_Ter & _
        '         " AND  ((pronropago IS NULL) OR (pronropago = 0)) "
        'objConn.Execute StrSql, , adExecuteNoRecords
        ''***********************************************************************************************
        ''***********************************************************************************************
        
        'FGZ - 04/11/2011 ------------------------------------------------------------------------------------
        'Creo que las condiciones del where estan mal dado que el procedimiento CalcularDiferencia inserta novretro
        ' con valor en el campo pronropago y luego nunca se borran y esto hace que cada vez que se simula se generen mas novedades
        StrSql = " DELETE FROM novretro "
        StrSql = StrSql & " WHERE simpronro = " & Nro_Proceso
        StrSql = StrSql & " AND ternro = " & Nro_Ter
        'strsql = strsql & " AND  ((pronropago IS NULL) OR (pronropago = 0)) "
        objConn.Execute StrSql, , adExecuteNoRecords
        'FGZ - 04/11/2011 ------------------------------------------------------------------------------------
...
On Error GoTo ME_Local
    ...
    StrSql = "DELETE FROM sim_vacvendidos WHERE pronro= " & pronro & " AND ternro= " & Tercero
    objConn.Execute StrSql, , adExecuteNoRecords
...
Public Sub Liqpro04_old(ByVal Cabecera As Long, ByVal Nro_Ter As Long, ByVal Nro_Proceso As Long, ByVal Nro_Periodo As Long, ByVal Excluye_Liq As Boolean)
' --------------------------------------------------------------------------
' Descripcion:  Desmarca los registros generados por el liquidador.
'               Usado para reliquidaciones y para excluir empleados
'               de un proceso de Liquidación.
...
Principal:
    ...
    '30/06/2009 - Martin Ferraro - Si no existe det
    StrSql = "SELECT cliqnro FROM sim_detliq WHERE cliqnro = " & Cabecera
    OpenRecordset StrSql, rs_Procesos
    If rs_Procesos.EOF And Usa_grossing = False Then
        If CBool(USA_DEBUG) Then Flog.writeline Espacios(Tabulador * 1) & "No hay datos de liquidacion a borrar del legajo " & Legajo
    ...
    If Not rs_Periodo.EOF Then
        ...
        End If
        StrSql = "SELECT * FROM sim_detliq WHERE cliqnro =" & Cabecera
        OpenRecordset StrSql, rs_Detliq
        
        If rs_Detliq.EOF Then
            If CBool(USA_DEBUG) Then
                Flog.writeline Espacios(Tabulador * 2) & "No se encontraron liquidaciones"
            End If
        End If
        '****************No se usa en la simulacion*****************************
'        Do While Not rs_Detliq.EOF
...
'                                 " AND acunro =" & rs_Acu_Liq!acunro
            ...
            If CBool(rs_Acu_Liq!acuimponible) Or CBool(rs_Acu_Liq!acuimpcont) Then
                'RESTAR DE LAS ESTRUCTURAS DE IMPONIBLE
                StrSql = "SELECT * FROM sim_impproarg " & _
                         " WHERE cliqnro =" & Cabecera & _
                         " AND acunro =" & rs_Acu_Liq!acuNro
                OpenRecordset StrSql, rs_ImpPro
            
                Do While Not rs_ImpPro.EOF  'por cada imponible
                    StrSql = "SELECT * FROM sim_impmesarg " & _
                             " WHERE imaanio =" & rs_Periodo!pliqanio & _
                             " AND ternro =" & Nro_Ter & _
                             " AND imames =" & rs_Periodo!pliqmes & _
                             " AND tconnro =" & rs_ImpPro!tconnro & _
...
'        objConn.Execute StrSql, , adExecuteNoRecords
        ...
        If CBool(USA_DEBUG) Then
            Flog.writeline Espacios(Tabulador * 1) & "Limpio la traza"
        End If
...
'-----------------------------------------------
        ...
        If CBool(USA_DEBUG) Then
            Flog.writeline Espacios(Tabulador * 1) & "BORRA LAS CUOTAS DE PRESTAMOS GENERADOS POR EL PROCESO"
        End If
        StrSql = "SELECT * FROM sim_prestamo " & _
                 "INNER JOIN sim_pre_cuota ON sim_prestamo.prenro = sim_pre_cuota.prenro" & _
                 " WHERE sim_prestamo.ternro = " & Nro_Ter & _
                 " AND sim_pre_cuota.pronro =" & Nro_Proceso & _
                 " AND sim_pre_cuota.cuocancela = -1"
        OpenRecordset StrSql, rs_Prestamos

        Do While Not rs_Prestamos.EOF
            StrSql = "UPDATE sim_pre_cuota SET pronro = null, cuocancela = 0 " & _
                     " WHERE pronro =" & Nro_Proceso & _
            ...
            If rs_Prestamos!estnro = 6 Then
                ' Lo apruebo para re-liq la cuota
                StrSql = "UPDATE sim_prestamo SET estnro = 3 " & _
                         " WHERE ternro = " & rs_Prestamos!Ternro & _
                         " AND prenro =" & rs_Prestamos!prenro
                 objConn.Execute StrSql, , adExecuteNoRecords
            End If
...
'            " ORDER BY altfec"
    ...
    StrSql = "SELECT * FROM sim_fases WHERE real = -1 AND empleado = " & buliq_empleado!Ternro & _
...
'            End If
    ...
    If Contrato_proporcion = 0 Then
        ...
        " FROM sim_his_estructura, replica_estr " & _
        " WHERE tenro = 18 " & _
        " AND sim_his_estructura.ternro = " & buliq_empleado!Ternro & _
        " AND sim_his_estructura.htetdesde <=" & ConvFecha(Fecha_Fin) & _
        " AND (sim_his_estructura.htethasta >= " & ConvFecha(Fecha_Inicio) & _
        " OR sim_his_estructura.htethasta IS NULL)" & _
        " AND replica_estr.estrnro = sim_his_estructura.estrnro "
...
'Contrato_proporcion = buliq_empleado!tcantnro
            ...
            Do While Not rs_WF_impproarg.EOF
                ...
                OpenRecordset StrSql, rs_ImpMesArg
                    ...
                    If rs_WF_impproarg!tconnro = 3 Then
                        ...
                        StrSql = "SELECT altfec FROM sim_fases WHERE estado = -1  AND real = -1 AND empleado = " & buliq_empleado!Ternro
                        OpenRecordset StrSql, rs_Fases
                        If Not rs_Fases.EOF Then
                        ...
                        StrSql = "SELECT sum(imamonto) SUMA ,sum(imacant) CANTIDAD FROM sim_impmesarg " & _
                                 " WHERE ternro = " & buliq_empleado!Ternro & _
                                 " AND acunro = " & rs_WF_impproarg!acuNro & _
                                 " AND tconnro = " & rs_WF_impproarg!tconnro & _
                    ...
                    Case 1:
                        'EAM (6.75) - Se modifico la forma de calcular el AMPO
                    ...
                    Case 1:
                        ...
                        If (rs_WF_impproarg!ipamonto) > Tope_Monto_Proporcional Then
                            ...
                            End If
                            'EAM(6.75) - Se modificó como se obtiene el tope máximo
  ...
  Else
                ...
                If rs_ImpPro.EOF Then
                    StrSql = "INSERT INTO sim_impproarg (cliqnro,acunro,ipacant,ipamonto,tconnro)" & _
                ...
                Else
                    ' Actualizo
                    StrSql = "UPDATE sim_impproarg SET ipamonto = " & rs_WF_impproarg!ipamonto & _
...
'
        ...
        If Not Indice_Actual_Cge_Segun = 0 Then
            ...
            If Arr_Cge_Segun(Indice_Actual_Cge_Segun).Nivel = 1 Then
                ...
                StrSql = " SELECT tenro, estrnro FROM sim_his_estructura " & _
...
If CBool(Arr_conceptos(Concepto_Actual).Concajuste) Then
        ...
        StrSql = "SELECT * FROM sim_novaju " & _
        ...
        Do While Not rs_NovAjuste.EOF
            If FirmaActiva20 Then
                '/* Verificar si esta en el NIVEL FINAL DE FIRMA */
                    StrSql = "select * from cysfirmas where cysfiryaaut = -1 AND cysfirfin = -1 " & _
...
SiguienteAjuste:
            ...
            If HayAjuste Then
                StrSql = "INSERT INTO sim_detliq (" & _
...
'        Resultado = Round(Resultado, Arr_Conceptos(Concepto_Actual).conccantdec)
        ...
        StrSql = "SELECT * FROM ampocontpa WHERE concnro = " & Arr_conceptos(Concepto_Actual).ConcNro & _
    ...
    End If 'Machetazo
        ...
        'If CBool(Concepto_Retroactivo) Then
            ...
            If YA_Ajustado Then
                ...
                StrSql = "UPDATE sim_detliq set ajustado = -1 WHERE " & _
...
'
                    If CBool(Arr_Acumulador(Aux_Acu_Actual).acutopea) Then
                        ...
                        StrSql = "SELECT * FROM " & TTempWF_impproarg & " WHERE acunro = " & Arr_Acumulador(Aux_Acu_Actual).acuNro & _
...
Do While Acumulador_Actual <> 0
    ...
    If objCache_Acu_Liq_Monto.EsSimboloDefinido(CStr(Acum)) Then
        ...
        End If
        ' Crear acu_liq
        StrSql = "SELECT * FROM sim_acu_liq WHERE acunro = " & Acum & _
                 " AND cliqnro = " & buliq_cabliq!cliqnro
        OpenRecordset StrSql, rs_AcuLiq
        
        If rs_AcuLiq.EOF Then
            ' Inserto
            StrSql = "INSERT INTO sim_acu_liq (" & _
                     ...
                     " )"
        Else
            StrSql = "UPDATE sim_acu_liq SET " & _
...
If CBool(USA_DEBUG) Then
    Flog.writeline Espacios(Tabulador * 1) & "-------------------------------------------------------------------------------"
    Flog.writeline Espacios(Tabulador * 1) & " SUMAR ACUMULADORES DEL PROCESO A LOS MENSUALES: acu_mes e impmesarg"
    Flog.writeline Espacios(Tabulador * 1) & "-------------------------------------------------------------------------------"
End If
' -------------------------------------------------------------------------------
...
Do While Not rs_AcuLiq.EOF
    If CBool(Arr_Acumulador(rs_AcuLiq!acuNro).acumes) Then
        ...
        StrSql = "SELECT * FROM sim_impproarg WHERE acunro = " & Arr_Acumulador(rs_AcuLiq!acuNro).acuNro & _
                 " AND cliqnro = " & buliq_cabliq!cliqnro
        OpenRecordset StrSql, rs_ImpPro
        
            StrSql = "SELECT * FROM sim_acu_mes WHERE acunro = " & Arr_Acumulador(rs_AcuLiq!acuNro).acuNro & _
            ...
            If rs_acumes.EOF Then
                ' Inserto
                StrSql = "INSERT INTO sim_acu_mes (acunro,amanio, ternro, ammes,ammonto,ammontoreal, amcant " & _
                         ") VALUES (" & _
            ...
            ' Actualizo
            StrSql = " UPDATE sim_acu_mes SET ammonto = ammonto + " & rs_AcuLiq!almonto & _
                ...
                If CBool(Arr_Acumulador(rs_AcuLiq!acuNro).acuimponible) Or CBool(Arr_Acumulador(rs_AcuLiq!acuNro).acuimpcont) Then
                    ...
                    Do While Not rs_ImpPro.EOF
                        ...
                        If rs_ImpMesArg.EOF Then
                            ...
                            StrSql = "INSERT INTO sim_impmesarg (acunro,imaanio, imames, ternro, tconnro, imamonto, imacant" & _
                                     ") VALUES (" & _
                                     Arr_Acumulador(rs_AcuLiq!acuNro).acuNro & _
                                     "," & buliq_periodo!pliqanio & _
                                     "," & buliq_periodo!pliqmes & _
...
'--------------------------------------------------------------------------------------------------------
StrSql = "SELECT * FROM " & TTempWF_Retroactivo
StrSql = StrSql & " WHERE concnro = " & Arr_conceptos(Concepto_Actual).ConcNro
OpenRecordset StrSql, rs_wf_Retro
Do While Not rs_wf_Retro.EOF
    ...
    Do While Not rs_acumulador.EOF
        ...
        If rs_Acu_Mes.EOF Then
            StrSql = "INSERT INTO sim_acu_mes (acunro,amanio,ammes,ternro,ammonto) VALUES (" & rs_acumulador!acuNro
            ...
            objConn.Execute StrSql, , adExecuteNoRecords
        Else
            StrSql = "UPDATE sim_acu_mes SET ammonto = ammonto + " & Result & _
...
Do While Not rs_wf_Retro.EOF
    ...
    Do While Not rs_acumulador.EOF
        ...
        If rs_Acu_Mes.EOF Then
            ...
            StrSql = StrSql & "," & rs_wf_Retro!Mes
            StrSql = StrSql & "," & buliq_empleado!Ternro
            StrSql = StrSql & "," & rs_wf_Retro!Monto & ")"
            objConn.Execute StrSql, , adExecuteNoRecords
        Else
            StrSql = "UPDATE sim_acu_mes SET ammonto = ammonto + " & rs_wf_Retro!Monto & _
                     " WHERE acunro = " & rs_acumulador!acuNro & _
                     " AND ternro = " & buliq_empleado!Ternro & _
                     " AND ammes = " & rs_wf_Retro!Mes & _
                     " AND amanio = " & rs_wf_Retro!Anio
        ...
        If rs_his_retro.EOF Then
            ...
            objConn.Execute StrSql, , adExecuteNoRecords
        Else
            StrSql = "UPDATE hisretroactivo SET ammonto = ammonto + " & rs_wf_Retro!Monto & _
                     " , dlimonto = dlimonto + " & rs_wf_Retro!Monto & _
                     " WHERE acunro = " & rs_acumulador!acuNro & _
                     " AND amanio = " & rs_wf_Retro!Anio & _
                     " AND ammes = " & rs_wf_Retro!Mes & _
                     " AND cliqnro = " & Cabecera_liq & _
...
Dim rs_AmpoAux As New ADODB.Recordset
    ...
    StrSql = "SELECT * FROM ampo WHERE ampofecha <= " & ConvFecha(buliq_proceso!profecfin)
    'Se agrego el orden
    StrSql = StrSql & " ORDER BY ampofecha DESC"
    OpenRecordset StrSql, rs_AmpoAux

    If Not rs_AmpoAux.EOF Then
        'Se quito la sgt linea
        'rs_AmpoAux.MoveLast
        If CBool(USA_DEBUG) Then
            Flog.writeline Espacios(Tabulador * 1) & "AMPO Fecha de Vigencia " & rs_AmpoAux!ampofecha
        End If
        
        StrSql = "SELECT * FROM ampo WHERE ampofecha = " & ConvFecha(rs_AmpoAux!ampofecha)
        ...
        Do While Not rs_Ampo.EOF
            ...
            Select Case rs_Ampo!ampotconnro
            Case 1:
                Cant_Diaria_Ampos_1 = rs_Ampo!ampodiario
                Cant_Ampo_Proporcionar_1 = 0
                Ampo_Proporciona_1 = CBool(rs_Ampo!ampopropo)
                Ampo_Max_1 = rs_Ampo!ampomax
                Ampo_Min_1 = rs_Ampo!ampomin
            Case 2:
                Cant_Diaria_Ampos_2 = rs_Ampo!ampodiario
                Cant_Ampo_Proporcionar_2 = 0
                Ampo_Proporciona_2 = CBool(rs_Ampo!ampopropo)
                Ampo_Max_2 = rs_Ampo!ampomax
                Ampo_Min_2 = rs_Ampo!ampomin
            Case 3:
                If EsNulo(rs_Ampo!ampomax) Then
                    Cant_Diaria_Ampos_3 = 30
                Else
                    Cant_Diaria_Ampos_3 = 30 / IIf(rs_Ampo!ampomax = 0, 1, rs_Ampo!ampomax)
                End If
                Cant_Ampo_Proporcionar_3 = 0
                Ampo_Proporciona_3 = CBool(rs_Ampo!ampopropo)
                Ampo_Max_3 = rs_Ampo!ampomax
                Ampo_Min_3 = rs_Ampo!ampomin
            Case 4:
                Cant_Diaria_Ampos_4 = rs_Ampo!ampodiario
                Cant_Ampo_Proporcionar_4 = 0
                Ampo_Proporciona_4 = CBool(rs_Ampo!ampopropo)
                Ampo_Max_4 = rs_Ampo!ampomax
                Ampo_Min_4 = rs_Ampo!ampomin
            Case 5:
                Cant_Diaria_Ampos_5 = rs_Ampo!ampodiario
                Cant_Ampo_Proporcionar_5 = 0
                Ampo_Proporciona_5 = CBool(rs_Ampo!ampopropo)
                Ampo_Max_5 = rs_Ampo!ampomax
                Ampo_Min_5 = rs_Ampo!ampomin
            End Select
...
' -----------------------------------------------------------------------------------
    ...
    On Error GoTo ME_Local
        ...
        'En simulacion No hay Anticipos
...
On Error GoTo ME_Local
    ...
    StrSql = "SELECT *"
    StrSql = StrSql & " FROM sim_proceso INNER JOIN"
    StrSql = StrSql & " tipoproc ON sim_proceso.tprocnro = tipoproc.tprocnro"
    StrSql = StrSql & " AND tprocrecalculo = -1"
    StrSql = StrSql & " Where sim_proceso.pronro = " & Proceso
...
On Error GoTo ME_Local
    ...
    StrSql = StrSql & " WHERE simpronro =" & buliq_proceso!pronro
    StrSql = StrSql & " AND ternro =" & buliq_empleado!Ternro
    objConn.Execute StrSql, , adExecuteNoRecords
...
On Error GoTo ME_Local
    ...
    StrSql = "UPDATE sim_gastos SET pronro = null "
    StrSql = StrSql & " WHERE pronro = " & buliq_proceso!pronro
    StrSql = StrSql & " AND ternro = " & buliq_empleado!Ternro
    objConn.Execute StrSql, , adExecuteNoRecords
...
On Error GoTo ME_Local
        ...
        'Busco las cuotas que fueron generadas por el proceso
        StrSql = "SELECT cuonro, cuototal, cuosaldo, cuogenera , pronrogenera, cuoordenliq "
        StrSql = StrSql & " FROM sim_pre_cuota"
        StrSql = StrSql & " INNER JOIN sim_prestamo ON sim_prestamo.prenro = sim_pre_cuota.prenro"
        StrSql = StrSql & " WHERE pronrogenera = " & buliq_proceso!pronro
        StrSql = StrSql & " AND sim_prestamo.ternro = " & buliq_empleado!Ternro
        StrSql = StrSql & " ORDER BY cuonrocuo, cuoordenliq"
        OpenRecordset StrSql, rs_Prestamos
        Do While Not rs_Prestamos.EOF
            
            'Actualizo la cuota que particiono
            StrSql = "UPDATE sim_pre_cuota"
            'If TodoNada Then
            '    StrSql = StrSql & " SET cuototal = " & rs_Prestamos!cuototal
            'Else
                StrSql = StrSql & " SET cuototal = cuototal + " & rs_Prestamos!cuototal
            ...
            'Borro la cuota generada
            StrSql = "DELETE sim_pre_cuota WHERE cuonro = " & rs_Prestamos!cuonro
            objConn.Execute StrSql, , adExecuteNoRecords
        ...
        StrSql = "SELECT sim_prestamo.estnro, sim_prestamo.prenro,cuonro FROM sim_prestamo " & _
                 "INNER JOIN sim_pre_cuota ON sim_prestamo.prenro = sim_pre_cuota.prenro" & _
                 " WHERE sim_prestamo.ternro = " & buliq_empleado!Ternro & _
                 " AND sim_pre_cuota.pronro =" & buliq_proceso!pronro & _
                 " AND sim_pre_cuota.cuocancela = -1"
        OpenRecordset StrSql, rs_Prestamos
        ...
        Do While Not rs_Prestamos.EOF
            ...
            StrSql = "UPDATE sim_pre_cuota SET cuosaldo = (cuosaldo + cuocancelado), cuototal = cuocancelado, cuocancelado = NULL "
            StrSql = StrSql & " WHERE pronro = " & buliq_proceso!pronro
            StrSql = StrSql & " AND prenro = " & rs_Prestamos!prenro
            StrSql = StrSql & " AND cuonro = " & rs_Prestamos!cuonro
            ...
            'Es una cuota cancelada, la marco como no cancelada
            StrSql = "UPDATE sim_pre_cuota SET pronro = null, cuocancela = 0 "
            StrSql = StrSql & " WHERE pronro = " & buliq_proceso!pronro
            StrSql = StrSql & " AND prenro = " & rs_Prestamos!prenro
            StrSql = StrSql & " AND cuonro = " & rs_Prestamos!cuonro
            objConn.Execute StrSql, , adExecuteNoRecords
...
Dim rs_Embargos As New ADODB.Recordset
    ...
    End If
    StrSql = "SELECT * FROM sim_embargo" & _
             " INNER JOIN tipoemb ON sim_embargo.tpenro = tipoemb.tpenro" & _
             " INNER JOIN sim_embcuota ON sim_embargo.embnro = sim_embcuota.embnro" & _
             " WHERE sim_embargo.ternro =" & Nro_Ter & _
             " AND sim_embcuota.pronro =" & Nro_Proceso & _
             " ORDER BY sim_embargo.embnro, sim_embcuota.embcnro "
    ...
    Do While Not rs_Embargos.EOF
        ...
        If (CInt(rs_Embargos!tpefordesc) < 1) Then ' El embargo es por monto
            If (CDbl(rs_Embargos!embcimpreal) = 0) And (CInt(rs_Embargos!embccancela) = 0) Then
                'Borro la cuota generada
                StrSql = "DELETE FROM sim_embcuota " & _
            ...
            Else 'Marco la cuota como no cancelada
                StrSql = "UPDATE sim_embcuota SET embccancela = 0 " & _
                        " , embcimpreal = 0" & _
                        " , pronro = null" & _
                        " WHERE embnro =" & rs_Embargos!embnro & _
                        " AND pronro =" & Nro_Proceso & _
...
'            End If
        ...
        If rs_Embargos!embest = "F" Then
            StrSql = "UPDATE sim_embargo SET embest = 'A' " & _
    ...
    StrSql = "UPDATE sim_embargo SET embest = embestant"
    StrSql = StrSql & ", pronro = 0"
    StrSql = StrSql & " WHERE sim_embargo.ternro =" & Nro_Ter
    StrSql = StrSql & " AND pronro =" & Nro_Proceso
...
' ---------------------------------------------------------------------------------------------
    ...
    If ReusaTraza Then
        StrSql = "UPDATE sim_traza SET cliqnro = -1, travalor = 0"
        StrSql = StrSql & " ,concnro = -1 ,tpanro = -1, tradesc = NULL"
        StrSql = StrSql & " WHERE cliqnro = " & Cabecera & " AND concnro = " & Concepto
        
    Else
        StrSql = "DELETE FROM sim_traza WHERE cliqnro = " & Cabecera & " AND concnro = " & Concepto
    End If
    objConn.Execute StrSql, , adExecuteNoRecords
End Sub
...
On Error GoTo MLocal:
    ...
    If ReusaTraza Then
        ...
        StrSql = "SELECT " & TOP(1) & " tranro FROM sim_traza " & NOLOCK & " WHERE cliqnro = -1 AND travalor = 0"
        ...
        Else
            ...
            StrSql = "INSERT INTO sim_traza (cliqnro,concnro,tpanro,tradesc,travalor,trafrecuencia)"
...
MLocal:
        ...
        Flog.writeline
End Sub
Public Sub CalcularDiferencias(NroCab As Long, NroEmpleado As Long, NroProc As Long, NroPer As Long, Traza As Boolean)
'Call CalcularDiferencias(buliq_cabliq!cliqnro, buliq_cabliq!Empleado, buliq_cabliq!pronro, buliq_proceso!PliqNro)
' --------------------------------------------------------------------------
' Descripcion:  Proceso que calcula las diferencias contra un proceso real
' Autor:        Diego Rosso
' Fecha:        02-08-2010
' Ultima Mod:   07-10-2010 - Diego Rosso - Estaba leyendo la cantidad mal ya que la tomaba de un recordset vacio
' Ultima Mod:   02-12-2010 - Diego Rosso - Cuando se crean las novedades retro en el campo pronro debe ir código del proceso de pago
'                                          asociado al proceso de Simulacion, campo sim_proceso.pronropago
'                                          (hoy esta poniendo el código de proceso real campo sim_proceso.pronroreal)
' Ultima Mod:   11-12-2010 - Diego Rosso - Se agrega la busqueda del pliqnro real ya que el que estaba poniendo en el alta de las
'                                           novedeades retro era el pago.
' --------------------------------------------------------------------------

' Definicion de Variable Locales
Dim Cantidad As Integer
Dim Monto As Double
Dim PliqnroReal As Integer

' Registros
Dim rs_Sim_Proceso As New ADODB.Recordset
Dim rs_sim_Detliq As New ADODB.Recordset
Dim rs_Detliq As New ADODB.Recordset
Dim rs_ProcesoReal As New ADODB.Recordset
Dim rs_NR As New ADODB.Recordset
    ...
    '1º levanto los parametros de Proceso.
    
    StrSql = " SELECT sp.pronro, sp.prodesc, sp.pliqnro, sp.profecpago, sp.empnro, sp.protipoSim," & _
             " sp.pronroreal, sp.tprocnroreal, sp.pliqnroreal, sp.profecplan, sp.profecini," & _
             " sp.profecfin, sp.proconceptos, sp.procajuretro, sp.pronropago" & _
             " FROM sim_proceso sp " & _
             " WHERE  sp.pronro= " & NroProc
            
    OpenRecordset StrSql, rs_Sim_Proceso
      If Not rs_Sim_Proceso.EOF Then
        'FGZ - 09/08/2011 -------
        'Le agregué control por Nulo
        If Not EsNulo(rs_Sim_Proceso!procajuretro) Then
          '2º Lo primero que me fijo es que sea un proceso de simulación de retroactivos. sino no hago nada
          If CBool(rs_Sim_Proceso!procajuretro) Then
             'aca va toda la logica
             
             '3º Levanto todos los conceptos liquidados (simulado) que tengan la marca de retroactivo automatico (campo concajuretro)
               StrSql = " SELECT sd.concnro, sd.dlimonto, sd.dlifec, sd.dlicant, sd.dlimonto_base, " & _
                        " sd.dlitexto, sd.fornro, sd.dliqdesde, sd.dliqhasta, sd.dliretro, " & _
                        " sd.ajustado, sd.tconnro, sd.dliporcent, sd.cliqnro " & _
                        " FROM sim_detliq sd " & _
                        " INNER JOIN concepto c ON c.concnro = sd.concnro " & _
                        " WHERE c.concajuretro = -1 AND sd.cliqnro = " & NroCab
              OpenRecordset StrSql, rs_sim_Detliq
              
              
              'Importante: Busco el pliqnro del proceso real porque el que tengo en sim_proceso(pliqnroreal) en realidad es el pliqnro de pago
              'Agregado el 11-12-2010
              StrSql = " SELECT p.pliqnro " & _
                       " FROM proceso p " & _
                       " WHERE p.pronro = " & rs_Sim_Proceso!pronroreal
              OpenRecordset StrSql, rs_ProcesoReal
              PliqnroReal = 0
              If Not rs_ProcesoReal.EOF Then
                  PliqnroReal = rs_ProcesoReal!PliqNro
              End If
              
              'Comparo concepto a concepto
              Do While Not rs_sim_Detliq.EOF
                  
                  '4º Busco en la liquidacion real si existe el mismo concepto para el empleado.(rs_Sim_Proceso!pronroreal)
                  StrSql = " SELECT d.concnro, d.dlimonto, d.cliqnro, d.dlicant, d.dlifec " & _
                           " FROM detliq d " & _
                           " INNER JOIN cabliq c ON c.cliqnro = d.cliqnro " & _
                           " WHERE c.pronro =  " & rs_Sim_Proceso!pronroreal & _
                           " AND c.empleado = " & NroEmpleado & _
                           " AND d.concnro =  " & rs_sim_Detliq!ConcNro
                  OpenRecordset StrSql, rs_Detliq
                  If rs_Detliq.EOF Then
                      'No existe el concepto en real. Grabo el monto simulado.
                      Monto = rs_sim_Detliq!dlimonto
                      Cantidad = rs_sim_Detliq!dlicant
                  Else
                      'Si existe en real tengo que restar el simulado con el real
                      Monto = rs_sim_Detliq!dlimonto - rs_Detliq!dlimonto
                      Cantidad = rs_Detliq!dlicant
                  End If
                  
                  'FGZ - 02/08/2013 ---------------------------
                  ' Busco nov retroactivas creadas ya al proceso y concepto para crear una nueva ajsutada
                    StrSql = " SELECT sum(nretromonto) monto, sum(nretrocant) cant  FROM novretro "
                    StrSql = StrSql & " WHERE pronro =  " & IIf(EsNulo(rs_Sim_Proceso!pronropago), 0, rs_Sim_Proceso!pronropago)
                    StrSql = StrSql & " AND ternro = " & NroEmpleado
                    'StrSql = StrSql & " AND pliqnro = " & PliqnroReal
                    StrSql = StrSql & " AND concnro = " & rs_sim_Detliq!ConcNro
                    OpenRecordset StrSql, rs_NR
                    If Not rs_NR.EOF Then
                        Monto = Monto - IIf(EsNulo(rs_NR!Monto), 0, rs_NR!Monto)
                        Cantidad = Cantidad - IIf(EsNulo(rs_NR!Cant), 0, rs_NR!Cant)
                    End If
                  'FGZ - 02/08/2013 ---------------------------
                  
                    If Monto <> 0 Then
                        '5º Insertar en NOVRETRO
                        StrSql = " INSERT INTO novretro ("
                        StrSql = StrSql & " ternro, concnro, nretromonto, nretrocant, pliqnro, simpronro, pronro  )"
                        StrSql = StrSql & " VALUES ( " & NroEmpleado
                        StrSql = StrSql & ", " & rs_sim_Detliq!ConcNro
                        StrSql = StrSql & ", " & Monto
                        StrSql = StrSql & ", " & Cantidad
                        'StrSql = StrSql & ", " & IIf(EsNulo(rs_Sim_Proceso!pliqnroreal), 0, rs_Sim_Proceso!pliqnroreal)
                        StrSql = StrSql & ", " & PliqnroReal
                        StrSql = StrSql & ", " & NroProc
                        StrSql = StrSql & ", " & IIf(EsNulo(rs_Sim_Proceso!pronropago), 0, rs_Sim_Proceso!pronropago)
                        StrSql = StrSql & " )"
                        objConn.Execute StrSql, , adExecuteNoRecords
                    End If
    
                  'Siguiente concepto
                  rs_sim_Detliq.MoveNext
              Loop
          End If
        End If
    End If
...
CE:
    ...
    'If CBool(USA_DEBUG) Then
        Flog.writeline
        Flog.writeline Espacios(Tabulador * 0) & "**********************************************************"
        Flog.writeline Espacios(Tabulador * 0) & " Error en CalcularDiferencias "
        Flog.writeline Espacios(Tabulador * 0) & " Empleado abortado: " & NroEmpleado
        Flog.writeline Espacios(Tabulador * 0) & " Error: " & Err.Description
        If InStr(1, Err.Description, "ODBC") > 0 Then
            'Fue error de Consulta de SQL
            Flog.writeline
