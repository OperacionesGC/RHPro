...
namespace ConsultaBaseC
{
    ...
    public class Consultas : System.Web.Services.WebService
    {
        ...
        public string Patch(string Base, string Idioma)
        {
            ...
            sql = "SELECT top 1 patchdesabr FROM patch ORDER BY patchnro DESC";
            da = new SqlDataAdapter(sql, cn);
            try
            {
                da.Fill(ds);
        ...
        public DataTable Link(string Usuario, string Base, string Idioma)
        {
            ...
            sql = "SELECT hlinktitulo, hlinkpagina FROM user_link";
        ...
        public DataTable Mensaje(string Base, string Idioma)
        {
            ...
            sql = "SELECT hmsjtitulo, hmsjcuerpo from home_mensaje";
        ...
        public DataTable Modulos(string Usuario, string Base, string Idioma)
        {
            ...
            if (Usuario.Trim().Length == 0)
            {
                ...
                sql = sql + " from menumstr where menuraiz = 74  and menuactivo = -1 ORDER BY menudesabr";
                da = new SqlDataAdapter(sql, cn);
                try
                {
                    da.Fill(ds);
            ...
            else
            {
                ...
                if (dsAux.Tables[0].Rows.Count > 0)
                {
                    ...
                    sql = "select menuaccess,menudesabr,menudetalle, 'abrirVentana(' + CHAR(39) + action + CHAR(39) + ','''',670,520)' action,menuobjetivo,menubeneficio,linkmanual,linkdvd";
                    ...
                    if (dsAux2.Tables[0].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow fila in tablaAux.Rows)
                        {
                            ...
                            if (Hay)
                                filaAux["action"] = fila["action"].ToString();
                            else
                                filaAux["action"] = "";
        ...
        public DataTable MRU(string Usuario, int Cant, string Base, string Idioma)
        {
            ...
            sql = "SELECT TOP (" + Cant + ") menumstr.menuname, menumstr.action, menuraiz.menunombre raiz, menuraiz.menudir FROM mru";
        ...
        public DataTable PagPie(string Usuario, string Base, string Idioma)
        {
            ...
            sql = "SELECT hpptitulo, hpppagina FROM home_pagpie";
            sql = sql + " WHERE UPPER(iduser) = '" + Usuario.ToUpper() + "'";
        ...
        {
            //Encriptar.Decrypt("56238",Encriptar.Encrypt("56238", "mferraro@&mferraro123"))
        ...
        public DataTable Login(string Usuario, string Pass, string SegNt, string Base, string Idioma)
        {
            ...
            //--------------------------------------------------------------------------------
            SqlConnection connUsu = new SqlConnection(DAL.constrUsu(Usuario, Encriptar.Encrypt(DAL.EncrKy(), Pass), SegNt, Base));
            try
            {
                connUsu.Open();
            }
            ...
            if ((Ingresa) && (SegNt != "-1"))
            {
                ...
                if (Password.valorHistPass(Usuario, "husrpass", Base) != Encriptar.Encrypt(DAL.EncrKy(), Pass))
                {
                    ...
                    passIntFallidos = (Password.valorUserPolCuenta(Usuario, "pass_int_fallidos", Base).Length != 0) ? Convert.ToInt64(Password.valorUserPolCuenta(Usuario, "pass_int_fallidos", Base)) : 0;
        ...
        public long Padre(long CodEmp, int Base)
        {
            ...
            sql = "SELECT empreporta, ternro CodEmp FROM empleado WHERE empleado.ternro = " + CodEmp.ToString();
        ...
        public DataTable Hijos(long CodEmp, int Base)
        {
            ...
            sql = "SELECT ternro CodEmp FROM empleado WHERE empleado.empreporta = " + CodEmp.ToString();
            sql = sql + " AND empleado.empest = -1";
            da = new SqlDataAdapter(sql, cn);
            try
            {
        ...
        public DataTable DatosEmp(long CodEmp, int Base)
        {
            ...
            sql = "SELECT empleado.empleg legajo, terape apellido,";
            sql = sql + " terape2 apellido2, ternom nombre, ternom2 nombre2, empemail mail, empinterno interno, empleado.ternro";
        ...
        private string Estructura(long CodEmp, long TeNro, int Base)
        {
            ...
            sql = sql + " AND (his_estructura.htetdesde <= " + Fecha.cambiaFecha(FechaAct, Base.ToString());
            sql = sql + " AND (his_estructura.htethasta is null or his_estructura.htethasta >= " + Fecha.cambiaFecha(FechaAct, Base.ToString()) + "))";
        ...
        public DataTable BuscarEmpleado(long Legajo, long Activo, int Base)
        {
            ...
            if (Legajo != 0)
                sql = "SELECT empleg Legajo, ternro CodEmp, terape + ' ' + terape2 apellido, ternom + ' ' + ternom2 nombre";
            else
                sql = "SELECT TOP(1) empleg legajo, ternro CodEmp, terape + ' ' + terape2 apellido, ternom + ' ' + ternom2 nombre";
            sql = sql + " FROM empleado";
/*          switch (Activo)
            {
                case -1:
